...
- Overview, inventory, model-matrix, and other table-only pages must also fill the usable page area. Do not leave a large blank lower half just because the page has only a few rows. Increase table font size, row height, padding, and block height, or split the overview into separate full-page tables. Verify rendered preview pages at fit-page scale; if the content still looks like a small top strip, regenerate before returning the report.
- For AI/model price-rank report pages, always include a relative price score in addition to raw provider pricing. Use the same numeric price basis that determines the rank, set the lowest nonzero basis to `1.00`, and show every other row as `price_basis / lowest_basis` rounded clearly, such as `1.20` or `1.30`. Keep the raw input, cached-input, output, image/output unit, and source price fields visible when available.
