...
- Typography, CTA, nav, hero-copy, and compact-control alignment fixes need a cross-browser pass in Chrome/Chromium and Safari/WebKit when those engines are available. On mobile, include both a normal phone width and a narrow phone width when practical, such as 390px and 320px. Centered UI must be explicitly centered on both the container/layout axis and the actual text element, not inferred from padding or one browser's font metrics.
...
14. For browser-rendered UI, run or inspect the affected viewport in Chrome/Chromium and Safari/WebKit when available. For mobile typography or CTA alignment, include a narrow-width check when practical. Verify centered text, CTA labels, nav labels, and compact controls by rendered screenshot plus DOM measurements in both engines: compare text-center and container-center positions, and check `scrollWidth`/`clientWidth` plus `scrollHeight`/`clientHeight` for clipping. Treat cross-browser drift, off-center text, or clipped labels as blockers.
