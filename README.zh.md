...
浏览器测试默认使用 Codex 或 ChatGPT 内置浏览器。只有用户明确指定某个外部浏览器时才使用它；内置浏览器不可用时必须保留这一限制，不能静默切换并把替代结果当成等价证据。
...
**本次基准没有证明节省。** token 包含缓存输入。两组收到同一份完整明确的任务说明；样本、缓存和延迟限制结论，且本次单模型对比不衡量自适应选模或记忆生命周期的效率。独立真实工作流已验证：受规则约束的审查用 Astra/ultra，独立计算用 Luna/low，两者并行；汇总和可见的记忆 Ending 均用 Astra/ultra。

Windows 八个已安装 Skill 与源代码和 macOS 字节一致；**60 项测试通过，1 项按平台预期跳过**。八个进程函数均通过原生检查，子进程没有控制台。额外模型调用在完成前因 HTTP 401 失败；SSH 无法读取现有密钥环，隐藏的交互认证探测也未能启动。Windows 模型执行仍未验证，额外检查不计入 macOS 基准。
