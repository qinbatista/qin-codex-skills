...
class ResultModelDisclosureTests(unittest.TestCase):
    ...
    def test_complete_no_switch_disclosure_is_accepted(self):
        disclosure_text = "\n".join(["Complexity: 38/100 (standard) · Model: gpt-6-sol|ultra · Route: no switch", "Evidence: verified entry (no runtime receipt)"])
    ...
    def test_unverified_identity_and_inconsistent_no_switch_are_rejected(self):
        disclosure_text = "\n".join(["Complexity: 38/100 (standard) · Model: unverified|unverified · Route: no switch", "Model path: gpt-6-sol|ultra -> gpt-6-sol|max", "Evidence: task assignment (no runtime receipt)"])
        failures = module.validate_result_model_disclosure(disclosure_text)
    ...
    def test_target_failure_replays_to_verified_entry_pair(self):
        target_entry = {"status": "verified", "model": "gpt-6-sol", "effort": "high"}
        repaired = disclosure_module.render_disclosure(75, entry_resolution=target_entry)
        self.assertEqual(repaired, "\n".join(["Complexity: 75/100 (advanced) · Model: gpt-6-sol|high · Route: no switch", "Evidence: verified entry (no runtime receipt)"]))
        self.assertEqual(module.validate_result_model_disclosure(repaired), [])
        defective = "\n".join(["Complexity: 75/100 (advanced) · Model: retired-model|unknown · Route: no switch", "Evidence: configured selection (no runtime receipt)"])
    ...
    def test_runtime_receipt_is_higher_authority_than_verified_entry(self):
        disclosure_text = disclosure_module.render_disclosure(38, runtime_receipt={"requested_pair": "gpt-6-sol|high", "resolved_pair": "gpt-6-sol|high", "effective_pair": "gpt-6-sol|medium"}, entry_resolution={"status": "verified", "model": "gpt-6-sol", "effort": "high"})
        self.assertEqual(disclosure_text, "\n".join(["Complexity: 38/100 (standard) · Model: gpt-6-sol|medium · Route: fallback", "Model path: gpt-6-sol|high -> gpt-6-sol|medium", "Evidence: runtime receipt"]))
        self.assertEqual(module.validate_result_model_disclosure(disclosure_text), [])
    ...
    def test_priority_selection_displays_adaptive_model_movement(self):
        receipt = {
            ...
            "switch_change": "gpt-6-sol|medium->gpt-6-luna|low",
        }
        disclosure_text = disclosure_module.render_disclosure(12, runtime_receipt=receipt)
        self.assertEqual(disclosure_text, "\n".join([
            "Complexity: 12/100 (small) · Model: gpt-6-luna|low · Route: downgrade",
            "Model path: gpt-6-sol|medium -> gpt-6-luna|low",
    ...
    def test_selected_model_outside_old_catalog_keeps_observed_switch_path(self):
        receipt = {"requested_pair": "gpt-6-luna|low", "resolved_pair": "gpt-6-luna|low",
                   "effective_pair": "gpt-6-luna|low", "switch_direction": "downgrade",
                   "switch_change": "gpt-6-astra|ultra->gpt-6-luna|low"}
        value = disclosure_module.render_disclosure(9, runtime_receipt=receipt)
        self.assertIn("Route: downgrade", value)
        self.assertIn("Model path: gpt-6-astra|ultra -> gpt-6-luna|low", value)
    ...
    def test_default_disclosure_omits_duplicate_detail_lines(self):
        disclosure_text = disclosure_module.render_disclosure(1, entry_resolution={"status": "verified", "model": "gpt-6-sol", "effort": "high"})
    ...
    def test_multi_stage_disclosure_lists_every_stage_score_pair_and_status(self):
        summary = {
            "nodes": [
                ...
                },
                {
                    ...
                    "score": 32,
                    "band": "standard",
                    "requested_pair": "gpt-6-sol|medium",
                    "resolved_pair": "gpt-6-sol|medium",
                    "effective_pair": "gpt-6-sol|medium",
                    "model_evidence_source": "runtime_receipt",
                    "route_change": "no_switch",
                    "status": "pass",
                ...
                },
                {
                    ...
                    "score": 67,
                    "band": "complex",
                    "requested_pair": "gpt-6-sol|high",
                    "resolved_pair": "gpt-6-sol|high",
                    "effective_pair": "gpt-6-sol|high",
        ...
        rendered = disclosure_module.render_disclosure(
            84,
            entry_resolution={"status": "verified", "model": "gpt-6-sol", "effort": "max"},
            model_switch_summary=summary,
        )
        self.assertIn("Model stages (3):", rendered)
        self.assertIn("Implement lifecycle gate [result:implementation] · Complexity: 84/100 (advanced) · Model: gpt-6-sol|max", rendered)
        self.assertIn("Run focused tests [result:quick-test] · Complexity: 32/100 (standard) · Model: gpt-6-sol|medium", rendered)
        self.assertIn("Independent real verification [ending:ending-real] · Complexity: 67/100 (complex) · Model: gpt-6-sol|high", rendered)
    ...
    def test_runtime_receipt_replaces_wrong_or_verbose_leading_disclosure(self):
        ...
        wrong = "\n".join([
            "Complexity: 8/100 (small) · Model: gpt-6-sol|high · Route: no switch",
        ...
        self.assertTrue(normalized.startswith("Complexity: 8/100 (small) · Model: gpt-6-luna|low · Route: no switch\nEvidence: runtime receipt"))
        self.assertEqual(normalized.count("Complexity:"), 1)
        self.assertIn("Changed the value.", normalized)

        verbose = "\n".join([
            "Complexity: 8/100 (small)",
            "Current model: gpt-6-sol | high",
    ...
    def test_only_three_active_model_families_are_allowed(self):
        allowed_pairs = disclosure_module._allowed_pairs()
        self.assertEqual({pair.split("|", 1)[0] for pair in allowed_pairs}, {"gpt-6-luna", "gpt-6-sol", "gpt-6-astra"})
        self.assertIn("gpt-6-sol|high", allowed_pairs)
        self.assertIn("gpt-6-sol|medium", allowed_pairs)
        self.assertIn("gpt-6-luna|low", allowed_pairs)
        self.assertNotIn("gpt-6-luna|ultra", allowed_pairs)
        self.assertNotIn("retired-model|high", allowed_pairs)

    def test_runtime_receipt_split_fields_build_exact_pairs(self):
        receipt = {"requested_model": "gpt-6-sol", "requested_effort": "high", "resolved_model": "gpt-6-sol", "resolved_effort": "high", "effective_model": "gpt-6-sol", "effective_effort": "medium"}
        identity = disclosure_module.resolve_disclosure_identity(runtime_receipt=receipt)
        self.assertEqual(identity["requested_pair"], "gpt-6-sol|high")
        self.assertEqual(identity["resolved_pair"], "gpt-6-sol|high")
        self.assertEqual(identity["effective_pair"], "gpt-6-sol|medium")
