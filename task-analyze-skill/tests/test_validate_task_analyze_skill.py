...
class ValidateTaskAnalyzeSkillTests(unittest.TestCase):
    ...
    def test_inline_bootstrap_requires_universal_post_result_ending_and_no_pre_result_verify(self):
        ...
        try:
            ...
            self.assertIn("score checks", bootstrap_text)
            self.assertIn("Every result,including read-only,triggers a visible End Task", bootstrap_text)
            self.assertIn("exact saved project", bootstrap_text)
            self.assertIn("ack threadId+hostId+projectId", bootstrap_text)
            self.assertIn("End/Fix stay visible", bootstrap_text)
            ...
            self.assertIn("route/calibration/Ending separate", bootstrap_text)
            self.assertIn("no reread/full-read/precheck", bootstrap_text)
            self.assertNotIn("Mini Verify", bootstrap_text)
            self.assertNotIn("projectless End/Fix", bootstrap_text)
            self.assertNotIn("records then self-archives", bootstrap_text)
