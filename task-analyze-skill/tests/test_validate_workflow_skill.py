...
class ValidateWorkflowSkillTests(unittest.TestCase):
    ...
    def test_global_entry_required_terms_match_compact_contract(self):
        ...
        self.assertIn("no hierarchy/local JSON", required)
        ...
        self.assertIn("no route/plan/guesses/reread/full read/pre-result check", required)
        self.assertIn("Ending <=60s evidence-only", required)
        self.assertIn("no extra tests/API/questions/wait", required)
        self.assertIn("concurrent state change=>BLOCKED+exit", required)
        self.assertIn("never gates main result", required)
