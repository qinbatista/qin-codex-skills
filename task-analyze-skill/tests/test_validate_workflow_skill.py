...
class ValidateWorkflowSkillTests(unittest.TestCase):
    ...
    def test_global_entry_required_terms_match_compact_contract(self):
        ...
        self.assertIn("same-session gate→Spark-low if no stronger route", required)
        self.assertIn("dynamic_task_graph", required)
        self.assertIn("task_route_dispatcher.py run-plan", required)
        self.assertIn("Direct `gpt-5.6-sol|ultra`", required)
        self.assertIn("Auto `gpt-5.6-luna|max`", required)
        ...
        self.assertIn("Only the final aggregate `ending-required` or `ending_required=true` receipt after all result nodes settle=>one independent global projectless Ending", required)
        self.assertIn("material=>durable project-memory closeout", required)
        self.assertIn("child/subprocess receipts never launch it", required)
        self.assertIn("same End Task continues one bounded action at a time", required)
        self.assertIn("`intentionally_skipped_simple_task`=>no End Task", required)
        self.assertIn("terminal memory/classification/record closeout", required)
        self.assertIn("call codex_app__create_thread target", required)
        self.assertIn("codex_app__list_threads", required)
        self.assertIn("projectId=null/absent", required)
        ...
        self.assertIn("fresh global projectless End Task with --repair-of-lifecycle-id", required)
        self.assertIn("fresh independent global projectless Repair Task", required)
        self.assertIn("never send/steer/interrupt/terminate/handoff/move/mutate any existing task/session", required)
        self.assertIn("up to 3 repairs", required)
        self.assertIn("BLOCKED only unavailable/external/limit,not active-task waiting", required)
    ...
    def test_routing_matrix_separates_single_producer_from_cost_admitted_graphs(self):
        ...
        self.assertEqual(routes["dependent-multi-file"], ["one adaptive producer or linear scored nodes", "owning skill", "result"])
        self.assertEqual(routes["dynamic-code-graph"][:2], ["Task Analyze", "scored dependency-ready nodes"])
    ...
    def test_executor_skills_support_inline_and_admitted_modes(self):
        ...
        self.assertIn("A low-risk, low-ambiguity code, write, probe, command, transform, or execute segment scoring `0-24` runs the same-session outcome gate, then tries the catalog priority producer only with no stronger session route", code_text)
        self.assertIn("other eligible implementations execute their score role or Obsidian-context quality pair", code_text)
