...
After the result and in-task verification complete, show a separate visible projectless Ending task for durable memory updates under the user-authorized Ending lifecycle. Use the selected model and effort; acknowledge its actual task link and memory readback before claiming completion. Ending is memory-only: summarize durable changes and preferences; never silently replace the requested task with an inline write. Read and update only related memory for the identified project; keep global preferences explicit. No useful new memory or missing memory means an explicit Ending skip. A missing task-creation capability means Ending is pending, not completed. Never mix projects, create empty memory scaffolding during a task, or run verification/repair task chains in Ending.

Keep reusable instructions concise. Resolve CODEX_HOME and use native portable paths. Keep task scratch in project Cache/tmp-*; preserve unrelated work. Publish or install only when authorized, with recoverable replacement and preservation of user AGENTS files.
