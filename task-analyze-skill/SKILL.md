...
4. Immediately after presentation, write a lifecycle start receipt with `verify-skill/scripts/ending_task_ledger.py`; bind `--producer-receipt` whenever the adaptive runner produced the result. Call `create_thread`, then `set_thread_title` with exactly `End Task-{concise related task name}`. Pass the lifecycle/producer receipts, Quick Check evidence, acceptance target, project root, touched files, and allowed repair scope. Link it and return without waiting or polling; never substitute a same-task subtask. The Ending thread automatically writes that producer's Real verdict to Obsidian before terminal PASS/BLOCKED.
...
6. The origin final requires a created End Task thread or explicit `BLOCKED`; it never waits for the verdict. The End Task thread final requires lifecycle `PASS` or explicit `BLOCKED`. A failure is logged before repair; repair is a new child lifecycle, its corrected result is presented, and a different Ending verifier runs again.
...
For a dispatched graph, enforce `run-plan` -> completed result -> bounded Quick Check when code -> show result -> `release-main-result` -> create and rename `End Task-{concise related task name}`. Pass the dispatcher handoff to that thread; only the new thread may call `run-ending`. For ordinary work, enforce Quick Check when code -> result presentation -> lifecycle `start --producer-receipt <receipt>` when routed -> create/rename/link End Task -> return without waiting. Ending owns Real Verify, broader regression, optimization proof, reports, logs, related memory, and the only receipt-backed pass/fail write to Obsidian after the result. A correctness failure first records lifecycle error evidence and any failed durable project state, then launches a repair child inside the End Task thread. Present the corrected result and launch a different Ending verifier; never let a repair self-certify.
...
For strategy comparisons, keep prompt, inputs, cwd, sandbox, configuration, output contract, acceptance, and Quick Check policy identical. Count each foreground session once. User-visible performance runs from task start through required Quick Check and completed-result presentation; receipt finalization, telemetry, and detached Ending Real are excluded, as are all Ending/verification tokens. Every tier requires matching correctness and fewer foreground tokens; Simple time uses the Direct cohort's measured noise envelope, Medium keeps the strict speed direction and majority gate, and Complex time is diagnostic.
