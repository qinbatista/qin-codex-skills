...
This is the full routing and model-strategy skill. The hookless bootstrap in `~/.codex/AGENTS.md` combines the saved highest-version quality ladder with matching project-scoped broad Obsidian `Model Switch.md` records; explicit multi-node routing still requires activation. The catalog fast producer is schedule-only for disjoint independent sources, never the ordinary Auto first attempt. The mandatory post-result Ending lifecycle always applies.
