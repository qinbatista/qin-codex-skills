...
1. Score and classify eligible ordinary work once from its project/task/module/file/symbol/code context without reading task-file content. Every eligible text/code production task must run `scripts/obsidian_adaptive_model_runner.py` through the Entry Parent Fast Path. A small 0-24, low-risk, low-ambiguity text/code edit attempts Spark-low first. Its contextual saved quality pair is the zero-result operational fallback. A receipt-backed Ending quality/correctness FAIL suppresses Spark for the matching project, task type, operation, code kind, and score band; the next matching task upgrades to the contextual quality pair. Other eligible work executes the saved contextual quality pair, using exactly one stronger quality pair only after a zero-result operational failure. Image/mixed and tool-only work stays inline after score-only classification. Two-or-three independent-source candidates use byte-metadata cost admission first; small candidates remain one producer and only admitted large/latency candidates schedule. Ordinary work reads the saved ladder unchanged. A missing ladder may bootstrap once from the local Codex cache without network access; only an explicit user model-update request may rescan that cache and replace a valid ladder. If the cache is unavailable, keep the last valid ladder. Unavailable or unconfigured Obsidian disables learning but uses the shared saved cold-start recommendation; it never blocks execution.
