...
The fast runner has one receipt-backed scheduled shape: a read-only request that explicitly lists two or three independent source files. It runs disjoint source nodes on the current catalog priority producer at low effort, then one adaptive dependency-results-only merge, and publishes one aggregate receipt containing every stage pair, token count, and critical-path time. An exact-expression contract or dependency-coupled multi-file edit remains one producer; exact/dependent manifests use one Sol-high producer rather than unsafe fan-out. This distinction is mandatory: never label one Terra/Spark child as an Auto Schedule, and never split shared-state code merely to manufacture a larger benchmark win.
...
1. Classify eligible ordinary work once from its project/task/module/file/symbol/code context without reading task files. Every eligible text/code production task must run `scripts/obsidian_adaptive_model_runner.py` through the Entry Parent Fast Path: it tries the saved catalog-selected priority producer (`low` for easy, `high` for complex when supported), then the selected quality pair only after a zero-result operational failure. Image/mixed and tool-only work stay inline. Exact read-only work stays inline except the explicit two-or-three independent-source scheduled shape above. Ordinary work reads the saved ladder unchanged. A missing ladder may bootstrap once from the local Codex cache without network access; only an explicit user model-update request may rescan that cache and replace a valid ladder. If the cache is unavailable, keep the last valid ladder. Unavailable or unconfigured Obsidian disables learning but uses the shared saved cold-start recommendation; it never blocks execution.
...
Missing or negative performance evidence forbids a savings claim or an open-ended model-generated graph, but it does not disable contextual quality selection or the bounded independent-source schedule. That built-in schedule has fixed safety gates: read-only wording, two or three existing project-relative files, disjoint source allowlists, maximum three parallel branches, and a dependency-results-only merge. Its aggregate receipt must expose every stage; a current simple/medium/complex benchmark is still required before publishing a savings claim. One foreground quality pair may be the current cold start, one-rung downgrade, one-rung upgrade, or frozen reuse; Ending Real supplies its verdict. A suite aggregate cannot hide a losing class. None of these conditions may suppress the mandatory Ending lifecycle.
...
- The same runner owns the bounded independent-source schedule. It uses `task_route_dispatcher.py` once, launches disjoint source nodes in parallel, starts the adaptive merge only from completed dependency results, and emits one aggregate scheduled receipt. Missing Obsidian changes the selection basis to `shared_cold_start`; it does not abort the schedule.
...
For strategy comparisons, keep prompt, inputs, cwd, sandbox, configuration, output contract, acceptance, and Quick Check policy identical. Every benchmark entry starts from the user-selected `gpt-5.6-sol|ultra` pair. Direct remains on that fixed entry pair; Global starts on the same entry pair and must produce a receipt proving the actual adaptive producer or scheduled graph. A fixed same-pair Global arm, a one-child result mislabeled as scheduled, or a Global arm that falls back to Sol inline is not evidence of Auto Best Model savings. Count each foreground session once. User-visible performance runs from task start through required Quick Check and completed-result presentation; receipt finalization, telemetry, and detached Ending Real are excluded, as are all Ending/verification tokens. Every tier requires matching correctness and fewer foreground tokens; Simple time uses the Direct cohort's measured noise envelope, Medium keeps the strict speed direction and majority gate, and Complex time is diagnostic. A scheduled Complex cohort must expose at least two result nodes, every requested/effective stage pair, parallel-branch count, and merge dependency proof. Show separately whether the optional background Ending diagnostic would preserve or erase the foreground token/time win; never use it to decide the foreground winner.
...
5. Prove the independent-source fixture runs multiple disjoint result nodes plus one dependency-results-only merge, while a dependency-coupled multi-file fixture stays one producer.
