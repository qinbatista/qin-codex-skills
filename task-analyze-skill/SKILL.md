...
description: "Use for explicit model routing, dynamic task segmentation, dependency graphs, Task Analyze maintenance, or benchmarking. Single-node ordinary production uses the compact adaptive runner from the always-loaded bootstrap."
---

# Task Analyze Skill

This is the full routing and model-strategy skill. The hookless bootstrap in `<codex-home>/AGENTS.md` assigns every task a visible 0-100 complexity score, resolves the observable entry pair, then calls `obsidian_model_memory.py recommend` through native Obsidian wikilinks: project index → Model Switch → category → shared category → exact-fingerprint project categories. It uses no JSON sidecar and `--compact` returns the bounded route capsule. It combines the saved highest-version quality ladder with exact-project Obsidian history and strongly matched transferable local history. It does not impose one starting model. Each quantifiable step receives a sanitized capability fingerprint from its step kind, controlled tags, operation, modality, risk, ambiguity, and difficulty band. Highly similar receipt-backed history wins over the entry model: a Sol-ultra/high entry may route down, a Luna-max/lower entry may route up, and a recovered or frozen lowest-correct boundary is reused from either direction even after a project-root move. Any request containing two or more materially distinct result stages automatically activates multi-node routing; it does not require the user to say "route" or "graph". Scores map to `small` 0-24, `standard` 25-49, `complex` 50-74, and `advanced` 75-100. The catalog fast producer is the first attempt for eligible small low-risk edits and remains available for disjoint independent source branches. The mandatory post-result Ending lifecycle always applies and persists score plus switch movement.
...
For eligible text/code, the current entry parent routes before loading an owning skill, project memory, task files, or implementation references. Resolve `CODEX_HOME` from the environment (falling back to the platform user Codex directory), join `skills/task-analyze-skill/scripts/obsidian_adaptive_model_runner.py` with native paths, and pipe the exact task to that script once in a non-TTY platform-Python command. Its zero-argument stdin fast path derives private receipt/result paths, resolves and records the current entry pair, calculates the 0-100 score and stable band without reading task files, derives the compatibility easy/complex effort class, and binds the child to the canonical current workdir; explicit `--entry-model` plus `--entry-effort` overrides entry discovery, and an explicit `complexity score: N` or `--complexity-score N` overrides inference. Exact read-only, tool-only, and image/mixed work uses `scripts/task_complexity_score.py` so every submission still reports and logs a score without fabricating a producer receipt. Do not start an interactive runner and send EOF later. Give the first command its full bounded yield; if it returns an ongoing session, use one empty `write_stdin` poll with up to 60 seconds. A receipt-authorized `LOCKED_ROUTE_NODE` or `ENDING_TASK_WORKER` is already below this boundary: it executes its assigned node directly and never re-enters the adaptive runner. The selected producer owns skill loading, memory recall, file reads/edits, Quick Check, and its result receipt. The entry parent never duplicates implementation, tests, file reads, or verification, and never performs repeated 30-second model-turn polling. Publish the saved child result and return.

The fast runner owns a genuinely single-node task. When the user request contains distinct bounded result segments, full Task Analyze builds one `dynamic_task_graph` instead: every node has its own `complexity_score`, band, exact model/effort, purpose, dependencies, stop condition, `step_kind`, and controlled capability tags. For example, “implement code, then run local tests” creates separately learned implementation and local-test steps, so either may reuse a cheaper proven model without forcing the other to do so. The plan proves its `max_safe` decomposition with exactly one inventory item per result node; an item that retains multiple logical stages stays linear and names its continuity reason. The parent score never chooses one model for every node; neither does the entry model. Dependency-ready nodes with disjoint reads/writes may run concurrently; shared-state, ordered, irreversible, or output-dependent nodes remain linear. Two or three explicit independent read-only sources still use the byte-cost admission shortcut. Never manufacture nodes by splitting one dependency-coupled implementation into arbitrary file slices.
...
1. Score the submission once, resolve the entry pair, then decide whether the work is one bounded producer or a real task graph before reading task files. A single eligible text/code result runs `scripts/obsidian_adaptive_model_runner.py`. A real graph scores every node independently and queries history for each node's own capability fingerprint and difficulty. Use explicit stable tags for distinctive work such as `image-generation` + `tool-control`, `local-test`, `browser-automation`, or `visual-verification`; otherwise derive the bounded vocabulary without persisting raw task text. Matching receipt-backed history chooses the proven boundary regardless of entry. A prior low-model success is reused even from Sol Ultra; a prior low-model failure followed by stronger success is reused directly even from Luna Max. With no exact match, choose the weaker of the contextual cold-start suggestion and the entry anchor, then upgrade one rung after quality failure. Any low-risk, low-ambiguity text/code/write/execute node scoring 0-24 attempts Spark-low even when the parent task is complex or advanced; a non-Spark node supplies a concrete exception category and reason. An independently assigned pair is planned work, not a fallback or switch: before a receipt it remains concrete with `task_assignment`, `UNVERIFIED (no runtime receipt)`, and `no_switch`. The contextual quality pair is the only zero-result operational fallback. Image/mixed and tool-only nodes stay on their owning execution surface. Ordinary work reads the saved ladder unchanged. A missing ladder may bootstrap once from the local Codex cache without network access; only an explicit user model-update request may rescan that cache and replace a valid ladder. If the cache is unavailable, keep the last valid ladder. Unavailable or unconfigured Obsidian disables projection but continues from local history and then the entry-aware shared cold start; it never blocks execution.
...
6. Every origin result and result/Ending node uses the Result Model Disclosure block below. For two or more model-executed result/Ending stages, the final also includes the generated `Model stages (N):` block with every stage's score/band/pair/purpose/dependencies/status; never show only the last management/publish pair. The origin final is complete after the result presentation and never waits for the audit. The End Task thread final requires lifecycle `PASS` or explicit `BLOCKED`; concurrent or missing evidence is terminal BLOCKED, not a user question. A repair is never automatic: it requires a new user request after a concrete recorded failure.
...
Ordinary single-node work must not read this full `SKILL.md`, show a pre-result route, or deliberate broadly. Explicit routing, Task Analyze maintenance, or a request with multiple materially distinct result segments activates this skill and one dispatcher graph. Code producers apply only the bounded Quick Check before presentation. After presentation, the producer receipt or aggregate graph receipt is bound to the lifecycle ledger and separate persistent End Task threads are required for independent real checks.

Eligible ordinary requests use one project-scoped Obsidian context lookup plus bounded strongly matched local transfer history with no visible plan; exact read-only and ineligible work remain inline. After the final source read, emit the requested output immediately with no separate foreground self-review, Mini, or verification pass.

## Task-Graph Admission And Performance Claims

There are two distinct decisions. Task-graph admission is structural: at least two bounded result segments have different owners, scopes, model needs, or dependency-ready concurrency, and splitting preserves one clear final-result owner. It does not require benchmark history. A savings or speed claim is a separate performance decision and requires comparable steady-state selected-execution evidence after the route is stable. Actual end-to-end wait remains a separate diagnostic. Mandatory post-result Ending tasks are outside both decisions.

Any savings claim requires all of the following:
...
- the performance comparison contains exactly two worlds: fixed `gpt-5.6-sol|ultra` Direct task completion with no Skill, and an Auto task entered through the frozen hash-only `AUTO_BENCHMARK_ENTRY` wrapper on `gpt-5.6-luna|max`; an atomic claim permits exactly one adaptive bridge workspace, and a controller with an empty final may publish only that bridge's strict-JSON result after its passing child receipt and output hash agree; a non-empty wrong controller result still fails; the Luna entry controller is an excluded routing diagnostic, never a third task layer;
- the Auto world contains at least one receipt/runtime-proven adaptive child or graph, and the published route is the receipt/runtime-proven adaptive child or graph rather than the entry model;
- each Auto task also runs one strict post-result `gpt-5.6-sol|ultra` entry-invariance probe outside both performance worlds; Luna and Sol entry must produce the exact expected result and the same stable route/capability assignment, while the probe's time and tokens remain diagnostic only;
- every task + Ending check passes. Primary savings compare only clean stable selected execution after history has chosen the correct route: Direct counts its complete fixed-strategy task census, while Auto counts the selected adaptive child or graph. Matching/controller time, calibration failures, retries, fallbacks, repairs, entry probes, and Ending are excluded from primary time/token savings and reported separately. Overall optimization passes only when aggregate steady execution time and logical tokens are both lower; first-result wait and per-tier regressions remain visible diagnostics and can never be relabelled as savings;
- evidence is complete, current, and workload-comparable.

The admitted cohort must prove lower aggregate steady selected-execution logical tokens and time under those governing rules. Do not turn one chosen percentage into the optimization target: keep removing reproducible deterministic waste until repeated comparisons show only runtime noise or no correctness-preserving change remains. Per-tier medians, wins, regressions, actual first-result wait, matching/controller overhead, calibration cost, entry probes, and total wall time remain visible diagnostics. Ending Real time and tokens never delay or charge the primary steady metric.

`scripts/strategy_performance.py` is the private authority for this separate decision. Its key includes the quality-profile fingerprint, current entry `model|effort`, configuration cohort, sandbox, strategy version, producer-contract version, and exact workload hash. Delegation needs at least six comparable paired samples. Every arm must have passing correctness and complete metrics. Primary steady selected-execution logical tokens and execution time each require a lower Global cohort total and raw median with non-negative paired-median savings; steady execution also requires a strict majority of faster pairs. Matching/controller time, calibration failures, retries/fallbacks/repairs, actual first-result wait, probes, and Total-wall/Ending latency remain separately visible diagnostics. Otherwise it returns `inline_entry`.

Missing or negative performance evidence forbids a savings claim, but it does not forbid a valid bounded task graph. A dynamic graph must expose every node score, band, requested/effective pair, dependencies, tokens, elapsed time, and critical path. Read-only source fan-out retains its disjoint allowlists and dependency-only-or-fused merge constraints. Writable graphs instead isolate ownership by file/symbol/output path and keep shared writes ordered. The aggregate steady metric governs the optimization verdict while every losing tier remains visibly reported and cannot be relabelled as a tier saving. None of these conditions may suppress the mandatory Ending lifecycle.
...
- **Dynamic graph:** a task-specific Mermaid plus a numbered `Workflow with models` list. Show every node score, exact pair, real dependencies, safe parallel waves, Main Goal Done Gate, immediate result release, and post-result Ending Task Real Verify.
...
- Resolve the entry pair before choosing a quality rung. Matching history is authoritative; absent history, never start above the entry anchor, so high entries can descend contextually and low entries can ascend only after verified quality failure.
- Use weaker catalog models for bounded work when the entry anchor, contextual cold-start policy, or verified descent permits them.
...
- No prior quality success outside that bounded priority: eligible production runs the weaker of its catalog cold-start pair and observable entry anchor; this bootstrap sample is what allows later descent, ascent, or freezing. A Sol-ultra/high entry therefore preserves the prior downgrade behavior, while a Luna-max/lower entry starts at its anchor and upgrades only when quality evidence requires it. Ineligible/tool-only/exact-read-only work remains inline after score-only classification.
...
- Like-for-like token/time evidence is diagnostic only. Once Real evidence proves more than one pair correct for the same step, select the lowest passing rung above the strongest failed rung; never choose a stronger passing pair merely because it used fewer tokens or less time. Operational failure is quality-neutral and may use one stronger fallback only before any result with zero tokens.
...
Ending Real alone records receipt-backed producer PASS/FAIL, outcome reason, complexity score/band, attempt/effective/next/recovery pair, and switch direction. It writes the local routing event before the canonical Obsidian category projection. Producers and ordinary runners never write learning. Operational failures remain quality-neutral and inline execution never fabricates a receipt.

Each receipt-backed Ending terminal record is rendered once in its stable category page. The six categories are `normal-script-update`, `code-design`, `finding-bugs`, `tests-verification`, `documentation-instructions`, and `general-work`; category pages are native wikilink nodes, while project/task/module/file/symbol remain record fields and never become per-context pages. Switch direction remains one of `initial`, `upgrade`, `downgrade`, `freeze`, `no_switch`, or `operational_fallback`.
...
For strategy comparisons, keep prompt, inputs, cwd, sandbox, frozen catalogs/memory, output contract, acceptance, and Quick Check policy identical. Direct always enters and remains on `gpt-5.6-sol|ultra` with no Skill. Auto enters on `gpt-5.6-luna|max`, must expose at least one receipt/runtime-proven stable adaptive child or graph selected per task step, and may move upward when matching history or quality failure requires it. Primary metrics count the Direct producer versus the frozen Auto selected producer or graph after route selection. Entry matching/controller work, calibration failures, retries, fallbacks, repairs, Sol-entry probes, and Ending are excluded from primary time/tokens and reported separately beside actual first-result and total-wall diagnostics. Never draw a third controller arm or treat the Sol-entry direction probe as another benchmark arm. A fixed inline Auto arm, a missing adaptive child/graph, or an Auto arm that silently substitutes Sol is not evidence of savings. Every task requires exact matching correctness, complete metrics, and detached Ending PASS before any cohort savings claim. Per-tier medians, wins, and regressions stay visible diagnostics even when aggregate totals pass. A scheduled Complex cohort must expose at least two result nodes, every requested/effective stage pair, parallel-branch count, and merge dependency proof.
...
`scripts/benchmark_suite_gate.py`—not a caller-authored manifest—derives correctness, completion, receipt/session coverage, steady execution/tokens, route-selection and calibration diagnostics, first-result time, total-wall time, retry/fallback/repair counts, and per-tier diagnostics. Overall PASS requires every task and Ending correctness/evidence gate to pass plus lower aggregate Auto steady execution time and logical tokens. A per-tier performance regression stays visible but does not override the declared aggregate cohort rule; any task or Ending correctness/evidence failure still fails the suite.

## Generated Files

### Project Cache Artifact Policy

Before the first Codex-selected project-support write, resolve the authoritative `<project-root>`, inspect `<project-root>/Cache/`, and choose the destination there. All agent-created disposable or supporting artifacts — plans, prompts, receipts, logs, benchmarks, test scripts/results/fixtures, debug data, intermediate code, image inspection downloads/renders, generated images, snapshots, comparisons, and probes — must live only under `<project-root>/Cache/`; redirect any proposed path there before writing. Requested durable project source changes and final deliverables remain in the project's declared source/output paths.
...
When Cache content is reusable, retained, workflow-required, or project-influencing, add one concise registry entry to project-root `AGENTS.md`: the exact Cache-relative path, one-line structural role, owner/source of truth, and retention/version-control status. Link to the owning source or detailed README instead of embedding its commands, dependencies, runbook, or regeneration procedure. Update `AGENTS.md` only when project structure, ownership, a critical entry point, or a hard constraint changes. Important Cache without this concise pointer is incomplete; one-off disposable outputs need no entry. Never delete documented important Cache content without explicit authorization; other cleanup may delete only the current task's named Cache folder or explicitly identified disposable files. Private contextual model evidence belongs in the local routing ledger and its native Obsidian category projection only.
...
4. Prove a single-node code task uses one producer, while a larger parent can route bounded small code/write/execute nodes to Spark based on each node score rather than the parent score.
5. Prove linear, parallel, and mixed dynamic fixtures execute only dependency-ready nodes, preserve shared-state ordering, and record every node score/pair; also retain the independent-source dependency-only and fused-final constraints.
