...
4. Immediately after presentation, write a lifecycle start receipt with `verify-skill/scripts/ending_task_ledger.py`; bind `--producer-receipt` whenever the adaptive runner produced the result, then launch an independent Ending subagent for every task level. The ledger automatically writes that producer's Real verdict to Obsidian before terminal PASS. This post-result worker is mandatory and bypasses result-producer performance admission.
...
Ordinary work must not read this full `SKILL.md`, show a pre-result route, or deliberate broadly. For every eligible production task the compact bootstrap invokes exactly one receipt-backed priority-first route through `scripts/obsidian_adaptive_model_runner.py`; only a zero-result operational priority-producer failure may start its selected quality fallback. It never launches `verify-skill` before presentation. After presentation, the producer receipt is bound to the lifecycle ledger and the independent Ending subagent is required regardless of complexity.
...
For a dispatched graph, enforce `run-plan` -> completed result -> show result -> `release-main-result` -> `run-ending`. For ordinary work, enforce result presentation -> lifecycle `start --producer-receipt <receipt>` when routed -> independent Ending subagent. Ending owns Real Verify, broader regression, optimization proof, reports, logs, related memory, and the only receipt-backed pass/fail write to Obsidian after the result; the ledger performs that write automatically for a bound producer receipt. Safe isolated branches run concurrently. A correctness failure first records lifecycle error evidence and any failed durable project state, then launches a repair child with a new lifecycle. Present the corrected result and launch a different Ending verifier; never let a repair self-certify.
