...
1. Score the submission once, then decide whether the work is one bounded producer or a real task graph before reading task files. A single eligible text/code result runs `scripts/obsidian_adaptive_model_runner.py`. A real graph scores every node independently. Any low-risk, low-ambiguity text/code/write/execute node scoring 0-24 attempts Spark-low even when the parent task is complex or advanced; a non-Spark node supplies a concrete exception category and reason. An independently assigned pair is planned work, not a fallback or switch: before a receipt it remains concrete with `task_assignment`, `UNVERIFIED (no runtime receipt)`, and `no_switch`. The contextual quality pair is the only zero-result operational fallback. Other result nodes use their own score role or Obsidian-context quality recommendation. Image/mixed and tool-only nodes stay on their owning execution surface. Ordinary work reads the saved ladder unchanged. A missing ladder may bootstrap once from the local Codex cache without network access; only an explicit user model-update request may rescan that cache and replace a valid ladder. If the cache is unavailable, keep the last valid ladder. Unavailable or unconfigured Obsidian disables learning but uses the shared saved cold-start recommendation; it never blocks execution.
...
- the public comparison contains exactly two worlds: fixed Direct task completion with no detached verifier, and Auto task completion plus its detached Ending check; the common Sol-ultra entry/controller is disclosed as an excluded routing diagnostic, never charged as a third task layer;
...
When a Cache directory or file is important — reusable, manually maintained, required by a project command/workflow, influential to project logic or behavior, or retained across tasks — update the project-root `AGENTS.md` in the same change. Emphasize the exact Cache-relative path, purpose, owner/source of truth, how to run/use/regenerate it, dependencies, retention/cleanup boundary, and whether it is source-controlled. Test scripts and intermediate code that influence the project are included. Work is incomplete if important Cache content is undocumented. One-off disposable outputs need no `AGENTS.md` entry. Never delete documented important Cache content without explicit authorization; other cleanup may delete only the current task's named Cache folder or explicitly identified disposable files, never an existing Cache category or another task's artifacts. Private contextual model evidence belongs in the local routing ledger and its broad Obsidian projection only. The generated shared ladder contains no personal evidence and is safe to mirror.
