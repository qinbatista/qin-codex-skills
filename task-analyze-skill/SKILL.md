...
- the public comparison contains exactly two worlds: fixed `gpt-5.6-sol|ultra` Direct task completion with no detached verifier, and an Auto task entered through the frozen hash-only `AUTO_BENCHMARK_ENTRY` wrapper on `gpt-5.6-luna|max` plus its separately owned Ending check; an atomic claim permits exactly one adaptive bridge workspace, and a controller with an empty final may publish only that bridge's strict-JSON result after its passing child receipt and output hash agree; a non-empty wrong controller result still fails; only the Auto Luna-max entry controller is disclosed as an excluded routing diagnostic, never charged as a third task layer;
...
Missing or negative performance evidence forbids a savings claim, but it does not forbid a valid bounded task graph. A dynamic graph must expose every node score, band, requested/effective pair, dependencies, tokens, elapsed time, and critical path. Read-only source fan-out retains its disjoint allowlists and dependency-only-or-fused merge constraints. Writable graphs instead isolate ownership by file/symbol/output path and keep shared writes ordered. A suite aggregate cannot hide a losing class. None of these conditions may suppress the mandatory Ending lifecycle.
...
For strategy comparisons, keep prompt, inputs, cwd, sandbox, frozen catalogs/memory, output contract, acceptance, and Quick Check policy identical. Direct always enters and remains on `gpt-5.6-sol|ultra` with no detached verifier. Auto enters on `gpt-5.6-luna|max`, must expose at least one receipt/runtime-proven adaptive child or graph selected per task step, and may move upward when matching history or quality failure requires it. Auto task tokens exclude only its Luna-max entry controller; Direct task tokens include its Sol-ultra producer. Report exactly (1) Direct task versus Auto task and (2) Direct task versus Auto task + Ending check. Never draw a third controller bar or treat the Sol-entry direction regression as another benchmark arm. A fixed inline Auto arm, a missing adaptive child/graph, or an Auto arm that silently substitutes Sol is not evidence of savings. Every tier requires exact matching correctness, complete metrics, and all detached Endings PASS before any savings claim; show whether Ending preserves or erases the task token/time win without hiding its added cost. Simple time uses the Direct cohort's measured noise envelope, Medium keeps the strict speed direction and majority gate, and Complex time is diagnostic. A scheduled Complex cohort must expose at least two result nodes, every requested/effective stage pair, parallel-branch count, and merge dependency proof.
