...
For code, run one bounded producer Quick Check before presentation. Do not run Mini/Fast Verify before first presentation; show the completed result immediately after Quick Check. End Task is hard-required after every result, including exact read-only work. Each independent check gets its own persistent `End Task-{task}-{check}`, complexity score, and quality-ladder model/effort in the exact current saved project. Resolve its project ID by exact canonical root, build host launch requests with `ending_verification_plan.py create-launches --project-id <resolved-id>`, invoke every listed project/local `codex_app__create_thread`, acknowledge each returned `threadId`, `hostId`, and project ID with `ack-launch`, and require `audit-launches` to report `end_task_trigger_rate=100%`. Worker prompts use project-root-relative paths. A handoff file without those acknowledgements is pending, not triggered. The task executes its smallest real observable check; all required checks must PASS. PASS/FAIL/BLOCKED tasks remain visible. Every terminal Ending reports task/check difficulty, producer/verifier pairs, attempts, first/retry pass, suitability, next pair/action, and Obsidian record status/link. FAIL creates a separate repair task in the same project with exact evidence and then a fresh Ending check. Missing task creation or exhausted repair attempts is BLOCKED, never verified; no End/Fix task auto-archives or deletes itself, and every terminal verdict is recorded automatically.
