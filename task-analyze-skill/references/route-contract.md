...
After the final result and its real verification, the parent may create one separate visible projectless Ending task for useful Obsidian memory updates. Ending is not a graph node or a result dependency. It cannot change this task's acceptance, launch repairs, or influence another active task. Leave it in recent tasks without pinning. Verification belongs to the main task.
