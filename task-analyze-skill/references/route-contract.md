...
For code, emit a user-visible `code-rule-notice` before source work, then load every announced owning/code-philosophy/language/platform/domain Skill. The Code Gate always includes the universal philosophy, exactly one active language profile when registered, and only materially matched categories; a missing notice or universal reference is a routing failure. New C# aliases use the single Unity C# profile. Write the code first, run exactly one smallest safe local Quick Check, and show `CODE READY` immediately. Do not run broad tests, builds, UI/visual checks, full lint, log cleanup, release gates, or repeated source review before presentation. A released result emits `ending-required` for an observable or material trigger; material receipts require durable memory closeout, while only explicit trivial value-only work with no other surface may skip. The parent creates and acknowledges one persistent global projectless `End Task-{task}` only from the final aggregate receipt after all result nodes/subprocesses settle and release; it passes `ending_verification_plan.py create-launches --producer-receipt <final-receipt>` and returns without polling. A child receipt never starts an Ending. Invoke only exact `codex_app__create_thread` arguments with target `{"type":"projectless"}` and no project/current-task fields, then read the exact thread back through `codex_app__list_threads`. `ack-launch` requires the origin project binding plus observed `threadProjectId=null`; `audit-launches` requires `end_task_trigger_rate=100%`. Project-attached, current-task, same-task-subtask, unread-back, or missing acknowledgement is BLOCKED. Spark-xhigh is first. On a real quota/five-hour/provider-rate/trusted retry limit, run `record-controller-restriction` once; `create-launches` then skips the cooling model and selects the next stronger controller until cooldown expiry. Never retry a cooling model or treat quality failure as quota. Deterministic checks run directly. Semantic runtime/integration/code-quality/prompt/UI/visual checks may use plan-saved Terra/Sol `ENDING_CHECK_WORKER` nodes; workers read listed Skills, run one check, write evidence, and never edit, repair, route, or close lifecycle. All evidence must PASS. FAIL creates a fresh independent global projectless Repair Task through `codex_app__create_thread` with no existing thread target; it never sends, steers, interrupts, terminates, hands off, moves, or mutates another session. Active-task write ownership makes Repair wait without messaging before scoped repair, Quick Check, and a fresh parent-linked Ending with `--repair-of-lifecycle-id`, for at most three attempts. Every terminal verdict is recorded automatically and every task remains visible.
...
- Requested text/code production uses the smallest adaptive producer surface. Code runs one proportional Quick Check before presentation. One detached Ending Real runs the bounded explicit tests and material memory closeout needed for the changed result; safe independent checks may run concurrently inside it, and failures create an isolated Repair Task followed by fresh restriction-aware verification.
...
For an admitted route, the entry model may coordinate the route but must not duplicate a child's source inspection. Every collaboration child starts with `LOCKED_ROUTE_NODE`, produces only its assigned result and Quick Check, and never starts End/Fix work. After its passing receipt exists, the entry parent binds it to Ending. The whole graph uses collaboration children or the dispatcher, never both.
...
## Collaboration Child-Session Topology

Every `complex`/`advanced` task and every existing `planned_graph` explicitly evaluates whether collaboration child sessions improve the topology. Admit them only when at least two branches are dependency-ready, ownership boundaries are complete, shared-checkout write allowlists are pairwise disjoint or each branch owns an independent copy/worktree, and parallel execution is expected to reduce waiting. Otherwise retain the current single-producer or dynamic-dispatcher route and record one bounded rejection reason. Collaboration unavailability is a fallback condition, never permission to invent a session.

Each admitted branch starts with `LOCKED_ROUTE_NODE`; the whole graph is assigned to exactly one surface, collaboration or dispatcher. The child contract contains parent ownership, runtime project/cwd, relative workdir, read and write allowlists, dependencies, inputs, outputs, `required`, Cache classification and retention, and stop condition. Durable plans, receipts, summaries, and memory use a project key and relative paths; they never store raw prompts/results/reasoning, absolute paths, secrets, email addresses, or session/thread/agent IDs. Same-checkout code may use the common project-root workdir and is parallel only across pairwise-disjoint writes; per-platform packaging may use isolated per-platform copies/worktrees; shared files/state, output dependencies, and irreversible effects are ordered dependencies.

The main session alone owns collaboration control: spawn, list/wait monitoring, send/followup updates or redirection, and targeted interrupt. Children cannot control siblings, launch Ending/Repair, or claim main completion. Root completion requires successful terminal state for every required child, output and allowlist-boundary readback, satisfied dependencies, no conflicts, and main-session aggregation, acceptance, and Quick Check. Child `CODE READY` is not root completion. Cancelling the main task targets all active children with interrupt and reports incomplete truthfully. Only the released final aggregate receipt after every child settles can launch one global projectless Ending.

Isolated copies, branch logs, and intermediates use one main-owned `Cache/tmp-*` root. `remote-*` is explicit-retention only; a date directory records its reuse reason and review point. Resource cleanup never substitutes for session control.
