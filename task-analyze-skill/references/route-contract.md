...
For code, run one bounded producer Quick Check before presentation. Do not run Mini/Fast Verify before first presentation; show the completed result immediately after Quick Check. A low-risk single-result small task (`0-24`) records `intentionally_skipped_simple_task` and does not create an End Task. Standard/complex/advanced work, any medium/high-risk task, and multi-stage work are Ending-required: its producer emits `ending-required` and the parent must create one persistent projectless `End Task-{task}`. Before launch, capture the immutable origin session (`thread_id`, `host_id`, exact project ID, and project root). Retain every explicit acceptance check in its plan. Each check keeps a complexity score for proportional scope and classification only; the Ending primary is fixed `gpt-5.3-codex-spark|xhigh`. Explicit Spark model, effort, scheduler, or required-modality unavailability permits the one registry-floor `gpt-5.6-luna|low` fallback; correctness, quality, protocol, execution, timeout, or acceptance failure never switches the verifier. Resolve the source project ID by exact canonical root, build the single host launch request with `ending_verification_plan.py create-launches --project-id <resolved-id>`, invoke its projectless `codex_app__create_thread`, acknowledge the returned `threadId`, `hostId`, project ID, actual pair, and any approved availability reason with `ack-launch`, and require `audit-launches` to report `end_task_trigger_rate=100%`. Worker prompts keep the originating project root as execution context and use project-root-relative paths. A missing acknowledgement is BLOCKED; a handoff file alone is never triggered. The Ending executes the smallest real observable checks and semantic acceptance; all required checks must PASS before one terminal memory/classification/record closeout. PASS/FAIL/BLOCKED End Tasks remain visible. Every terminal Ending reports task/check difficulty, producer/verifier pairs, attempts, first/retry pass, suitability, next pair/action, and Obsidian model-record status/link. FAIL or acceptance mismatch records exact evidence and automatically submits a bounded `repair_prompt` through `codex_app__send_message_to_thread` to the immutable origin session. That source session repairs the authorized producer result, runs Quick Check, presents a new result and receipt, and starts a fresh Spark-first projectless Ending with `--repair-of-lifecycle-id`, for at most three attempts. Missing origin metadata, failed prompt submission, or exhausted repair attempts is BLOCKED, never verified; no replacement repair session or End/Fix task auto-archives or deletes itself, and every terminal verdict is recorded automatically.
