...
For code, run one bounded producer Quick Check before presentation. Do not run Mini/Fast Verify before first presentation; show the completed result immediately after Quick Check. End Task is required only for complex work or an important result that needs tests, records, or durable memory; exact read-only work, opening a file, and a one-value edit skip it. Before launch, capture the immutable origin session (`thread_id`, `host_id`, exact project ID, and project root). Each eligible independent check gets its own persistent projectless `End Task-{task}-{check}`, complexity score, and quality-ladder model/effort in the global task list. Resolve the source project ID by exact canonical root, build host launch requests with `ending_verification_plan.py create-launches --project-id <resolved-id>`, invoke every listed projectless `codex_app__create_thread`, acknowledge each returned `threadId`, `hostId`, and project ID with `ack-launch`, and require `audit-launches` to report `end_task_trigger_rate=100%`. Worker prompts keep the originating project root as execution context and use project-root-relative paths. A handoff file without those acknowledgements is pending, not triggered. The eligible task executes its smallest real observable check and semantic acceptance check; all required checks must PASS. PASS/FAIL/BLOCKED End Tasks remain visible. Every terminal Ending reports task/check difficulty, producer/verifier pairs, attempts, first/retry pass, suitability, next pair/action, and Obsidian record status/link. FAIL or acceptance mismatch records exact evidence and automatically submits a bounded `repair_prompt` through `codex_app__send_message_to_thread` to the immutable origin session. That source session repairs the authorized producer result, runs Quick Check, presents a new result and receipt, and starts a fresh projectless Ending with `--repair-of-lifecycle-id`, for at most three attempts. Missing origin metadata, failed prompt submission, or exhausted repair attempts is BLOCKED, never verified; no replacement repair session or End/Fix task auto-archives or deletes itself, and every terminal verdict is recorded automatically.
...
- Open Chrome: perform the action and present completion; skip Ending unless the user needs a recorded/tested state.
