...
Use `model_identity_disclosure.py` to disclose the task score, model and route, and each delegated stage. Surface its message in the conversation; JSON fields hidden in tool output are insufficient. It distinguishes runtime receipts, verified entry identity, task assignment and unavailable identity. Keep routing artifacts private under project `Cache/temp-*`, and never place raw prompts or results into shared model capability metadata.
