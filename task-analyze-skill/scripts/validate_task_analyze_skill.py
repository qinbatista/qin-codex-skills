...
REQUIRED_GLOBAL_BOOTSTRAP_TEXT = [
    ...
    "task_route_dispatcher.py run-plan", "no benchmark required", "Exact one-source/tool/image uses `task_complexity_score.py`",
    "2 Real PASS trial down 1 rung", "quality FAIL up 1", "missing Obsidian uses local history,queues projection,no block",
    "Producer owns files/skills/Quick Check", "Every result,including read-only,triggers a visible End Task", "exact saved project",
    "codex_app__list_projects", "ending_verification_plan.py create-launches --project-id", "codex_app__create_thread",
    "ack threadId+hostId+projectId", "audit-launches end_task_trigger_rate=100%", "handoff is pending", "all checks PASS",
    "End/Fix stay visible", "never auto-archive/delete", "attempts,first/retry pass", "suitability", "Obsidian link/status",
    "same-project Fix", "fresh End Task", "up to 3 repairs", "BLOCKED only unavailable/external/limit",
    "never same-task subtask/emulate/wait/self-verify", "Terminal receipts write local then same event ID to Obsidian",
    ...
    "compare task vs +Ending", "all exact PASS", "all result/Ending PASS", "Primary=execution time+tokens", "primary totals lower",
...
REQUIRED_PROMPT_SKILL_TEXT = ["Always use for every task", "100% global prompt-task gate across projects", "Ordinary prose does not trigger it", "Prompt-in-code also loads its owning code executor", "Present the completed prompt or instruction artifact immediately", "mandatory scored Ending lifecycle", "ending_verification_plan.py", "exact saved project", "Every terminal task stays visible", "Never auto-archive or delete", "fresh verifier"]
...
REQUIRED_VERIFY_SKILL_TEXT = ["execute a real proportional test", "ending_verification_plan.py", "own `0-100` check score", "All required checks must PASS", "{\"type\":\"project\"", "create-launches --project-id", "ack-launch", "end_task_trigger_rate=100%", "remain visible", "Never call `set_thread_archived`", "same saved project", "structured model assessment", "fresh Ending task", "BLOCKED does not count as verified"]
REQUIRED_ENDING_PLAN_IMPLEMENTATION = ["BAND_ROLES", "weak_default", "balanced_default", "balanced_complex", "frontier_complex", "separate_persistent_tasks", "THREAD_TARGET", "resolve_exact_project_root", "TERMINAL_THREAD_POLICY", "keep_visible", "projectId", "project_binding", "subprocess.run", "create_repair_task_then_fresh_ending", "max_repair_attempts", "codex_app__create_thread", "create-launches", "ack-launch", "audit-launches", "end_task_trigger_rate", "structured model_assessment", "Never call set_thread_archived"]
...
def validate(skill_dir, models_cache_path, global_agents_path=Path.home() / ".codex" / "AGENTS.md", global_skills_root=Path.home() / ".codex" / "skills", global_hooks_path=Path.home() / ".codex" / "hooks.json"):
    ...
    failures.extend(missing_terms("agents/openai.yaml", agent_text, ["score every task 0-100", "resolve entry", "show score/band plus route change", "2+ stages=>per-stage graph", "final Model stages includes every difficulty/pair/status", "History picks lowest-correct", "entry can move up/down", "no-history<=entry", "obsidian_adaptive_model_runner.py", "exact/tool/image uses task_complexity_score.py", "Small low-risk edit <=24 tries Spark-low", "quality failure suppresses its band", "Every result requires End Task in the exact saved project", "ending_verification_plan.py", "scored/model project/local End/check", "ack-launch(threadId+hostId+projectId)", "audit-launches=100%", "all PASS and remain visible", "never auto-archive/delete", "attempt count", "first/retry pass", "suitability", "Obsidian link", "same-project Fix", "fresh verification<=3", "BLOCKED is not verified", "Local+Obsidian records", "Never emulate task creation", "same-task/self-verify forbidden"]))
