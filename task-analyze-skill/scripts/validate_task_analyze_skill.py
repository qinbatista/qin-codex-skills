...
REQUIRED_FILES = [
    ...
    "scripts/benchmark_prompt_contract.py",
    "scripts/benchmark_auto_entry_bridge.py",
    ...
    "tests/test_benchmark_auto_entry_bridge.py",
...
REQUIRED_SKILL_TEXT = [
    ...
    "Ending is required when the released result exposes",
    "Only explicit `trivial_value_only` work with no other surface records `intentionally_skipped_simple_task`",
    "its parent must create and acknowledge one global projectless End Task",
    ...
    "Every observable or material final aggregate code receipt emits `ending-required`",
    "real_test",
    "information_update",
    "memory_update",
    ...
    "`step_kind`, and controlled capability tags",
    ...
    "record-controller-restriction",
    "skips the cooling model and selects the next stronger supported controller",
    "fresh independent projectless Repair Task",
    "never messages, steers, interrupts, terminates, hands off, moves, or mutates an existing task/session",
    ...
    "lower steady selected-execution logical tokens and time in every score tier and in aggregate",
    "Per-tier medians, wins, actual first-result wait",
    "runtime noise",
    ...
    "two receipt-matched Real passes",
    "Receipt-backed producer PASS/FAIL is learning-eligible",
    "known unreceipted assignment is retained as an observation only",
    "writes the local routing event before the canonical Obsidian category projection",
    "Direct always enters and remains on `gpt-5.6-sol|ultra`",
    "Auto enters on `gpt-5.6-luna|max`",
    "at least one receipt/runtime-proven adaptive child or graph",
    "receipt/runtime-proven adaptive child or graph",
    "parallel-source/fused-merge graph",
    "valid bounded task graph",
    "Writable graphs",
    "Any savings claim requires",
    "entry-aware shared cold start",
    "task + Ending check",
    ...
    "same small, standard, complex, and advanced benchmark cohort",
...
REQUIRED_ROUTE_TEXT = [
    ...
    "A released result emits `ending-required` for an observable or material trigger",
    ...
    "end_task_trigger_rate=100%",
    "record-controller-restriction",
    "skips the cooling model and selects the next stronger controller",
    "fresh independent global projectless Repair Task",
    "never sends, steers, interrupts, terminates, hands off, moves, or mutates another session",
    "Eligible single-node text/code production calls `obsidian_adaptive_model_runner.py` exactly once even on cold start",
    "Every model node has its own score, band, exact pair",
...
REQUIRED_ADAPTIVE_TEXT = [
    ...
    "scoring `0-24` runs the same-session outcome gate before Spark",
    "suppress Spark for the matching step-capability fingerprint",
    "zero-result, zero-token Spark operational failure",
    "Obsidian evidence and same-name/display-page evidence never cross project keys",
    "Local receipt evidence may transfer across modules or roots only when the exact step-capability fingerprint and difficulty context match",
    "generic work also requires the same module",
    "`strategy_performance.py` remains the separate authority",
    "automatically writes a receipt-backed producer outcome",
]
REQUIRED_OBSIDIAN_RUNNER_IMPLEMENTATION = ["project-memory-skill", "obsidian_model_memory.py", "obsidian_model_memory.recommend_model", "model_execution_receipt.adaptive_producer_authorization", "node_role=\"result-producer\"", "attempt_pair", "active_fallback_pair", "operational_failure_pairs", "immediate_operational_fallback", "result_lifecycle_policy", "ending_surface_requirements", "real_test", "information_update", "memory_update", "ending_skip_reason", "no_real_ending_surface", "ending_required", "ending_requirement", "ending_real_status", "intentionally_skipped_simple_task", "missing_expected_code_ending", "missing_expected_non_simple", "producer_check_scope", "one_smallest_local_quick_check", "first_result_release", "immediate_after_quick_check", "deferred_verification_owner", "_emit_ending_required", "_final_aggregate_fields", "final_aggregate_receipt", "all_result_nodes_settled", "subprocesses_settled", "ending_launch_ready", "scheduled_graph_release_failed", "ending-required", "create_projectless_end_task", "ack_required", "pending", "resolve_fast_path_args", "infer_complexity_score", "infer_memory_symbol", "complexity_score", "complexity_band", "step_kind", "capability_tags", "capability_fingerprint", "switch_direction", "switch_change", "hashlib.sha256", "explicit_fields", "fast_path", "recommendation = _exact_contract_recommendation(prompt, _recommend(args, prompt))", "adaptive-producer", "workspace-write", "scheduled_source_paths", "schedule_admission", "SINGLE_PRODUCER_SOURCE_BYTE_LIMIT", "single_producer_lower_estimated_logical_tokens", "parallel_independent_sources", "parallel_sources_fused_final", "fuses_owned_source_with_dependencies", "_deterministic_capture_eligible", "task_route_dispatcher.DETERMINISTIC_SOURCE_READ", "_lean_context_eligible", "lean_context_mode", "task_route_dispatcher.run_plan", "ending_fast_route_fields", "scheduled_result_node_count", "parallel_branch_count", "model-switch-notice", "user_visible_message", "increased the model to", "_graph_model_route_notice", "execution_lifecycle_contract", "_emit_execution_lifecycle_notice", "execution-lifecycle-notice", "planned_single", "planned_graph", "_graph_route_required_summary", "graph-route-required", "route-required", "material_result_stages", "graph_required", "multiple_material_result_stages_require_dynamic_task_graph", "build_dynamic_task_graph_and_call_task_route_dispatcher_once"]
...
REQUIRED_ROUTING_POLICY_ENDING_FAST_IMPLEMENTATION = ["ENDING_FAST_CONFIG", "ENDING_FAST_PRIMARY_PAIR", "ENDING_FAST_FALLBACK_PAIR", "ENDING_FAST_DEFINITIONS", "ending_fast_route_fields", "EXECUTION_LIFECYCLE_VERSION", "execution_lifecycle_contract", "lowest_suitable_capability_history_entry_pair", "quality_neutral_retry_or_allowed_fallback"]
REQUIRED_DISPATCHER_ENDING_FAST_IMPLEMENTATION = ["ENDING_FAST_CONFIG", "ENDING_FAST_PRIMARY_PAIR", "ending_fast_route_fields", "apply_ending_fast_route", "ending_fast_primary", "availability_only", "ending_availability_fallback", "failure_class", "execution_lifecycle_for_plan", "execution_lifecycle"]
REQUIRED_DISPATCHER_MEMORY_CONSISTENCY_IMPLEMENTATION = ["PROJECT_RESULT_CONSISTENCY_ACTIONS", "process_skill_and_agents", "real_execution", "effective_result_memory", "ending_memory_only", "isolated_projectless_repair", "immutable_evidence_only", "wait_without_interruption", "effective_only", "project_result_consistency_action", "append_correction", "reconcile_projection", "launch_isolated_projectless_repair", "existing_session_mutation"]
...
REQUIRED_RECEIPT_GUARD_IMPLEMENTATION = [
    ...
    "prepare_lean_context_home",
    "LEAN_CONTEXT_HOME_NAME",
    "LEAN_CONTEXT_DISABLED_FEATURES",
    "lean_context_mode",
    "lean_context_disabled_features",
    "unavailable_fallback",
    "--sandbox",
]
REQUIRED_BENCHMARK_RUNNER_RECOVERY_IMPLEMENTATION = ["recover_before_snapshot_from_after_and_rollout_delta", "after_db_plus_complete_rollout_delta"]
REQUIRED_GLOBAL_BOOTSTRAP_TEXT = [
    ...
    "project→Model Switch→category→shared links", "classify material stages before any producer", "2+ result stages activate one graph", "single-stage only",
    "each stage records score,band,pair,purpose,deps,stop", "Eligible low-risk low-ambiguity text/code/write/execute score<=24",
    "same-session gate→Spark-low if no stronger route", "Ending FAIL suppresses fingerprint", "Execution lifecycle:", "ultra-simple single result=>direct", "other single result=>plan+single execution", "multi-result=>plan+dependency graph", "emit execution-lifecycle-notice", "operational failure is quality-neutral", "new topic resets", "Independent nodes parallel",
    "shared writes/order/output deps linear", "resolve CODEX_HOME", "pipe exact task once via platform Python",
    "skills/task-analyze-skill/scripts/obsidian_adaptive_model_runner.py", "never read/edit/answer inline", "graph-route-required", "route-required", "no model ran", "full Task Analyze", "no single-producer retry", "LOCKED_ROUTE_NODE emits Code Gate", "universal philosophy+one active language profile+matched categories", "missing Code Gate/reference is routing failure", "C# aliases use only Unity C#", "does code/result+one Quick Check,emits CODE READY,stops",
    "Parent launches one End only after final aggregate `ending-required` receipt/release", "child/subprocess receipts never launch it", "ENDING_TASK_WORKER controller is Spark-xhigh first", "same End Task continues one bounded action at a time", "ENDING_CHECK_WORKER reads listed Skills", "dynamic_task_graph",
    ...
    "Producer owns files/skills/one Quick Check only", "Only the final aggregate `ending-required` or `ending_required=true` receipt after all result nodes settle=>one independent global projectless Ending", "real_test", "information_update", "memory_update", "create-launches --producer-receipt(final) --project-id(context-only)", "`intentionally_skipped_simple_task`=>no End Task", "ending_skip_reason=no_real_test_or_information_or_memory_or_material_update", "missing acknowledgement=>BLOCKED", "call codex_app__create_thread target",
    "resolve root with codex_app__list_projects", "ending_verification_plan.py create-launches --producer-receipt(final) --project-id(context-only)", "codex_app__list_threads", "projectId=null/absent",
    ...
    "fresh independent global projectless Repair Task", "immutable origin is context only", "never send/steer/interrupt/terminate/handoff/move/mutate any existing task/session", "active task owns required write surface", "waits without messaging/interruption", "fresh global projectless End Task with --repair-of-lifecycle-id", "up to 3 repairs", "BLOCKED only unavailable/external/limit,not active-task waiting",
    "never repair in verifier", "bounded check-worker waits occur only inside detached Ending", "Terminal events write local+same-ID Obsidian records", "only producer receipts move routing", "approval_policy=\"never\"", "never open Terminal.app, Computer Use, or a second terminal to click Allow", "host Allow prompt is BLOCKED",
    ...
    "one independent global projectless Ending", "gpt-5.3-codex-spark|xhigh", "score scopes checks only", "skip cooling model,next stronger supported controller",
    "Process Skill/AGENTS,real execution,effective project-result memory agree", "Ending appends only memory-only correction", "Skill/execution defects launch isolated Repair Task", "Next tasks read effective project-result memory only",
...
REQUIRED_PYTHON_REFERENCE_TEXT = ["## Quick Check And Detached Ending", "run the smallest safe focused smoke", "Present `CODE READY` immediately after exactly one Quick Check", "A durable Python change normally exposes `real_test`", "trivial_value_only", "persistent global projectless Ending", "list_threads", "projectId=null", "capability-routed Terra/Sol `ENDING_CHECK_WORKER`", "Multiple checks stay inside that one Ending lifecycle", "every required check must PASS", "effective project-result memory", "memory-only defect", "fresh isolated Repair Task", "never contacts or interrupts an existing session", "wait without messaging", "fresh parent-linked Ending"]
REQUIRED_WORKFLOW_MEMORY_CONSISTENCY_TEXT = ["Process Skill/AGENTS", "fresh real execution evidence", "effective project-result memory", "memory_record_defect", "memory_projection_defect", "skill_contract_defect", "execution_drift", "append-only correction", "isolated Repair Task", "Next tasks read only effective project-result memory"]
...
REQUIRED_PROMPT_SKILL_TEXT = ["Always use for every task", "100% global prompt-task gate across projects", "Ordinary prose does not trigger it", "Prompt-in-code also loads its owning code executor", "Present the completed prompt or instruction artifact immediately", "conceptual/process material", "durable project-memory closeout", "ending-required", "global projectless Ending", "Spark-xhigh is first", "persisted real quota/five-hour/provider restriction", "next stronger supported controller", "All checks PASS", "fresh independent projectless Repair Task", "waits without messaging", "Never auto-archive or delete Ending or Repair tasks"]
REQUIRED_PROMPT_AGENT_TEXT = ["Always use $prompt-skill", "durable prompts/instructions", "conceptual/process material", "projectless Ending plus durable memory", "Spark-xhigh first", "persisted real quota cooldown", "independent projectless Repair", "never contacts or interrupts existing sessions", "fresh parent-linked Ending"]
REQUIRED_VERIFY_SKILL_TEXT = ["post-result Ending Real Verify when the released result exposes", "material structural, non-trivial code, conceptual, or process update", "trivial_value_only", "persistent global projectless", "final aggregate producer receipt", "child/subprocess receipt never launches it", "compact, one-check actions", "gpt-5.3-codex-spark|xhigh", "next stronger supported controller", "record-controller-restriction", "Terra/Sol `ENDING_CHECK_WORKER`", "run every direct plan item", "delegated_check_worker", "All required checks must PASS", "{\"type\":\"projectless\"", "codex_app__list_threads", "projectId=null", "--thread-scope global --thread-project-id null", "absolute origin project root", "projectless cwd is unrelated", "create-launches", "ack-launch", "end_task_trigger_rate=100%", "remains visible", "Never call `set_thread_archived`", "fresh projectless Repair Task", "codex_app__create_thread", "never message, steer, interrupt, terminate, hand off, move, or mutate", "structured `model_assessment`", "fresh global projectless Ending", "BLOCKED does not count as verified", "strict no-op for preference memory"]
REQUIRED_GLOBAL_BOOTSTRAP_TEXT += [
...
REQUIRED_GLOBAL_ENTRY_ASSET_TEXT += REQUIRED_GLOBAL_BOOTSTRAP_TEXT[-9:]
REQUIRED_VERIFY_SKILL_TEXT += [
...
REQUIRED_WORKFLOW_RESOURCE_TEXT = [
...
REQUIRED_WORKFLOW_RESOURCE_LEDGER_TEXT = [
    "SCHEMA_VERSION = 2",
    "def acquire_path(",
    "def seal_path(",
    "def cleanup_path(",
...
def validate_shared_ladder(text):
    ...
    if not isinstance(private_contract, dict) or private_contract.get("authority") != "dual_local_and_obsidian" or private_contract.get("local_path_template") != "~/.codex/model-routing-memory/events.jsonl" or private_contract.get("projection_path_template") != "Model Switch.md" or private_contract.get("record_path_template") != "<owner-routing>/<Category>.md" or private_contract.get("native_wikilink_graph") is not True or private_contract.get("stable_categories") != REQUIRED_MODEL_SWITCH_CATEGORIES or private_contract.get("event_id_dedupe") is not True or private_contract.get("specificity_order") != ["project_task", "module", "file", "symbol"] or private_contract.get("fields_only") is not True or private_contract.get("hierarchy_notes") is not False or private_contract.get("legacy_local_json") != "read_only_inactive":
...
def validate(skill_dir, models_cache_path, global_agents_path=Path.home() / ".codex" / "AGENTS.md", global_skills_root=Path.home() / ".codex" / "skills", global_hooks_path=Path.home() / ".codex" / "hooks.json"):
    ...
    benchmark_prompt_text = read_text(paths["scripts/benchmark_prompt_contract.py"])
    benchmark_bridge_text = read_text(paths["scripts/benchmark_auto_entry_bridge.py"])
    ...
    failures.extend(missing_terms("agents/openai.yaml", agent_text, ["score 0-100", "route per stage", "2+=>graph", "Code Gate", "one Quick Check", "CODE READY", "observable or material structural/non-trivial-code/conceptual/process update", "material=>durable memory closeout", "trivial_value_only", "Global projectless End", "null project readback", "Spark-xhigh first", "quota/5-hour/provider cooldown", "skip cooling model", "next stronger controller", "independent projectless Repair Task", "never contact/steer/interrupt/terminate/handoff/move/mutate existing sessions", "active-task write conflict waits", "fresh parent-linked Ending<=3", "Workers evidence-only", "all PASS", "visible"]))
    ...
    failures.extend(missing_terms("benchmark runner recovery", benchmark_runner_text, REQUIRED_BENCHMARK_RUNNER_RECOVERY_IMPLEMENTATION))
    failures.extend(missing_terms("benchmark mode gate", benchmark_gate_text, ["receipt_node_type_mismatch", "receipt_entry_context_active", "receipt_authorization_source_mismatch", "receipt_benchmark_run_id_mismatch", "receipt_workload_id_mismatch", "receipt_auto_launch_unverified", "receipt_result_source_invalid", "receipt_execution_prompt_mismatch", "receipt_result_ready_event_invalid", "receipt_result_ready_timing_mismatch", "benchmark-global-inline", "bootstrap-task", "context_mode", "lean_bounded_worker", "MAXIMUM_PAIRED_TIME_REGRESSION_MS = 2_000", "material_pair_regression_count"]))
    failures.extend(missing_terms("benchmark prompt binding", benchmark_prompt_text, ["BOUND_WORKLOAD_SHA256", "CODEX_AUTO_BENCHMARK_PYTHON", "no interpreter discovery or substitution", "workload body is deliberately absent", "hashlib.sha256", "Launch exactly one bridge process", "poll that same session until exit", "must never launch a second process"]))
    failures.extend(missing_terms("benchmark Auto bridge", benchmark_bridge_text, ["PYTHON_ENV", "interpreter does not match its binding", "claim_adaptive_launch", "os.O_EXCL", "prepare_read_only_workspace", "source_tree_sha256", "context_mode", "lean_bounded_worker", "benchmark adaptive producer was already launched"]))
    ...
    failures.extend(missing_terms("dispatcher project-result consistency", dispatcher_text, REQUIRED_DISPATCHER_MEMORY_CONSISTENCY_IMPLEMENTATION))
    ...
    failures.extend(missing_terms("benchmark evidence", benchmark_evidence_text, ["# Adaptive Model Test and Benchmark Evidence", "Implementation status: PASS", "Correctness status: PASS", "Steady-state performance hypothesis: PASS", "End-to-end first-result diagnostic: FAIL", "41 tests", "29 tests", "551 tests", "102 tests", "65 tests", "Non-benchmark routing holdout", "46/46 cases passed", "4,600 classifications", "deterministic classification regression benchmark", "12/12 PASS", "all_correct=true", "gpt-5.6-sol|ultra", "gpt-5.6-luna|max", "Spark-low", "Terra-medium", "Luna-low", "3/3 PASS", "49.260%", "26.203%", "120.046", "not a universal", "simple", "medium", "complex"]))
    ...
    workflow_skill_path = global_skills_root / "workflow-skill" / "SKILL.md"
    if not workflow_skill_path.exists():
        ...
        if not workflow_resource_path.exists():
            ...
            failures.extend(missing_terms("Workflow task resource lifecycle", read_text(workflow_resource_path), REQUIRED_WORKFLOW_RESOURCE_TEXT))
        if not workflow_resource_ledger_path.exists():
            failures.append(f"Workflow task resource ledger missing: {workflow_resource_ledger_path}")
        else:
            workflow_resource_ledger_text = read_text(workflow_resource_ledger_path)
            failures.extend(missing_terms("Workflow task resource ledger", workflow_resource_ledger_text, REQUIRED_WORKFLOW_RESOURCE_LEDGER_TEXT))
            for forbidden in ("subprocess", "os.kill(", "send_message_to_thread", "interrupt_agent", "set_thread_archived", "terminate_task"):
                if forbidden in workflow_resource_ledger_text:
                    failures.append(f"Workflow task resource ledger contains forbidden cleanup control primitive: {forbidden}")
