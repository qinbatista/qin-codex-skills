...
REQUIRED_GLOBAL_BOOTSTRAP_TEXT = [
    ...
    "Producer owns files/skills/one Quick Check only", "Only the final aggregate `ending-required` or `ending_required=true` receipt after all result nodes settle=>one independent global projectless Ending", "create-launches --producer-receipt(final) --project-id(context-only)", "`intentionally_skipped_simple_task`=>no End Task and only for eligible non-code", "missing acknowledgement=>BLOCKED", "call codex_app__create_thread target",
