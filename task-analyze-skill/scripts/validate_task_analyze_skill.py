...
REQUIRED_ADAPTIVE_TEXT = [
    ...
    "scoring `0-24` still tries Spark first",
...
REQUIRED_GLOBAL_BOOTSTRAP_TEXT = ["# Task Lifecycle", "Score every submission 0-100", "small 0-24", "standard 25-49", "complex 50-74", "advanced 75-100", "show compact score/model/route status", "Resolve entry first", "exact step-capability/band history wins across roots", "Sol/high may downgrade", "Luna-max/lower may upgrade", "no match stays <=entry", "reuse recovered/lowest-correct", "native project→Model Switch→category→shared-category wikilinks", "no JSON sidecar", "Split distinct work only", "2+ material result stages auto-activate one graph", "each result/Ending records score,band,pair,purpose,deps,stop", "parent score never fixes the project model", "Eligible low-risk low-ambiguity text/code/write/execute score<=24", "executes Spark-low first", "the entry routes it instead of answering inline", "Ending quality FAIL suppresses that fingerprint", "Dependency-ready independent nodes run in parallel", "shared writes/order/output deps stay linear", "Before eligible text/code", "entry MUST resolve CODEX_HOME", "pipe exact task once via platform Python", "skills/task-analyze-skill/scripts/obsidian_adaptive_model_runner.py", "never read/edit/answer inline", "multi-node work saves one schema-2 `dynamic_task_graph`", "task_route_dispatcher.py run-plan", "Never require a benchmark for a valid graph", "Exact one-source/tool/image uses `task_complexity_score.py`", "2 Real PASS down 1 rung", "quality FAIL up 1", "missing Obsidian uses local history,queues projection,no block", "Producer owns files/skills/Quick Check", "End Task required after result", "score each check", "ending_verification_plan.py create-launches", "codex_app__create_thread", "ack each threadId+hostId", "audit-launches end_task_trigger_rate=100%", "handoff alone is pending", "global projectless End/Fix Tasks", "all checks PASS", "PASS records then self-archives", "FAIL records evidence", "fresh End Task", "up to 3 repairs", "BLOCKED only unavailable/external/limit", "never same-task subtask/emulate/wait/self-verify", "Terminal receipts write local history first", "the same event ID to Obsidian", "future routes merge/dedupe", "entry/selected/effective/next pairs", "step fingerprint,score/band,direction,recovery", "AUTO_BENCHMARK_ENTRY", "Direct Sol-ultra no Skill", "Auto Luna-max stable child/graph", "Primary=execution time+tokens", "route/calibration/Ending separate", "first-result diagnostic", "Sol probe exact result+route", "all result/Ending PASS", "primary totals lower", "No hook", "Final PASS/BLOCKED Ending-only"]
...
REQUIRED_PROMPT_SKILL_TEXT = ["Always use for every task", "100% global prompt-task gate across projects", "Ordinary prose does not trigger it", "Prompt-in-code also loads its owning code executor", "Present the completed prompt or instruction artifact immediately", "mandatory scored Ending lifecycle", "ending_verification_plan.py", "global projectless", "PASS records durable evidence then self-archives", "fresh verifier"]
...
def validate(skill_dir, models_cache_path, global_agents_path=Path.home() / ".codex" / "AGENTS.md", global_skills_root=Path.home() / ".codex" / "skills", global_hooks_path=Path.home() / ".codex" / "hooks.json"):
    ...
    failures.extend(missing_terms("agents/openai.yaml", agent_text, ["score every task 0-100", "resolve entry", "show score/band plus route change", "2+ stages=>per-stage graph", "final Model stages includes every difficulty/pair/status", "task-type/step/difficulty history picks lowest-correct", "Sol/high may downgrade", "Luna-max/lower may upgrade", "no-history starts at/below entry", "obsidian_adaptive_model_runner.py", "exact/tool/image uses task_complexity_score.py", "Small low-risk edit <=24 tries Spark-low", "quality failure suppresses its matching band", "End Task is hard-required", "ending_verification_plan.py", "scored/model projectless End/check", "create-launches->codex_app__create_thread->ack-launch(threadId+hostId)->audit-launches=100%", "all must PASS", "PASS records then self-archives", "FAIL/BLOCKED stays visible", "FAIL=>separate projectless Fix with exact error", "fresh verification", "up to three attempts", "BLOCKED never means verified", "Local lifecycle always records", "receipt events=>Obsidian", "entry/selected/effective/next", "Never emulate task creation", "same-task subagent", "self-verify"]))
