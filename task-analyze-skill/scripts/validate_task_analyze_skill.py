...
REQUIRED_ROUTE_TEXT = [
    ...
    "A released result emits `ending-required` only when it exposes `real_test`, `information_update`, or `memory_update`",
...
REQUIRED_DISPATCHER_MEMORY_CONSISTENCY_IMPLEMENTATION = ["PROJECT_RESULT_CONSISTENCY_ACTIONS", "process_skill_and_agents", "real_execution", "effective_result_memory", "ending_memory_only", "immutable_origin", "effective_only", "project_result_consistency_action", "append_correction", "reconcile_projection", "return_to_origin"]
...
REQUIRED_GLOBAL_BOOTSTRAP_TEXT = [
    ...
    "never emulate another Ending,repair in verifier,or wait for origin", "bounded check-worker waits occur only inside detached Ending", "Terminal events write local+same-ID Obsidian records", "only producer receipts move routing", "approval_policy=\"never\"", "never open Terminal.app, Computer Use, or a second terminal to click Allow", "host Allow prompt is BLOCKED",
...
def validate(skill_dir, models_cache_path, global_agents_path=Path.home() / ".codex" / "AGENTS.md", global_skills_root=Path.home() / ".codex" / "skills", global_hooks_path=Path.home() / ".codex" / "hooks.json"):
    ...
    failures.extend(missing_terms("agents/openai.yaml", agent_text, ["score 0-100", "resolve entry up/down", "show band/route", "classify stages", "2+=>graph", "route-required=no model", "full Task Analyze+run-plan once", "Model stages:each pair/status", "history=lowest-correct", "obsidian_adaptive_model_runner.py", "LOCKED_ROUTE_NODE emits code-rule-notice before source", "Code Gate=universal philosophy+one active language profile+matched categories", "missing notice/reference fails", "C# aliases only Unity C#", "one Quick Check", "CODE READY", "Ending surfaces=real_test|information_update|memory_update", "ending_skip_reason=no_real_test_or_information_or_memory_update", "global projectless End via", "ending_verification_plan.py", "exact create_thread target", "codex_app__list_threads", "projectId=null/absent", "same-task-subtask BLOCKED/not verified", "return without polling", "gpt-5.3-codex-spark|xhigh", "fallback gpt-5.6-luna|low", "ENDING_CHECK_WORKER reads Skills", "never edits/repairs/routes/lifecycle", "immutable origin", "trigger=100%", "all PASS;visible", "repair_prompt", "codex_app__send_message_to_thread", "fresh Spark-first End<=3"]))
