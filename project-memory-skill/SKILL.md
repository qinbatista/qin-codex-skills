...
- The same private ledger also records sanitized session effort: hashed Codex session key, optional task name and task-scope key, project/task/module, same-task turn count, user-effort count, repeated-failure state, solving surface, information burden, model difficulty/family, estimated step count/class, low-through-ultra effort class, task length, prior model pair(s), preferred solving pair, selected route, and task fingerprint. A repeated corrective submission with the same topic chooses model family from difficulty and information burden, estimates effort from the steps needed to reach the goal, and only then moves to a stronger model at that effort; a topic jump in the same session starts a separate effort state. Never store raw prompts, raw results, or session IDs.
