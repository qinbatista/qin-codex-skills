...
For memory operations, validate an explicit `--vault` first, then the project `obsidian_vault` registry entry, `CODEX_OBSIDIAN_VAULT`, and a readable configured-open vault in platform-native `obsidian.json`; without one of those sources, report Obsidian unavailable.
