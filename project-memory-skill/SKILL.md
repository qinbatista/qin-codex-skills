...
- The local event ledger at `~/.codex/model-routing-memory/events.jsonl` is durable fast history. Receipt-backed producer outcomes are learning-eligible; Ending controller assignments, including restriction-aware cooldown escalation, are visible non-learning observations and never move producer routing. Project change-memory remains separate.
...
Personal preference and technical-working-trait memory is a third stream. Every observable or material result is scanned inside Ending; material structural/non-trivial-code/conceptual/process updates must complete durable project-memory closeout. Only explicit trivial value-only work with no other surface may skip. The active restriction-aware controller owns scan and terminal record; check workers never write preference, project-result, or model-routing memory.
