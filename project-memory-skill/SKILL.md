...
- The receipt-backed local event ledger at `~/.codex/model-routing-memory/events.jsonl` is the durable fast history. Obsidian projects it as a compact project `Model Switch.md` entry plus stable native-linked category pages. Project change-memory remains a separate system.
