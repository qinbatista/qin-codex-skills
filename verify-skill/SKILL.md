...
3. build one plan for every new task containing the smallest exact real/completion checks; each check keeps its own `0-100` score only to scope risk and evidence, never to select the Ending model;
...
- On PASS, perform one terminal routing/classification record, the bounded personal-memory scan, and the sanitized project-change result closeout when the task supplied durable-change memory context. Process philosophy never becomes result-memory payload.
