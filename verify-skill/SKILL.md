...
Verification has two scopes: user-requested verification as the task itself, and mandatory post-result Ending Real Verify. Every durable code change, plus standard/complex/advanced, medium/high-risk, and multi-stage work, requires one Ending; only eligible low-risk single-result small non-code work records `intentionally_skipped_simple_task`. Code producers run exactly one smallest local Quick Check and present immediately. Build one plan and launch one persistent global projectless Ending. Its Spark-xhigh controller runs deterministic checks directly and may route semantic runtime, integration, code-quality, prompt, UI, or visual checks to saved Terra/Sol `ENDING_CHECK_WORKER` nodes. All required checks must PASS with fresh evidence.
...
4. write a scored lifecycle receipt, bind `--producer-receipt` when present, then create exactly one persistent global projectless `End Task-{task}`. Invoke only the generated `codex_app__create_thread` request whose target is exactly `{"type":"projectless"}` and has no project/current-task attachment fields; read the exact thread back with `codex_app__list_threads`, require `projectId=null` or absent, and persist that null placement plus the origin project binding through `ack-launch`;
...
- Build a plan with one check object per independent acceptance surface, but create exactly one persistent global projectless task for the plan. The source project ID remains execution/repair context only and never enters `create_thread.target`. `ack-launch` requires `--thread-scope global --thread-project-id null --placement-readback-tool codex_app__list_threads`; project/current-task/same-task-subtask placement is BLOCKED. Pass its lifecycle/plan IDs, exact commands, score/bands, actual Ending pair, receipts, project root, touched files, and allowed repair scope.
