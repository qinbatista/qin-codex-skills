...
4. bind the final passing published aggregate `--producer-receipt` only after it marks all result nodes and subprocesses settled, then create exactly one persistent global projectless `End Task-{task}`. Invoke only the generated `codex_app__create_thread` request whose target is exactly `{"type":"projectless"}` and has no project/current-task attachment fields; read the exact thread back with `codex_app__list_threads`, require `projectId=null` or absent, and persist that null placement plus the origin project binding through `ack-launch`;
