...
Verification has two scopes: user-requested verification as the task itself, and post-result Ending Real Verify when the released result exposes `real_test`, `information_update`, `memory_update`, or a material structural, non-trivial code, conceptual, or process update. Material updates require project-memory closeout; only an explicitly proven `trivial_value_only` edit may use the no-surface skip. Code producers normally expose `real_test` through exactly one smallest local Quick Check and present immediately. Launch one persistent global projectless Ending only after the final aggregate producer receipt and result release; a child/subprocess receipt never launches it. Its `gpt-5.3-codex-spark|xhigh` controller runs compact, one-check actions directly and may route semantic runtime, integration, code-quality, prompt, UI, or visual checks to saved Terra/Sol `ENDING_CHECK_WORKER` nodes. All required checks must PASS with fresh evidence.
...
A same-task subagent may never substitute for the global projectless Ending. Inside the already-created detached Ending only, the controller may launch bounded `ENDING_CHECK_WORKER` subagents with the exact saved pair, check, required Skills, and evidence path. They are not repair tasks and cannot edit. Only the final aggregate `ending-required` receipt gets one global Ending, acknowledged at `end_task_trigger_rate=100%`. FAIL creates a fresh independent global projectless repair session; captured origin metadata is immutable context only. Neither Ending nor Repair may send to, steer, interrupt, terminate, hand off, move, or mutate an existing task/session. Active-task write ownership makes the Repair wait without interruption; it is not a reason to stop the active task.
...
3. after the whole producer flow settles, build one plan for the final aggregate `ending-required` result containing the smallest exact real/completion checks; each check keeps its own `0-100` score only to scope risk and evidence, never to select the Ending model;
4. bind the final passing published aggregate `--producer-receipt` only after it marks all result nodes and subprocesses settled. Before lifecycle creation, validate its sanitized `model_learning_context`, capability profile, entry identity, and executed pair against the same model-memory contract used by terminal recording; reject an incompatible receipt before writing a lifecycle. Then create exactly one persistent global projectless `End Task-{task}`. Invoke only the generated `codex_app__create_thread` request whose target is exactly `{"type":"projectless"}` and has no project/current-task attachment fields; read the exact thread back with `codex_app__list_threads`, require `projectId=null` or absent, and persist that null placement plus the origin project binding through `ack-launch`;
5. run the bounded checks in that Ending and require all checks to PASS before one terminal closeout;
6. on FAIL, record exact bounded evidence and automatically create a fresh projectless Repair Task through `codex_app__create_thread`; its launch contains no existing thread/host target and must never message, steer, interrupt, terminate, hand off, move, or mutate any existing task/session;
7. the independent Repair checks write ownership read-only, waits without messaging or interruption while an active task owns the required surface, then repairs only the authorized producer scope, runs Quick Check, presents a new producer receipt, and supplies the failed parent ID through `--repair-of-lifecycle-id <id>` for a fresh global projectless Ending;
8. repeat the isolated repair and fresh-reverification loop for at most three repairs; only unavailable external state or the limit is BLOCKED, while an active-task conflict remains WAITING;
...
10. let every terminal ledger event record local and Obsidian history; only receipt-backed producer PASS/FAIL may update adaptive model learning. Keep every failed Ending and independent Repair visible; never call `set_thread_archived`, let the verifier edit the result, or expose private chain-of-thought.
...
- Build a plan with one check object per independent acceptance surface, but create exactly one persistent global projectless task only from the final aggregate receipt after all producer/subprocess work settles. The source project ID remains execution/repair context only and never enters `create_thread.target`. `ack-launch` requires `--thread-scope global --thread-project-id null --placement-readback-tool codex_app__list_threads`; project/current-task/same-task-subtask placement is BLOCKED. Start it with the explicit absolute origin project root plus compact project-relative plan, evidence, and receipt pointers because projectless cwd is unrelated; resolve those pointers against that root, then keep the same task active for one saved bounded check or closeout action at a time without serializing the plan into its prompt.
...
- A receipt/protocol failure in a lifecycle created before the current validator may terminate as FAIL with model learning explicitly unavailable; it must never become a false PASS or silently change producer quality routing. The origin republishes a schema-valid aggregate receipt before the parent-linked repair Ending starts.
- The Ending controller is Spark-xhigh first. A real model quota, five-hour limit, provider rate limit, or trusted retry-after is written to the durable controller-restriction store with restriction and retry timestamps. Run `record-controller-restriction` once, regenerate `create-launches`, skip every cooling model, and select the next stronger supported controller; after cooldown Spark is first again. Never retry the cooling model. Quality/correctness failures never create controller cooldowns.
- Prefer deterministic commands, schema/bounds checks, and immutable release-report digests so Spark closes quickly. Use capability-routed Terra/Sol check workers for real semantic image/UI, code-quality, prompt, integration, or runtime judgment; never replace required judgment with shallow Spark prose.
- Run every direct plan item with `ending_verification_plan.py run-check`. For `delegated_check_worker`, spawn exactly one dependency-ready worker whose prompt begins `ENDING_CHECK_WORKER`; require it to read all listed Skills and run that same exact command into the assigned evidence file. Spark accepts the file, never worker prose alone. Safe checks may run concurrently; shared state stays ordered.
- Load [Task Resource Lifecycle](../workflow-skill/references/task-resource-lifecycle.md). Each check writes evidence and its terminal record first; after last-consumer readback, release only exact Ending-owned or explicitly handed-off disposable resources in reverse acquisition order. Retain or defer preexisting, shared, conflicted, Unity, date, and remote resources. Resource cleanup never controls another Codex task, thread, session, or Ending.
...
- On failure or acceptance mismatch, record terminal FAIL and execute `repair_handoff.repair_launch` as one new global projectless session. The immutable origin is context only; never send or steer it. The independent Repair reruns the original acceptance command through a fresh parent-linked Ending after repair.
...
Every Ending-required user submission gets one bounded memory-relevance scan inside its mandatory Ending. An eligible low-risk single-result small non-code task records no Ending candidate. The scan remains separate from adaptive routing, check-worker assignment, and project-change memory.
...
- `skill_contract_defect`: fresh evidence shows the process contract itself is wrong; terminal FAIL with `action=isolated_repair` and create a separate Repair Task. Ending never edits the Skill.
- `execution_drift`: the process contract is correct but the producer did not follow it; terminal FAIL with `action=isolated_repair`. Never rewrite result memory to conceal the drift or steer the origin.
...
7. On missing evidence, timeout, or unavailable external state, record lifecycle `BLOCKED` and remain visible. Active-task ownership is `WAITING`, and never authorizes interruption.
8. A correctness failure or acceptance mismatch automatically creates a scoped independent projectless Repair Task with sanitized evidence. It waits for active write ownership to clear, runs Quick Check after repair, presents the corrected result, and starts a fresh independent Ending check. Repeat for at most three repairs.
...
- Never use a same-task subagent instead of the global projectless Ending. Bounded `ENDING_CHECK_WORKER` subagents are allowed only inside the real detached Ending and only for saved checks.
...
- Do not let the failing Ending verifier alter the result or contact the origin. Create the exact bounded independent projectless Repair Task, then require that task to start a fresh verifier after repair.
