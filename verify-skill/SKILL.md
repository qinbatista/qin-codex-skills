---
name: verify-skill
description: "Verification executor selected by workflow-skill. Use when work needs proof, tests, QA, review, UI/visual checks, artifact verification, regression comparison, or failure triage. Default foreground gate is one mini real test; route deeper result tests, logs, docs, and Obsidian sweeps to workflow's Ending Workflow subagent unless high-risk or the user asks to wait. Require concrete input/output evidence."
---
...
Return deeper real result testing to `workflow-skill` as `Ending Workflow` subagent scope when the update is major, changes user-visible behavior, touches broad/shared code, changes UI/generated artifacts, or when more confidence is useful but the extra checks do not change the delivered code/artifact. If an ending real test fails, reopen the task, report the failing evidence, repair the issue, and rerun the foreground mini test.
...
Post-pass deeper tests, log checks, DailyLog entries, wiki/log updates, Obsidian memory pages, Markdown summaries, optimization notes, report indexing, and similar records are secondary to the foreground pass verdict. Return the verdict and essential evidence to `workflow-skill`; `workflow-skill` owns the mandatory `Ending Workflow` subagent delegation. Do not decide that ending work is deferred, skipped, or not needed inside `verify-skill`. If no subagent tool is callable, report that condition back to `workflow-skill`.

When returning a foreground pass while ending work remains, say the task result is done by the mini-test gate and that extended checks or closeout must continue through `workflow-skill`'s `Ending Workflow` subagent. If any ending worker check fails, resume the task with the failing evidence instead of treating the earlier mini-test pass as final truth.

Model rule: verification judgment, comprehensive review, image/visual assessment, and final pass/fail decisions always use the best available verification/reasoning model by default. Any post-pass log/wiki/DailyLog/Obsidian/Markdown drafting or file editing is Spark-default execution and must use `GPT-5.3-Codex-Spark` (`gpt-5.3-codex-spark`) unless `workflow-skill` visibly switches that phase to the protected `GPT-5.5` light fallback (`gpt-5.5`, low effort) because Spark is unreachable or limited.

## Obsidian Regression Sweep

When verifying connected project work, repeated workflows, generated artifacts, UI/visual/Unity/image/browser/deployment work, or global skill behavior, treat available Obsidian project memory as a regression baseline. Return the full sweep to `workflow-skill` as `Ending Workflow` subagent scope by default after the foreground mini test passes unless the task is explicitly about a repeated failure, high-risk output, or user-requested full verification before return.
...
4. Do not pass full closeout if any applicable checklist item is untested, lacks evidence, or fails, even if ordinary tests, file existence, screenshots, metrics, or the main requested change pass. If the foreground mini test already returned to the user and the ending sweep finds a failure, reopen the task and report that the `Ending Workflow` regression check found an issue.
...
3. Run the foreground mini real test first unless the task is high-risk or the user asked to wait for full verification. Return the Obsidian Regression Sweep and deeper tests to `workflow-skill` as `Ending Workflow` subagent scope by default when they are relevant but not required to decide whether the immediate result works.
...
10. Report what passed in the foreground mini test, what ending checks remain, what failed if anything, what remains unverified, and where the evidence lives. After a foreground pass verdict, return the result immediately to `workflow-skill`; route extended testing and non-user-facing logging/wiki/Markdown closeout through `workflow-skill`'s mandatory `Ending Workflow` subagent.
...
Scale from the Default Mini Real Test Rule: one focused real check is the foreground completion gate. Broader real result testing goes to `workflow-skill` as `Ending Workflow` subagent scope by default after the user-facing result, except for high-risk, destructive, production/security/payment/data/API, explicitly user-blocking, or failing cases.
...
4. Run executable proof when code, scripts, browser flows, or automations are involved. The foreground proof must exercise the changed path with concrete inputs; broader regression comparisons go to `workflow-skill` as `Ending Workflow` subagent scope unless the task is high-risk or the user asks to wait.
5. Compare observed output to the target and baseline. Mark intentional differences as pass only when they are requested or necessary for the goal. If an ending comparison fails after the user-facing result was returned, reopen the task with the failing evidence.
...
- Do not block the user-facing result on background real tests, log checks, DailyLog, wiki, Obsidian, Markdown, optimization notes, or other secondary closeout after the foreground mini real test has passed. Return evidence to `workflow-skill` so it can delegate the mandatory `Ending Workflow` subagent.
