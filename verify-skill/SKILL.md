...
Verification has two scopes: user-requested verification as the task itself, and mandatory post-result Ending Real Verify. Every durable code change, plus standard/complex/advanced, medium/high-risk, and multi-stage work, requires one Ending; only eligible low-risk single-result small non-code work records `intentionally_skipped_simple_task`. Code producers run exactly one smallest local Quick Check and present immediately. Launch one persistent global projectless Ending only after the final aggregate producer receipt and result release; a child/subprocess receipt never launches it. Its Spark-xhigh controller runs deterministic checks directly and may route semantic runtime, integration, code-quality, prompt, UI, or visual checks to saved Terra/Sol `ENDING_CHECK_WORKER` nodes. All required checks must PASS with fresh evidence.
