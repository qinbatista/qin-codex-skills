...
Verification has two scopes: user-requested verification as the task itself, and post-result Ending Real Verify when the released result exposes `real_test`, `information_update`, or `memory_update`. A no-surface result records `intentionally_skipped_simple_task` with `ending_skip_reason=no_real_test_or_information_or_memory_update`, regardless of score, risk, or stage count. Code producers normally expose `real_test` through exactly one smallest local Quick Check and present immediately. Launch one persistent global projectless Ending only after the final aggregate producer receipt and result release; a child/subprocess receipt never launches it. Its Spark-xhigh controller runs deterministic checks directly and may route semantic runtime, integration, code-quality, prompt, UI, or visual checks to saved Terra/Sol `ENDING_CHECK_WORKER` nodes. All required checks must PASS with fresh evidence.
...
A same-task subagent may never substitute for the global projectless Ending. Inside the already-created detached Ending only, the Spark controller may launch bounded `ENDING_CHECK_WORKER` subagents with the exact saved pair, check, required Skills, and evidence path. They are not End/Fix tasks and cannot edit or repair. Only the final aggregate `ending-required` receipt gets one global Ending, acknowledged at `end_task_trigger_rate=100%`; the same task continues with compact, one-check actions instead of new Ending prompts. FAIL sends exact evidence to the immutable origin; only that source session repairs and starts a fresh Ending. Missing task creation, origin, or prompt submission is BLOCKED.
...
- The Ending primary and sole lifecycle controller is fixed to `gpt-5.3-codex-spark|xhigh`; explicit controller availability failure alone permits registry-floor `gpt-5.6-luna|low`. Check score and verification surface may select a Terra/Sol `ENDING_CHECK_WORKER`, but that never changes the controller pair or producer-learning boundary.
...
One-task disposable artifacts, including ordinary test scratch, use `tmp-<name>`. Use `<YYYYMMDD>` only for short reuse with a recorded reason and review point. Use `remote-<name>` or `remote-test/` only for explicitly retained material; reusable formal tests remain source tests.
...
When Cache content is reusable, retained, workflow-required, or project-influencing, add one concise registry entry to project-root `AGENTS.md`: the exact Cache-relative path, one-line structural role, owner/source of truth, and retention/version-control status. Link to the owning source or detailed README instead of embedding its commands, dependencies, runbook, or regeneration procedure. Update `AGENTS.md` only when project structure, ownership, a critical entry point, or a hard constraint changes. Important Cache without this concise pointer is incomplete; one-off disposable outputs need no entry. Never delete documented important Cache content without explicit authorization; other cleanup follows Task Resource Lifecycle and may delete only sealed exact paths below the current task's exclusive `Cache/tmp-*` root after all barriers. Final reports go only to the requested output location.
