...
10. Start the lifecycle ledger and bind the producer or aggregate graph receipt when present. A low-risk single-result small code task records `intentionally_skipped_simple_task`; all other code work is Ending-required. Capture the immutable origin session before launch, then build the smallest real-test plan with `verify-skill/scripts/ending_verification_plan.py` and create exactly one projectless `End Task-{task}` containing its explicit acceptance checks. Each check keeps a `0-100` score for scope and classification only. The Ending primary is fixed `gpt-5.3-codex-spark|xhigh`; explicit model, effort, scheduler, or required-modality unavailability permits only registry-floor `gpt-5.6-luna|low`. Correctness, quality, protocol, execution, timeout, or acceptance failure never changes the verifier. Worker prompts keep the originating project root as execution context and use project-root-relative paths. The one task runs its actual commands and semantic acceptance; all checks must PASS before one terminal memory/classification/record closeout. PASS/FAIL/BLOCKED End Tasks remain visible in the global task list; never auto-archive or delete them. A terminal Ending reports stage difficulty and pairs, attempt count, first/retry pass, suitability, next routing action/pair, and Obsidian model-record link/status. A FAIL or acceptance mismatch records exact evidence and submits the bounded repair prompt through `codex_app__send_message_to_thread` to the immutable origin session; that source session performs the authorized repair, runs Quick Check, presents a new result, and starts a fresh Spark-first projectless Ending with `--repair-of-lifecycle-id`, for up to three repair attempts. When thread tools, origin-session metadata, or prompt submission are unavailable, return terminal `BLOCKED` plus the exact handoff; never treat the lifecycle as verified, emulate task creation, wait, poll, self-verify, create a replacement repair session, or use a same-task Ending subagent.
...
The post-result Ending lifecycle runs for every Ending-required code task. Return published code after Quick Check, then release the single real-test Ending task. The origin does not poll it. A failing Ending verifier records exact evidence and submits the repair prompt to the immutable source session; the repaired code gets a fresh Spark-first verifier. The lifecycle always records score/band, check evidence, actual verifier pair and availability provenance, and repair chain locally and in Obsidian. Only a receipt-backed producer event may change routing; an unreceipted known Ending assignment is observation-only.
