...
10. Start the scored lifecycle ledger and bind the producer or aggregate graph receipt when present. End Task is conditional: use it only for complex work or an important result needing tests, records, or durable memory; simple reads and one-value edits skip it. Build the smallest real-test plan with `verify-skill/scripts/ending_verification_plan.py` and create one independently scored/modelled projectless `End Task-{task}-{check}` per independent acceptance check. Worker prompts keep the originating project root as execution context and use project-root-relative paths. Each task runs its actual command and all checks must PASS. PASS/FAIL/BLOCKED End Tasks remain visible in the global task list; never auto-archive or delete them. A terminal Ending reports stage difficulty and pairs, attempt count, first/retry pass, suitability, next routing action/pair, and Obsidian model-record link/status. A FAIL creates a project-bound `Fix Task-{task}-{check}` with the exact error, followed by a fresh projectless Ending check, for up to three repair attempts. When thread tools or required project repair resolution are unavailable, return terminal `BLOCKED` plus the exact handoffs; never treat the lifecycle as verified, emulate task creation, wait, poll, self-verify, or use a same-task Ending subagent.
...
The post-result Ending lifecycle runs when the result is complex or important and needs tests, records, or durable memory. Return published code after Quick Check, then release the real-test Ending tasks. The origin does not poll them. A failing Ending verifier records exact evidence and launches a separate repair task; the repaired code gets a fresh different verifier. The lifecycle always records score/band, check evidence, selected verifier pair, and repair chain locally and in Obsidian. Only a receipt-backed producer event may change routing; an unreceipted known assignment is observation-only.
