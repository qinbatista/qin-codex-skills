...
10. Start the scored lifecycle ledger and bind the producer or aggregate graph receipt when present. End Task is hard-required after every result. Build the smallest real-test plan with `verify-skill/scripts/ending_verification_plan.py`; resolve the exact saved project and create one independently scored/modelled project/local `End Task-{task}-{check}` per independent acceptance check. Worker prompts use project-root-relative paths. Each task runs its actual command and all checks must PASS. PASS/FAIL/BLOCKED tasks remain visible; never auto-archive or delete them. A terminal Ending reports stage difficulty and pairs, attempt count, first/retry pass, suitability, next routing action/pair, and Obsidian model-record link/status. A FAIL creates a project-bound `Fix Task-{task}-{check}` with the exact error, followed by a fresh Ending check, for up to three repair attempts. When thread tools or project resolution are unavailable, return terminal `BLOCKED` plus the exact handoffs; never treat the lifecycle as verified, emulate task creation, wait, poll, self-verify, or use a same-task Ending subagent.
