...
The post-result Ending lifecycle runs for every new user task. Return published code after Quick Check, then release the single real-test Ending task. The origin does not poll it. A failing Ending verifier records exact evidence and submits the repair prompt to the immutable source session; the repaired code gets a fresh Spark-first verifier. The lifecycle always records score/band, check evidence, actual verifier pair and availability provenance, and repair chain locally and in Obsidian. Only a receipt-backed producer event may change routing; an unreceipted known Ending assignment is observation-only.
