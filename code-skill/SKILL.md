...
10. Bind the final producer or aggregate receipt. Non-trivial code is a material update and requires durable project-memory closeout; only explicit `trivial_value_only` work with no other surface may record `intentionally_skipped_simple_task` and `ending_skip_reason=no_real_test_or_information_or_memory_or_material_update`. Use `ending_verification_plan.py` to create exactly one global projectless `End Task-{task}`, invoke only its generated `create_thread` request, and require `codex_app__list_threads` to prove `projectId=null`. Spark-xhigh is first controller. A real quota/five-hour/provider-rate/trusted retry limit is recorded once; regenerated launches skip the cooling model and choose the next stronger supported controller until expiry. Quality failure never creates cooldown. Plan-saved `ENDING_CHECK_WORKER` nodes read every listed Skill and never edit producer files. All checks must PASS. FAIL creates a fresh independent projectless Repair Task with no existing thread target. Repair never sends, steers, interrupts, terminates, hands off, moves, or mutates another session; it waits without messaging while an active task owns a required write surface, then repairs, runs Quick Check, and starts a fresh parent-linked Ending.
...
Post-result Ending runs for every observable or material code result. Return code after one Quick Check and do not poll. Spark is first, with restriction-aware stronger controller selection only during a persisted provider cooldown. A failing check creates an isolated Repair Task; repaired code gets a fresh parent-linked Ending. Only receipt-backed producer events change producer routing.
