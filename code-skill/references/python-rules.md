...
- Present `CODE READY` with Quick Check evidence, then build real proportional Ending checks. Create exactly one persistent projectless Ending for the new task and return without polling. Multiple unit, integration/API, and runtime checks stay inside that one Ending and retain their own scores. Every required check must PASS.
