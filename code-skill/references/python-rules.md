...
- Present `CODE READY` with Quick Check evidence. A low-risk single-result small task records `intentionally_skipped_simple_task` and returns without an Ending. All other code work is Ending-required: emit `ending-required`, then create and acknowledge exactly one persistent projectless Ending before finalizing. Multiple unit, integration/API, and runtime checks stay inside that one Ending and retain their own scores. Every required check must PASS.
