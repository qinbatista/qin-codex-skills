...
- Do not add fallback/default/compatibility branches, alternate input aliases, empty-value substitutes, or caller-side data repair unless the user explicitly asks for them or a real external API contract requires them.
- When the requested change is case A, write code for case A only. Do not add branches, comments, validators, or warnings for unrelated cases B/C/D unless the user named them or an existing contract requires them.
...
- Do not invent defaults for missing values unless the task specifically asks for fallback behavior.
- Do not mention or guard against unrequested sibling cases just to be safe.
