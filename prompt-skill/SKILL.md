...
11. Start the mandatory scored Ending lifecycle after every result. Create separate real representative-case checks with `ending_verification_plan.py` as projectless global End Tasks, each using its own score-derived model/effort; all required cases must PASS. Keep the originating project root only as execution and evidence context. Every terminal End Task stays visible and reports attempts, first/retry pass, model suitability, next pair/action, and its Obsidian model-record link/status.
