...
11. A low-risk single-result small task (`0-24`) records `intentionally_skipped_simple_task` and does not create an End Task. Standard/complex/advanced work, any medium/high-risk task, and multi-stage work are Ending-required: the producer emits `ending-required`, then the parent creates and acknowledges one scored projectless Ending lifecycle. Keep representative cases as distinct checks in one `ending_verification_plan.py` plan; all must PASS. Ending requests fixed `gpt-5.3-codex-spark|xhigh`, with registry-floor Luna-low allowed only for explicit model/effort/scheduler or required-modality unavailability; score scopes checks and never selects the verifier. Keep the originating project root only as execution and evidence context. A missing acknowledgement is BLOCKED. Every terminal End Task stays visible and reports attempts, first/retry pass, model suitability, next pair/action, and its Obsidian model-record link/status.
...
13. If a real prompt check fails or the final prompt behavior differs from the original request, record the exact case/output/error, submit the bounded repair prompt through `codex_app__send_message_to_thread` to the immutable origin session, then let that source session run Quick Check and start a fresh projectless verifier; allow up to three repair attempts. Missing infrastructure, origin-session metadata, failed prompt submission, or exhausted attempts is BLOCKED and is not verified. Never auto-archive or delete Ending tasks or repair handoffs.
