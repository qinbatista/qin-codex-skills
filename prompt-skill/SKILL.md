...
11. A prompt result emits `ending-required` only when it exposes `real_test`, `information_update`, or `memory_update`; prompt-in-code normally exposes `real_test` through its focused check. A low-risk single-result small task with no surface follows the same skip record: `intentionally_skipped_simple_task` with `ending_skip_reason=no_real_test_or_information_or_memory_update`, regardless of score, risk, or stage count. Present the prompt first, then create one global projectless Ending with exact `create_thread` target `{"type":"projectless"}` and `codex_app__list_threads` readback showing `projectId=null` or absent. The origin project is context only; project/current-task/same-task-subtask placement is BLOCKED. Spark-xhigh remains its controller at `gpt-5.3-codex-spark|xhigh`. Keep representative cases as distinct checks in one `ending_verification_plan.py` plan; plan-saved Terra/Sol `ENDING_CHECK_WORKER` nodes may load `prompt-skill`, run only the assigned semantic case, and write evidence without editing or repairing. All checks must PASS and all must PASS before closeout; Spark availability alone permits registry-floor Luna-low controller fallback.
...
13. If a real prompt check fails or the final prompt behavior differs from the original request, record the exact case/output/error, submit the bounded repair prompt through `codex_app__send_message_to_thread` to the immutable origin session, then let that source session run Quick Check and start a fresh global projectless verifier; allow up to three repair attempts. Missing infrastructure, origin-session metadata, failed prompt submission, or exhausted attempts is BLOCKED and is not verified. Never auto-archive or delete Ending tasks or repair handoffs.
...
Before the first Codex-selected project-support write, resolve the authoritative `<project-root>`, inspect `<project-root>/Cache/`, and choose the destination there. All agent-created disposable or supporting artifacts — prompt test scripts/results/fixtures, debug logs/data, intermediate code, image inspection downloads/renders, generated images, receipts, snapshots, comparisons, and probes — must live only under `<project-root>/Cache/`; redirect any proposed path there before writing. Requested durable project source changes and final deliverables remain in the project's declared source/output paths.
