...
11. Start the mandatory scored Ending lifecycle after every result. Resolve the exact saved project and create separate real representative-case checks with `ending_verification_plan.py` as project/local tasks, each using its own score-derived model/effort; all required cases must PASS. Every terminal task stays visible and reports attempts, first/retry pass, model suitability, next pair/action, and its Obsidian model-record link/status.
...
13. If a real prompt check fails, record the exact case/output/error, create a separate repair task in the same saved project, then run a fresh verifier; allow up to three repair attempts. Missing infrastructure, project resolution, or exhausted attempts is BLOCKED and is not verified. Never auto-archive or delete End/Fix tasks.
