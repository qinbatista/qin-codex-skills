...
13. If a real prompt check fails, record the exact case/output/error, create a separate repair task in the same saved project, then run a fresh projectless verifier; allow up to three repair attempts. Missing infrastructure, required repair-project resolution, or exhausted attempts is BLOCKED and is not verified. Never auto-archive or delete End/Fix tasks.
