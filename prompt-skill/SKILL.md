...
11. Start one scored projectless Ending lifecycle for every new user task; simple reads and one-value edits use the smallest observable completion/record check. Keep representative cases as distinct checks in one `ending_verification_plan.py` plan; all must PASS. Ending requests fixed `gpt-5.3-codex-spark|xhigh`, with registry-floor Luna-low allowed only for explicit model/effort/scheduler or required-modality unavailability; score scopes checks and never selects the verifier. Keep the originating project root only as execution and evidence context. Every terminal End Task stays visible and reports attempts, first/retry pass, model suitability, next pair/action, and its Obsidian model-record link/status.
