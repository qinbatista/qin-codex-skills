...
|---|---|---|
| Management | `2d-bone-animation` | Prepare and guide 2D character bone rigging, Sprite Skin binding, and Animator/Animation Clip authoring for the AIAnimation2D Unity project. |
| General | `aivfx2d-vfx-gallery` | Open, rebuild, inspect, or summarize the local AIVFX2D VFX gallery. Use when the user asks to view all AIVFX2D VFX, preview imported VFX, open the gallery webpage, rebuild the VFX gallery index/contact sheet, or find where AIVFX2D VFX preview images and manifests are stored. |
