...
| open-chrome | Open an always-loaded browser profile. | task-analyze-skill -> workflow-skill -> direct tool action | Entry is current selection only; Luna for observable state checks. | Chrome window opens. | No-op records only. |
| open-youtube | Open YouTube directly without a dispatch graph. | task-analyze-skill -> workflow-skill -> direct tool action | Entry is current selection only; Luna for minimal observable checks. | `youtube.com` loaded. | No-op records only. |
| search-cctv-on-youtube | One bounded search interaction with query visible in results. | task-analyze-skill -> workflow-skill -> direct tool action | Entry is current selection only; Luna for bounded interaction and check. | Query and visible results are present. | No-op records only. |
