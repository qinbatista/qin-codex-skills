...
| skill-edit | User asks to create, merge, rename, delete, reorganize, or update global Codex skills. | Skill names, frontmatter, trigger behavior, references/scripts, old-name cleanup, validation, and GitHub sync target are explicit. | workflow-skill -> github-sync -> code-skill -> test-skill -> verify-skill -> github-sync | Skill files, validation commands, generated overview/report, sync status, remote hash, and pass reason. |
...
| github-sync | User asks to sync, commit, push, inspect, or publish global skills to GitHub. | Repository, mirror scope, public-safety requirements, local/remote state, and expected final hash are explicit. | workflow-skill -> github-sync -> verify-skill | Status output, safety scan, pushed commit hash or no-diff result, and pass reason. |
| codex-switch | User asks to inspect or switch local Codex auth profiles. | Requested profile action, local-only scope, confirmation requirement, and privacy constraints are explicit. | workflow-skill -> codex-switch -> verify-skill | Local profile command output or file state, no token disclosure, and pass reason. |
...
- For global skill work, run `github-sync` before edits and after successful verification when the user asks to push.
