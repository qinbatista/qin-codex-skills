...
Eligible single-node text/code production enters the compact adaptive runner exactly once. Multi-segment work creates one dynamically scored dependency graph: each node chooses its own pair, and eligible small code/write/execute nodes use Spark regardless of the parent score. Tool-only, image/mixed, and exact one-source read-only work stay inline. Verification belongs to separate post-result Ending tasks.

| Scenario | Use when | Foreground route | Model pattern | First presentation | Ending Task |
|---|---|---|---|---|---|
| ordinary-production | One eligible text/code result, edit, or implementation. | entry bootstrap -> adaptive producer -> result | Saved contextual quality pair; one stronger fallback only after zero result. | For code, run one bounded Quick Check; then present. | Create `End Task-{related task}` and return without waiting. |
...
| ordinary-code-domain | Implement or edit an active registry-owned code domain. | adaptive producer -> code-skill -> Quick Check -> result | Small low-risk edit tries Spark; otherwise contextual quality pair. | Present changed paths, behavior, and Quick Check. | Separate scored real-test tasks; all checks must PASS. |
| dynamic-code-graph | Distinct bounded design, implementation, execution, and integration segments need different model strengths. | Task Analyze -> scored dependency-ready nodes -> final owner -> result | Each node uses its own score; eligible 0-24 code/write/execute nodes use Spark even under a complex parent. | Present the transitive main result after producer Quick Checks. | Separate score-routed Ending checks. |
...
- Spark is first priority for eligible small low-risk edits and remains available for admitted schedule-source branches; a matching quality failure suppresses it and upgrades the next task.
- Dynamic fan-out requires distinct bounded ownership, per-node scores/pairs, and safe dependency-ready concurrency; read-only source fan-out additionally requires pre-read cost admission.
