...
REQUIRED_WORKFLOW = [
    ...
    "For a complex or important result that needs tests, records, or durable memory",
...
NEGATIVE_AGENT_PREFIXES = {"code_agent": "$code-skill: artifact-free exact read-only lookup/audit stays skill-free", "verify_agent": "$verify-skill: eligible complex/important results launch an independent post-result Ending", "optimization_agent": "$optimization-skill: do not load from benchmark repetition alone or for exact-scoped read-only work", "management_agent": "$management-skill: do not load for ordinary exact-scoped read-only work or benchmark worker arms"}
...
def validate(skill_dir):
    ...
    failures.extend(missing_terms("code-skill agent", texts["code_agent"], ["artifact-free exact read-only lookup/audit stays skill-free", "requested Cache/report output requires the runner", "Score every submission 0-100", "small low-risk edit tries Spark-low", "quality failure suppresses the matching band", "Complex/important results needing tests, records, or durable memory", "ending_verification_plan.py", "projectless End Task", "exact project binding remains execution context", "all must PASS and remain visible", "Never auto-archive/delete", "difficulty/pairs", "attempts", "first/retry pass", "suitability", "Obsidian link", "codex_app__send_message_to_thread", "source session", "fresh projectless verification", "never self-verify", "same-task subagent"]))
    failures.extend(missing_terms("verify-skill agent", texts["verify_agent"], ["eligible complex/important results launch an independent post-result Ending", "simple reads, file opens, and one-value edits skip it", "ending_verification_plan.py", "projectless End Task", "preserve exact project binding as execution context", "Score each verifier 0-100", "all checks PASS", "Every terminal task stays visible", "never auto-archive/delete", "attempt", "first/retry", "suitability", "Obsidian record link/status", "Without receipt, record a non-learning assignment observation", "records exact command/output/error", "codex_app__send_message_to_thread", "immutable origin thread/session", "fresh projectless verifier", "up to three", "BLOCKED is not verified", "Never expose chain-of-thought", "same-task subagent"]))
    ...
    failures.extend(missing_terms("management-skill agent", texts["management_agent"], ["do not load for ordinary exact-scoped read-only work or benchmark worker arms", "explicit management request or admitted node", "native Obsidian Model Switch/category/shared-category links", "local model_experience.json and central TaskModelExperience are legacy read-only", "Every task stores a 0-100 score", "Small low-risk edits try Spark", "matching quality failure suppresses that band", "two Real PASS results", "quality FAIL", "Ending real checks record local evidence", "receipt-backed producer verdicts", "Never publish private state"]))
