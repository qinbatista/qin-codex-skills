...
REQUIRED_ENTRY = ["# Task Lifecycle", "Merge this section into `~/.codex/AGENTS.md`", "Score every submission 0-100", "obsidian_adaptive_model_runner.py", "before skills/memory/files", "task_complexity_score.py", "Spark-low first", "2 Real PASS down 1 rung", "quality FAIL up 1", "ending_verification_plan.py", "one own scored/model End Task per independent real test/API/render", "all checks must PASS", "FAIL creates Fix Task with exact error then fresh End Task", "up to 3 repairs", "BLOCKED only unavailable/external/limit", "Terminal events sync local history+Obsidian Model Switch", "`gpt-5.6-sol|ultra`", "Direct fixed/no verify", "Auto receipt=child/graph", "task vs task+Ending", "controller excluded", "No hook", "Final PASS/BLOCKED Ending-only"]
...
def validate(skill_dir):
    ...
    failures.extend(missing_terms("workflow agent", texts["agent"], ["eligible small low-risk edits try Spark", "matching Obsidian context", "saved highest-family quality pair", "One Real PASS keeps it", "two PASS downgrade one rung", "quality FAIL upgrades one", "operational failure may use one stronger fallback", "Publish output after Quick Check", "ending_verification_plan.py", "one scored/modelled persistent task per independent real check", "all must PASS", "FAIL records exact evidence", "separate repair task", "fresh verification", "up to three attempts", "BLOCKED is not verified", "Local lifecycle", "receipt-backed Obsidian learning", "Never self-verify", "same-task subagent"]))
    ...
    failures.extend(missing_terms("code-skill agent", texts["code_agent"], ["exact read-only lookup/audit stays skill-free", "Score every submission 0-100", "small low-risk edit tries Spark-low", "quality failure suppresses the matching band", "End Task is hard-required", "ending_verification_plan.py", "independent real test", "all must PASS", "separate repair task", "fresh verification", "BLOCKED is not verified", "never self-verify", "same-task subagent"]))
    failures.extend(missing_terms("verify-skill agent", texts["verify_agent"], ["every user task launches an independent post-result Ending lifecycle", "ending_verification_plan.py", "independent real unit/integration/API/build/render/state check", "own 0-100 score", "all required checks must PASS", "Never summarize prior evidence", "FAIL records exact command/output/error", "separate repair task", "fresh verifier", "up to three attempts", "BLOCKED never counts as verified", "Never let a verifier repair its own target", "same-task subagent"]))
