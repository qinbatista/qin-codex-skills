...
REQUIRED_WORKFLOW = [
    ...
    "fresh Ending task",
    "Real Verify",
    "one persistent `End Task-{task}-{check}` per independent real unit/integration/API/build/render/state check",
    ...
    "create-launches",
    "acknowledge each returned `threadId`, `hostId`, and project ID",
...
REQUIRED_MATRIX = ["# Adaptive And Admitted Routing Matrix", "Eligible single-node text/code production enters the compact adaptive runner exactly once", "Multi-segment work creates one dynamically scored dependency graph", "dynamic-code-graph", "ordinary-production", "independent-small-sources", "independent-large-sources", "dependent-multi-file", "explicit-routing-no-graph", "explicit-benchmark", "admitted-complex", "One receipt-valid Real PASS retains the selected pair", "two matched PASS outcomes try one lower rung", "quality failure upgrades one rung", "eligible 0-24 code/write/execute nodes use Spark", "all checks must PASS", "codex_app__send_message_to_thread", "fresh verifier", "Direct uses the exact raw prompt", "public two-world comparison", "fixed Sol-ultra Direct task", "Luna-max-entry Auto task", "Auto excludes only its Luna-max entry controller", "--direct-task", "--bootstrap-task"]
REQUIRED_CODE = ["score role or Obsidian-context quality pair", "bounded native Obsidian route", "project/task/module/file/symbol/code", "One Real PASS retains a quality pair", "two PASS results downgrade one rung", "quality FAIL upgrades one rung", "small band tries Spark first", "zero-result, zero-token Spark operational failure", "Old local `model_experience.json` remains legacy read-only", "requested output artifact is producer work", "Cache note or report is an artifact", "End Task is mandatory for every new user task", "ending_verification_plan.py", "projectless `End Task-{task}-{check}`", "All checks must PASS", "remain visible", "never auto-archive or delete", "immutable origin session", "codex_app__send_message_to_thread", "fresh projectless Ending check", "unreceipted known assignment is observation-only", "same-task Ending subagent", "different `verify-skill` worker performs independent verification"]
REQUIRED_VERIFY = ["Verification has two scopes", "post-result Ending Real Verify", "execute a real proportional test", "ending_verification_plan.py", "own `0-100` check score", "All required checks must PASS", "{\"type\":\"projectless\"", "create-launches", "ack-launch", "end_task_trigger_rate=100%", "Model stages (N):", "remain visible", "Never call `set_thread_archived`", "exact saved project", "structured `model_assessment`", "learning_eligible=false", "fresh projectless Ending", "BLOCKED does not count as verified"]
...
def validate(skill_dir):
    ...
    failures.extend(missing_terms("workflow agent", texts["agent"], ["eligible small low-risk edits try Spark", "matching Obsidian context", "saved highest-family quality pair", "One receipt-backed Real PASS keeps it", "two PASS trial down", "quality FAIL upgrades", "operational failure is neutral", "LOCKED_ROUTE_NODE publishes result/Quick Check only", "after its passing receipt the entry parent captures the immutable origin session", "launches ending_verification_plan.py", "preserves exact project binding", "projectless End Task", "All PASS and remain visible", "never auto-archive/delete", "difficulty/pairs", "attempts", "first/retry", "suitability", "Obsidian link", "no receipt writes a non-learning observation", "codex_app__send_message_to_thread", "source session", "fresh projectless verification", "up to three", "Never expose private reasoning", "self-verify", "same-task subagent"]))
    ...
    failures.extend(missing_terms("code-skill agent", texts["code_agent"], ["artifact-free exact read-only lookup/audit stays skill-free", "requested Cache/report output requires the runner", "Score every submission 0-100", "small low-risk edit tries Spark-low", "quality failure suppresses the matching band", "Every new user task captures immutable origin session", "smallest completion/record check", "ending_verification_plan.py", "projectless End Task", "exact project binding remains execution context", "all must PASS and remain visible", "Never auto-archive/delete", "difficulty/pairs", "attempts", "first/retry pass", "suitability", "Obsidian link", "codex_app__send_message_to_thread", "source session", "fresh projectless verification", "never self-verify", "same-task subagent"]))
    failures.extend(missing_terms("verify-skill agent", texts["verify_agent"], ["every new user task launches an independent post-result Ending", "reads, file opens, and one-value edits use a smallest completion/record check", "ending_verification_plan.py", "projectless End Task", "preserve exact project binding as execution context", "Score each verifier 0-100", "all checks PASS", "Every terminal task stays visible", "never auto-archive/delete", "attempt", "first/retry", "suitability", "Obsidian record link/status", "Without receipt, record a non-learning assignment observation", "records exact command/output/error", "codex_app__send_message_to_thread", "immutable origin thread/session", "fresh projectless verifier", "up to three", "BLOCKED is not verified", "Never expose chain-of-thought", "same-task subagent"]))
    failures.extend(missing_terms("optimization-skill agent", texts["optimization_agent"], ["do not load from benchmark repetition alone", "user-requested optimization", "explicitly authorized reusable improvement", "positively admitted node", "catalog-generated ladder", "present the optimized result immediately", "no foreground Mini/Fast Verify", "visible projectless Ending for every new optimization task", "smallest completion/record check", "different Real verifier", "Ending time is excluded from first-result time", "never self-certify savings"]))
    failures.extend(missing_terms("management-skill agent", texts["management_agent"], ["do not load for ordinary exact-scoped read-only work or benchmark worker arms", "explicit management request or admitted node", "native Obsidian Model Switch/category/shared-category links", "local model_experience.json and central TaskModelExperience are legacy read-only", "Every task stores a 0-100 score", "Small low-risk edits try Spark", "matching quality failure suppresses that band", "two Real PASS results", "quality FAIL", "Every new management task launches a visible projectless Ending", "terminal memory/classification/record closeout", "real checks", "receipt-backed producer verdicts", "Never publish private state"]))
