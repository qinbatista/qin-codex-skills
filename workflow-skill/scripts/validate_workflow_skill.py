...
REQUIRED_ENTRY = ["# Task Lifecycle", "Merge this section into `~/.codex/AGENTS.md`", "Score every submission 0-100", "Resolve entry first", "Sol/high may downgrade", "Luna-max/lower may upgrade", "matching type/step/band history wins across roots", "cold start stays <=entry", "reuse frozen lowest-correct", "Split distinct work only", "each result/Ending records score,band,pair,purpose,deps,stop", "parent score never fixes the project model", "Spark-low first", "Dependency-ready independent nodes run in parallel", "shared writes/order/output deps stay linear", "obsidian_adaptive_model_runner.py", "dynamic_task_graph", "task_route_dispatcher.py run-plan", "task_complexity_score.py", "2 Real PASS down 1 rung", "quality FAIL up 1", "global projectless End/Fix Tasks", "all checks PASS", "PASS records then self-archives", "fresh End Task", "up to 3 repairs", "BLOCKED only unavailable/external/limit", "Terminal receipt events write local routing history first", "same event ID to Obsidian", "AUTO_BENCHMARK_ENTRY", "Direct `gpt-5.6-sol|ultra`", "Auto `gpt-5.6-luna|max`", "MUST child/graph then return child JSON", "gate owns separate Ending", "excludes only controller", "compare task vs +Ending", "all exact PASS", "No hook", "Final PASS/BLOCKED Ending-only"]
