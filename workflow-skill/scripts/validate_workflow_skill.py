...
REQUIRED_WORKFLOW = [
    ...
    "A released result uses independent Ending after presentation when it exposes `real_test`, `information_update`, or `memory_update`",
    "ending_skip_reason=no_real_test_or_information_or_memory_or_material_update",
    ...
    "execute `repair_launch` as a new projectless session",
    "next fresh Ending",
    "Real Verify",
    "one persistent global projectless `End Task-{task}`",
    "Spark-xhigh is first priority",
    "record the restriction once",
    "use the next stronger controller",
    "never retry a cooling model",
    "fresh independent global projectless Repair Task",
    "waits without messaging",
...
REQUIRED_MATRIX = ["# Adaptive And Admitted Routing Matrix", "Eligible single-node text/code production enters the compact adaptive runner exactly once", "Multi-segment work creates one dynamically scored dependency graph", "dynamic-code-graph", "ordinary-production", "independent-small-sources", "independent-large-sources", "dependent-multi-file", "explicit-routing-no-graph", "explicit-benchmark", "admitted-complex", "One receipt-valid Real PASS retains the selected pair", "two matched PASS outcomes try one lower rung", "quality failure upgrades one rung", "Same-session outcome gate; then a small low-risk edit tries Spark only with no stronger session route", "all checks must PASS", "persisted provider cooldown", "fresh independent projectless Repair Task", "never messages, steers, interrupts, terminates, hands off, moves, or mutates an existing session", "waits without messaging", "Direct uses the exact raw prompt", "public two-world comparison", "fixed Sol-ultra Direct task", "Luna-max-entry Auto task", "Auto excludes only its Luna-max entry controller", "--direct-task", "--bootstrap-task"]
REQUIRED_CODE = ["score role or Obsidian-context quality pair", "bounded native Obsidian route", "project/task/module/file/symbol/code", "One Real PASS retains a quality pair", "two PASS results downgrade one rung", "quality FAIL upgrades one rung", "requested output artifact is producer work", "run exactly one producer-side Quick Check", "Present `CODE READY`", "Non-trivial code is a material update", "ending_skip_reason=no_real_test_or_information_or_memory_or_material_update", "ending_verification_plan.py", "exactly one global projectless `End Task-{task}`", "codex_app__list_threads", "projectId=null", "Spark-xhigh is first controller", "next stronger supported controller", "ENDING_CHECK_WORKER", "read every listed Skill", "never edit producer files", "All checks must PASS", "fresh independent projectless Repair Task", "never sends, steers, interrupts, terminates, hands off, moves, or mutates another session", "waits without messaging", "fresh parent-linked Ending", "do not poll"]
REQUIRED_VERIFY = ["Verification has two scopes", "post-result Ending Real Verify when the released result exposes", "material structural, non-trivial code, conceptual, or process update", "persistent global projectless", "ending_verification_plan.py", "final aggregate producer receipt", "child/subprocess receipt never launches it", "compact, one-check actions", "each check keeps its own `0-100` score", "All required checks must PASS", "{\"type\":\"projectless\"", "codex_app__list_threads", "projectId=null", "--thread-scope global --thread-project-id null", "create-launches", "record-controller-restriction", "next stronger supported controller", "Model stages (N):", "remains visible", "Never call `set_thread_archived`", "origin project binding", "structured `model_assessment`", "learning_eligible=false", "fresh global projectless Ending", "gpt-5.3-codex-spark|xhigh", "fresh projectless Repair Task", "codex_app__create_thread", "ENDING_CHECK_WORKER", "delegated_check_worker", "BLOCKED does not count as verified"]
REQUIRED_OPTIMIZATION = ["Do not infer optimization from repeated benchmark arms or exact-scoped read-only work", "Use this skill directly only when the user requests optimization", "catalog-derived adaptive producer", "positively admitted", "non-trivial code or process optimization is material", "durable project-memory closeout", "ending_skip_reason=no_real_test_or_information_or_memory_or_material_update", "exactly one smallest local Quick Check", "Spark-xhigh is first", "next stronger supported controller", "fresh independent projectless Repair Task", "waits without messaging", "ENDING_CHECK_WORKER", "different verifier", "before/after", "producer receipt", "--direct-task", "--bootstrap-task", "neither arm enters Task Analyze context"]
REQUIRED_MANAGEMENT = ["Do not load this skill for ordinary exact-scoped read-only work or Direct/Global benchmark worker arms", "directly only for an explicit routing-record", "admitted a delegated route", "native Obsidian category page", "Every task stores a `0-100` score", "Eligible small low-risk edits with no stronger same-session outcome route try Spark first", "Spark quality failure suppresses", "two PASS results trial one rung down", "obsidian_model_memory.py", "observable surfaces or material structural/non-trivial-code/conceptual/process updates", "durable project-memory closeout", "Spark-xhigh is first controller", "durable restriction-aware stronger escalation", "fresh independent projectless Repair Task", "terminal memory/classification/record closeout", "Terra/Sol check workers", "terminal ledger event records an Ending result automatically", "legacy read-only", "Model stages (N):", "end_task_trigger_rate=100%", "explicit install/update, skip inspect/gate/compare", "Legacy `sync` is an install-only remote replacement alias", "Never push/publish unless the user explicitly requested publishing"]
REQUIRED_ENTRY = [
    "# Task Lifecycle", "This template is written only by the explicit `install-global-agents` command", "Score every submission 0-100", "Resolve entry first", "Sol/high may downgrade", "Luna-max/lower may upgrade", "exact step-capability/band history wins", "no match stays <=entry", "reuse lowest-correct", "classify material stages before any producer", "2+ result stages activate one graph", "single-stage only", "each stage records score,band,pair,purpose,deps,stop", "same-session gate→Spark-low if no stronger route",
    "Execution lifecycle:", "ultra-simple single result=>direct", "other single result=>plan+single execution", "multi-result=>plan+dependency graph", "emit execution-lifecycle-notice", "operational failure is quality-neutral", "new topic resets", "Independent nodes parallel", "shared writes/order/output deps linear", "obsidian_adaptive_model_runner.py", "graph-route-required", "route-required", "no model ran", "full Task Analyze", "no single-producer retry", "LOCKED_ROUTE_NODE emits Code Gate", "universal philosophy+one active language profile+matched categories", "missing Code Gate/reference is routing failure", "C# aliases use only Unity C#", "does code/result+one Quick Check,emits CODE READY,stops",
    "Parent launches one End only after final aggregate `ending-required` receipt/release", "child/subprocess receipts never launch it", "ENDING_TASK_WORKER controller is Spark-xhigh first", "record restricted_at+retry_at/cooldown_until", "skip cooling model,next stronger supported controller", "expiry=>Spark first", "never loop cooling model", "quality FAIL creates no cooldown", "same End Task continues one bounded action at a time", "ENDING_CHECK_WORKER reads listed Skills", "dynamic_task_graph", "task_route_dispatcher.py run-plan", "task_complexity_score.py", "no reread/full-read/precheck", "output artifact MUST runner", "2 Real PASS trial down 1 rung", "quality FAIL up 1",
    "Only the final aggregate `ending-required` or `ending_required=true` receipt after all result nodes settle=>one independent global projectless Ending", "material=>durable project-memory closeout", "create-launches --producer-receipt(final) --project-id(context-only)", "`intentionally_skipped_simple_task`=>no End Task", "ending_skip_reason=no_real_test_or_information_or_memory_or_material_update", "missing acknowledgement=>BLOCKED", "gpt-5.3-codex-spark|xhigh", "score scopes checks only", "terminal memory/classification/record closeout", "call codex_app__create_thread target", "codex_app__list_threads", "projectId=null/absent", "resolve root with codex_app__list_projects", "ack threadId+hostId+originProjectId+threadProjectId=null+actual pair", "project/current-task/same-task-subtask placement", "audit-launches end_task_trigger_rate=100%", "all checks PASS", "End Task stays globally visible", "never auto-archive/delete",
    "fresh independent global projectless Repair Task", "immutable origin is context only", "never send/steer/interrupt/terminate/handoff/move/mutate any existing task/session", "active task owns required write surface", "waits without messaging/interruption", "fresh global projectless End Task with --repair-of-lifecycle-id", "up to 3 repairs", "BLOCKED only unavailable/external/limit,not active-task waiting", "never repair in verifier", "Terminal events write local+same-ID Obsidian records", "only producer receipts move routing", "AUTO_BENCHMARK_ENTRY", "Direct `gpt-5.6-sol|ultra`", "Auto `gpt-5.6-luna|max`", "MUST child/graph then return child JSON", "gate owns separate Ending", "excludes only controller", "compare task vs +Ending", "all exact PASS", "Final Ending PASS/BLOCKED", "lock+fsync recovery", "materialize+backup only→replace all managed Skills",
]
...
def validate(skill_dir):
    ...
    failures.extend(missing_terms("code-skill agent", texts["code_agent"], ["$code-skill", "Code Gate", "universal philosophy+one language profile", "one Quick Check", "CODE READY", "Non-trivial code is material", "global projectless Ending+durable memory", "trivial_value_only", "Spark-xhigh first", "provider quota cooldown persisted", "cooling model skipped", "next stronger controller", "fresh independent projectless Repair", "never contact/interrupt/steer/mutate existing sessions", "active write conflict waits", "fresh parent-linked Ending", "Workers evidence-only", "all PASS", "never self-verify"]));
    failures.extend(missing_terms("verify-skill agent", texts["verify_agent"], ["$verify-skill", "real_test|information_update|memory_update", "material structural/non-trivial-code/conceptual/process update", "durable memory closeout", "trivial_value_only", "global projectless Ending", "list_threads projectId=null/absent", "Spark-xhigh first", "quota/5-hour/provider limit", "record cooldown timestamps", "skip cooling model", "next stronger controller", "quality failure never cooldown", "fresh independent projectless Repair Task", "never send/steer/interrupt/terminate/handoff/move/mutate existing sessions", "active write conflict=>wait without messaging", "fresh parent-linked Ending<=3", "Check workers evidence-only", "all PASS", "tasks stay visible"]));
    failures.extend(missing_terms("optimization-skill agent", texts["optimization_agent"], ["$optimization-skill", "requested optimization/authorized reusable improvement/admitted node only", "Preserve behavior", "one Quick Check", "CODE READY", "Non-trivial code/process optimization is material", "global projectless Ending+durable memory", "trivial_value_only", "Spark-xhigh first", "quota/5-hour/provider cooldown", "skip cooling model", "next stronger controller", "fresh independent projectless Repair", "never contact/steer/interrupt/terminate/handoff/move/mutate existing sessions", "active write conflict waits", "fresh parent-linked Ending", "Workers evidence-only", "all PASS", "never self-certify savings"]));
    failures.extend(missing_terms("management-skill agent", texts["management_agent"], ["$management-skill", "explicit request/admitted node only", "Native Obsidian+local receipts", "score 0-100", "quality evidence moves producer routes", "Observable/material management updates", "global projectless Ending", "material=>durable memory", "trivial_value_only", "Spark-xhigh first", "quota/5-hour/provider cooldown", "skip cooling model", "next stronger controller", "fresh independent projectless Repair", "never contact/steer/interrupt/terminate/handoff/move/mutate existing sessions", "active write conflict waits", "fresh parent-linked Ending", "Consumer install/update=lock+fsynced recovery+opaque backup+exact-target replace only", "maintainer push runs one full gate before mutation", "source/deployed/remote separate", "never publish private state"]));
