...
REQUIRED_WORKFLOW = [
    ...
    "every new task uses the independent Ending lifecycle after presentation",
...
REQUIRED_OPTIMIZATION = ["Do not infer optimization from repeated benchmark arms or exact-scoped read-only work", "Use this skill directly only when the user requests optimization", "catalog-derived adaptive producer", "positively admitted", "Every new optimization task launches Ending", "smallest completion/record check", "different verifier", "before/after", "producer receipt", "--direct-task", "--bootstrap-task", "neither arm enters Task Analyze context"]
REQUIRED_MANAGEMENT = ["Do not load this skill for ordinary exact-scoped read-only work or Direct/Global benchmark worker arms", "directly only for an explicit routing-record", "admitted a delegated route", "native Obsidian category page", "Every task stores a `0-100` score", "Eligible small low-risk edits try Spark first", "Spark quality failure suppresses", "two PASS results trial one rung down", "obsidian_model_memory.py", "Ending Real starts with the producer receipt", "Every new management task launches a visible projectless Ending Task", "terminal memory/classification/record closeout", "terminal ledger event records the result automatically", "legacy read-only", "Model stages (N):", "end_task_trigger_rate=100%", "Never push/sync/publish unless the user explicitly requested publishing"]
REQUIRED_ENTRY = ["# Task Lifecycle", "Merge this section into `~/.codex/AGENTS.md` and `~/AGENTS.md`", "Score every submission 0-100", "Resolve entry first", "Sol/high may downgrade", "Luna-max/lower may upgrade", "exact step-capability/band history wins", "no match stays <=entry", "reuse recovered/lowest-correct", "2+ material result stages auto-activate one graph", "each result/Ending records score,band,pair,purpose,deps,stop", "executes Spark-low first", "Dependency-ready independent nodes run parallel", "shared writes/order/output deps stay linear", "obsidian_adaptive_model_runner.py", "LOCKED_ROUTE_NODE does result/Quick Check only", "parent launches one End after every new task receipt", "ENDING_TASK_WORKER does assigned checks only:no nested lifecycle", "dynamic_task_graph", "task_route_dispatcher.py run-plan", "task_complexity_score.py", "no reread/full-read/precheck", "output artifact MUST runner", "2 Real PASS trial down 1 rung", "quality FAIL up 1", "Every new task=>projectless End", "gpt-5.3-codex-spark|xhigh", "score scopes checks only", "registry-floor Luna-low", "terminal memory/classification/record closeout", "codex_app__create_thread projectless", "resolve root with codex_app__list_projects", "ending_verification_plan.py create-launches --project-id", "ack threadId+hostId+projectId+actual pair", "audit-launches end_task_trigger_rate=100%", "handoff pending", "all checks PASS", "End Task stays globally visible", "never auto-archive/delete", "attempts,first/retry pass", "suitability", "Obsidian link/status", "immutable origin session", "codex_app__send_message_to_thread", "repair_prompt", "fresh projectless End Task", "--repair-of-lifecycle-id", "up to 3 repairs", "missing source session or failed prompt submission is BLOCKED", "BLOCKED only unavailable/external/limit", "Terminal events write local+same-ID Obsidian records", "only receipts move routing", "AUTO_BENCHMARK_ENTRY", "Direct `gpt-5.6-sol|ultra`", "Auto `gpt-5.6-luna|max`", "MUST child/graph then return child JSON", "gate owns separate Ending", "excludes only controller", "compare task vs +Ending", "all exact PASS", "Final Ending PASS/BLOCKED"]
...
NEGATIVE_AGENT_PREFIXES = {"code_agent": "$code-skill: artifact-free exact read-only lookup/audit stays skill-free", "verify_agent": "$verify-skill: every new task launches one independent projectless Ending after result", "optimization_agent": "$optimization-skill: do not load from benchmark repetition alone or exact-scoped read-only work", "management_agent": "$management-skill: do not load for ordinary exact-scoped read-only work or benchmark workers"}
