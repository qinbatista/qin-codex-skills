...
REQUIRED_WORKFLOW = [
    ...
    "create-launches",
    "acknowledge each returned `threadId` and `hostId`",
...
REQUIRED_VERIFY = ["Verification has two scopes", "post-result Ending Real Verify", "execute a real proportional test", "ending_verification_plan.py", "own `0-100` check score", "All required checks must PASS", "{\"type\":\"projectless\"}", "create-launches", "ack-launch", "end_task_trigger_rate=100%", "Model stages (N):", "set_thread_archived", "FAIL and BLOCKED workers remain unarchived", "create `Fix Task-{task}-{check}`", "fresh Ending task", "BLOCKED does not count as verified"]
...
REQUIRED_ENTRY = ["# Task Lifecycle", "Merge this section into `~/.codex/AGENTS.md`", "Score every submission 0-100", "Resolve entry first", "Sol/high may downgrade", "Luna-max/lower may upgrade", "exact step-capability/band history wins across roots", "no match stays <=entry", "reuse recovered/lowest-correct", "Split distinct work only", "2+ material result stages auto-activate one graph", "each result/Ending records score,band,pair,purpose,deps,stop", "parent score never fixes the project model", "Spark-low first", "Dependency-ready independent nodes run in parallel", "shared writes/order/output deps stay linear", "obsidian_adaptive_model_runner.py", "dynamic_task_graph", "task_route_dispatcher.py run-plan", "task_complexity_score.py", "2 Real PASS down 1 rung", "quality FAIL up 1", "ending_verification_plan.py create-launches", "codex_app__create_thread", "ack each threadId+hostId", "audit-launches end_task_trigger_rate=100%", "handoff alone is pending", "global projectless End/Fix Tasks", "all checks PASS", "PASS records then self-archives", "fresh End Task", "up to 3 repairs", "BLOCKED only unavailable/external/limit", "Terminal receipts write local history first", "same event ID to Obsidian", "AUTO_BENCHMARK_ENTRY", "Direct `gpt-5.6-sol|ultra`", "Auto `gpt-5.6-luna|max`", "MUST child/graph then return child JSON", "gate owns separate Ending", "excludes only controller", "compare task vs +Ending", "all exact PASS", "No hook", "Final PASS/BLOCKED Ending-only"]
...
def validate(skill_dir):
    ...
    failures.extend(missing_terms("workflow agent", texts["agent"], ["eligible small low-risk edits try Spark", "matching Obsidian context", "saved highest-family quality pair", "One Real PASS keeps it", "two PASS downgrade one rung", "quality FAIL upgrades one", "operational failure may use one stronger fallback", "Publish output after Quick Check", "ending_verification_plan.py", "one scored/modelled global projectless task per independent real check", "all must PASS", "PASS records then self-archives", "FAIL/BLOCKED stays visible", "FAIL records exact evidence", "separate projectless repair task", "fresh verification", "up to three attempts", "BLOCKED is not verified", "Local lifecycle", "receipt-backed Obsidian learning", "Never self-verify", "same-task subagent"]))
    ...
    failures.extend(missing_terms("code-skill agent", texts["code_agent"], ["exact read-only lookup/audit stays skill-free", "Score every submission 0-100", "small low-risk edit tries Spark-low", "quality failure suppresses the matching band", "End Task is hard-required", "ending_verification_plan.py", "global projectless task per independent real test", "all must PASS", "PASS records then self-archives", "FAIL/BLOCKED stays visible", "separate projectless repair task", "fresh verification", "BLOCKED is not verified", "never self-verify", "same-task subagent"]))
    failures.extend(missing_terms("verify-skill agent", texts["verify_agent"], ["every user task launches an independent post-result Ending lifecycle", "ending_verification_plan.py", "global projectless End Task", "independent real unit/integration/API/build/render/state check", "own 0-100 score", "all required checks must PASS", "After durable PASS, self-archive", "archive may terminate the turn", "FAIL/BLOCKED stays visible", "Never summarize prior evidence", "FAIL records exact command/output/error", "separate projectless repair task", "fresh verifier", "up to three attempts", "BLOCKED never counts as verified", "Never let a verifier repair its own target", "same-task subagent"]))
