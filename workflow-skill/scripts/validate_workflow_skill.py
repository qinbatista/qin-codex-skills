...
REQUIRED_CODE = ["score role or Obsidian-context quality pair", "bounded native Obsidian route", "project/task/module/file/symbol/code", "One Real PASS retains a quality pair", "two PASS results downgrade one rung", "quality FAIL upgrades one rung", "small band tries Spark first", "zero-result, zero-token Spark operational failure", "Old local `model_experience.json` remains legacy read-only", "requested output artifact is producer work", "Cache note or report is an artifact", "mandatory post-result Ending lifecycle still runs", "ending_verification_plan.py", "exact saved project", "All checks must PASS", "remain visible", "never auto-archive or delete", "project-bound `Fix Task-{task}-{check}`", "fresh Ending check", "unreceipted known assignment is observation-only", "same-task Ending subagent", "different `verify-skill` worker performs independent verification"]
REQUIRED_VERIFY = ["Verification has two scopes", "post-result Ending Real Verify", "execute a real proportional test", "ending_verification_plan.py", "own `0-100` check score", "All required checks must PASS", "{\"type\":\"project\"", "create-launches --project-id", "ack-launch", "end_task_trigger_rate=100%", "Model stages (N):", "remain visible", "Never call `set_thread_archived`", "same saved project", "structured model assessment", "learning_eligible=false", "fresh Ending task", "BLOCKED does not count as verified"]
...
def validate(skill_dir):
    ...
    failures.extend(missing_terms("code-skill agent", texts["code_agent"], ["artifact-free exact read-only lookup/audit stays skill-free", "requested Cache/report output requires the runner", "Score every submission 0-100", "small low-risk edit tries Spark-low", "quality failure suppresses the matching band", "Every result requires ending_verification_plan.py", "project/local task", "exact saved project", "all must PASS and remain visible", "Never auto-archive/delete", "difficulty/pairs", "attempts", "first/retry pass", "suitability", "Obsidian link", "same-project repair task", "fresh verification", "never self-verify", "same-task subagent"]))
    failures.extend(missing_terms("verify-skill agent", texts["verify_agent"], ["every user task launches an independent post-result Ending", "including exact read-only work", "ending_verification_plan.py", "exact saved project", "project/local End Task", "Score each verifier 0-100", "all checks PASS", "Every terminal task stays visible", "never auto-archive/delete", "attempt", "first/retry", "suitability", "Obsidian record link/status", "Without receipt, record a non-learning assignment observation", "never summarize old evidence", "FAIL records exact command/output/error", "same-project repair", "fresh verifier", "up to three", "BLOCKED is not verified", "Never expose chain-of-thought", "same-task subagent"]))
