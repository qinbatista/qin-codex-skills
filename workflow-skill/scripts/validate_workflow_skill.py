...
REQUIRED_WORKFLOW = [
    ...
    "Every durable code change and every standard/complex/advanced, medium/high-risk, or multi-stage result uses the independent Ending lifecycle after presentation",
    "only eligible low-risk single-result small non-code work records `intentionally_skipped_simple_task`",
...
REQUIRED_CODE = ["score role or Obsidian-context quality pair", "bounded native Obsidian route", "project/task/module/file/symbol/code", "One Real PASS retains a quality pair", "two PASS results downgrade one rung", "quality FAIL upgrades one rung", "requested output artifact is producer work", "run exactly one producer-side Quick Check", "Present `CODE READY`", "Every successful durable code change", "`intentionally_skipped_simple_task` is only for eligible non-code work", "ending_verification_plan.py", "exactly one global projectless `End Task-{task}`", "codex_app__list_threads", "projectId=null", "gpt-5.3-codex-spark|xhigh", "registry-floor `gpt-5.6-luna|low`", "ENDING_CHECK_WORKER", "read every listed Skill", "never edit producer files", "All checks must PASS", "immutable origin", "codex_app__send_message_to_thread", "fresh Spark-first Ending", "do not poll"]
REQUIRED_VERIFY = ["Verification has two scopes", "mandatory post-result Ending Real Verify", "Every durable code change", "persistent global projectless", "ending_verification_plan.py", "final aggregate producer receipt", "child/subprocess receipt never launches it", "compact, one-check actions", "each check keeps its own `0-100` score", "All required checks must PASS", "{\"type\":\"projectless\"", "codex_app__list_threads", "projectId=null", "--thread-scope global --thread-project-id null", "create-launches", "ack-launch", "end_task_trigger_rate=100%", "Model stages (N):", "remains visible", "Never call `set_thread_archived`", "origin project binding", "structured `model_assessment`", "learning_eligible=false", "fresh global projectless Ending", "gpt-5.3-codex-spark|xhigh", "gpt-5.6-luna|low", "ENDING_CHECK_WORKER", "delegated_check_worker", "BLOCKED does not count as verified"]
REQUIRED_OPTIMIZATION = ["Do not infer optimization from repeated benchmark arms or exact-scoped read-only work", "Use this skill directly only when the user requests optimization", "catalog-derived adaptive producer", "positively admitted", "Every code optimization emits `ending-required`", "only eligible small non-code optimization may record `intentionally_skipped_simple_task`", "exactly one smallest local Quick Check", "ENDING_CHECK_WORKER", "different verifier", "before/after", "producer receipt", "--direct-task", "--bootstrap-task", "neither arm enters Task Analyze context"]
REQUIRED_MANAGEMENT = ["Do not load this skill for ordinary exact-scoped read-only work or Direct/Global benchmark worker arms", "directly only for an explicit routing-record", "admitted a delegated route", "native Obsidian category page", "Every task stores a `0-100` score", "Eligible small low-risk edits try Spark first", "Spark quality failure suppresses", "two PASS results trial one rung down", "obsidian_model_memory.py", "Every durable code change", "small non-code management task records `intentionally_skipped_simple_task`", "terminal memory/classification/record closeout", "Terra/Sol check workers", "terminal ledger event records an Ending result automatically", "legacy read-only", "Model stages (N):", "end_task_trigger_rate=100%", "Never push/sync/publish unless the user explicitly requested publishing"]
REQUIRED_ENTRY = ["# Task Lifecycle", "Merge this section into `~/.codex/AGENTS.md` and `~/AGENTS.md`", "Score every submission 0-100", "Resolve entry first", "Sol/high may downgrade", "Luna-max/lower may upgrade", "exact step-capability/band history wins", "no match stays <=entry", "reuse recovered/lowest-correct", "2+ material result stages auto-activate one graph", "each result/Ending records score,band,pair,purpose,deps,stop", "executes Spark-low first", "Dependency-ready independent nodes run parallel", "shared writes/order/output deps stay linear", "obsidian_adaptive_model_runner.py", "LOCKED_ROUTE_NODE loads required Skills,does code/result+one Quick Check,emits CODE READY,stops", "parent launches one End only after the final aggregate `ending-required` receipt and release", "child/subprocess receipts never launch it", "same End Task continues one bounded action at a time", "ENDING_TASK_WORKER is the sole lifecycle controller", "ENDING_CHECK_WORKER reads listed Skills", "no nested End/Fix", "dynamic_task_graph", "task_route_dispatcher.py run-plan", "task_complexity_score.py", "no reread/full-read/precheck", "output artifact MUST runner", "2 Real PASS trial down 1 rung", "quality FAIL up 1", "Only the final aggregate `ending-required` or `ending_required=true` receipt after all result nodes settle=>one independent global projectless Ending", "create-launches --producer-receipt(final) --project-id(context-only)", "`intentionally_skipped_simple_task`=>no End Task and only for eligible non-code", "missing acknowledgement=>BLOCKED", "gpt-5.3-codex-spark|xhigh", "score scopes checks only", "registry-floor Luna-low", "terminal memory/classification/record closeout", "call exact codex_app__create_thread target", "codex_app__list_threads", "projectId=null/absent", "resolve root with codex_app__list_projects", "ack threadId+hostId+originProjectId+threadProjectId=null+actual pair", "project/current-task/same-task-subtask placement", "audit-launches end_task_trigger_rate=100%", "all checks PASS", "End Task stays globally visible", "never auto-archive/delete", "attempts,first/retry pass", "suitability", "Obsidian link/status", "immutable origin session", "codex_app__send_message_to_thread", "repair_prompt", "fresh global projectless End Task", "--repair-of-lifecycle-id", "up to 3 repairs", "missing source session or failed prompt submission is BLOCKED", "BLOCKED only unavailable/external/limit", "Terminal events write local+same-ID Obsidian records", "only producer receipts move routing", "AUTO_BENCHMARK_ENTRY", "Direct `gpt-5.6-sol|ultra`", "Auto `gpt-5.6-luna|max`", "MUST child/graph then return child JSON", "gate owns separate Ending", "excludes only controller", "compare task vs +Ending", "all exact PASS", "Final Ending PASS/BLOCKED"]
...
def validate(skill_dir):
    ...
    failures.extend(missing_terms("code-skill agent", texts["code_agent"], ["artifact-free exact read-only lookup/audit stays skill-free", "references/code-writing-philosophy.md", "relevant language/platform/domain Skills", "Unity game C#", "Controller/Manager/ScriptableObject core", "exactly one smallest local Quick Check", "publish CODE READY", "broad tests/builds/UI/full lint/log cleanup/repeated review move to Ending", "Every durable code change emits ending-required", "ending_verification_plan.py", "one visible global-only projectless End Task", "list_threads", "projectId=null/absent", "gpt-5.3-codex-spark|xhigh", "gpt-5.6-luna|low", "ENDING_CHECK_WORKER", "read listed Skills", "never edit/repair/route/lifecycle", "All checks PASS", "codex_app__send_message_to_thread", "immutable origin", "fresh Spark-first Ending", "Never self-verify"]));
    failures.extend(missing_terms("verify-skill agent", texts["verify_agent"], ["every durable code change", "intentionally_skipped_simple_task is only for eligible small non-code work", "CODE READY", "one global-only projectless Ending", "ending_verification_plan.py", "codex_app__list_threads", "projectId=null/absent", "without polling", "gpt-5.3-codex-spark|xhigh", "gpt-5.6-luna|low", "ENDING_CHECK_WORKER", "read listed Skills", "never edit/repair/route/lifecycle", "Capture immutable origin", "all checks PASS", "codex_app__send_message_to_thread", "origin repairs", "fresh Spark-first Ending", "up to three", "Never self-repair"]));
    failures.extend(missing_terms("optimization-skill agent", texts["optimization_agent"], ["do not load from benchmark repetition alone", "requested optimization", "authorized reusable improvement", "admitted node", "Preserve behavior", "one smallest Quick Check", "present CODE READY", "Every code optimization emits ending-required", "intentionally_skipped_simple_task", "one visible global-only projectless Ending", "list_threads", "projectId=null/absent", "gpt-5.3-codex-spark|xhigh", "Luna-low", "ENDING_CHECK_WORKER", "never edit or own lifecycle", "A different worker verifies", "Ending time stays separate", "Never self-certify savings"]));
    failures.extend(missing_terms("management-skill agent", texts["management_agent"], ["do not load for ordinary exact-scoped read-only work or benchmark workers", "explicit request/admitted node", "Native Obsidian links plus local receipts", "Producers score 0-100", "small low-risk edits try Spark", "quality evidence moves producer routes", "Every durable code change emits ending-required", "small non-code work records intentionally_skipped_simple_task", "global-only projectless Ending", "list_threads", "projectId=null/absent", "gpt-5.3-codex-spark|xhigh", "Luna-low", "ENDING_CHECK_WORKER", "terminal memory/classification/record closeout", "Retained capabilities remain mandatory", "full regression PASS gates local deploy and GitHub push", "source/deployed/remote separate", "never publish private state"]));
