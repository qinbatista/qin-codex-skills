...
NEGATIVE_AGENT_PREFIXES = {"code_agent": "$code-skill: artifact-free exact read-only lookup/audit stays skill-free", "verify_agent": "$verify-skill: Ending only when released result has real_test, information_update, or memory_update", "optimization_agent": "$optimization-skill: do not load from benchmark repetition alone or exact-scoped read-only work", "management_agent": "$management-skill: do not load for ordinary exact-scoped read-only work or benchmark workers"}
