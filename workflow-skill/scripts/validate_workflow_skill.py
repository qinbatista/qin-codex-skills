...
REQUIRED_WORKFLOW = [
    ...
    "low-risk single-result small task is explicitly `intentionally_skipped_simple_task`",
    "standard/complex/advanced, medium/high-risk, and multi-stage work use the independent Ending lifecycle after presentation",
    ...
    "Ending begins only after the main result and its passing producer receipt is available",
    ...
    "create-launches",
    "actual pair, and any approved availability reason",
...
REQUIRED_MATRIX = ["# Adaptive And Admitted Routing Matrix", "Eligible single-node text/code production enters the compact adaptive runner exactly once", "Multi-segment work creates one dynamically scored dependency graph", "dynamic-code-graph", "ordinary-production", "independent-small-sources", "independent-large-sources", "dependent-multi-file", "explicit-routing-no-graph", "explicit-benchmark", "admitted-complex", "One receipt-valid Real PASS retains the selected pair", "two matched PASS outcomes try one lower rung", "quality failure upgrades one rung", "eligible 0-24 code/write/execute nodes use Spark", "all checks must PASS", "codex_app__send_message_to_thread", "fresh Spark-first verifier", "Direct uses the exact raw prompt", "public two-world comparison", "fixed Sol-ultra Direct task", "Luna-max-entry Auto task", "Auto excludes only its Luna-max entry controller", "--direct-task", "--bootstrap-task"]
REQUIRED_CODE = ["score role or Obsidian-context quality pair", "bounded native Obsidian route", "project/task/module/file/symbol/code", "One Real PASS retains a quality pair", "two PASS results downgrade one rung", "quality FAIL upgrades one rung", "small band tries Spark first", "zero-result, zero-token Spark operational failure", "Old local `model_experience.json` remains legacy read-only", "requested output artifact is producer work", "Cache note or report is an artifact", "intentionally_skipped_simple_task", "Ending-required", "ending_verification_plan.py", "exactly one projectless `End Task-{task}`", "gpt-5.3-codex-spark|xhigh", "registry-floor `gpt-5.6-luna|low`", "all checks must PASS", "remain visible", "never auto-archive or delete", "immutable origin session", "codex_app__send_message_to_thread", "fresh Spark-first projectless Ending", "unreceipted known Ending assignment is observation-only", "same-task Ending subagent", "different `verify-skill` worker performs independent verification"]
REQUIRED_VERIFY = ["Verification has two scopes", "post-result Ending Real Verify", "execute a real proportional test", "ending_verification_plan.py", "each check keeps its own `0-100` score", "All required checks must PASS", "{\"type\":\"projectless\"", "create-launches", "ack-launch", "end_task_trigger_rate=100%", "Model stages (N):", "remains visible", "Never call `set_thread_archived`", "exact saved project", "structured `model_assessment`", "learning_eligible=false", "fresh projectless Ending", "gpt-5.3-codex-spark|xhigh", "gpt-5.6-luna|low", "Correctness, quality, acceptance, protocol, execution, or timeout failure never changes the Ending model", "BLOCKED does not count as verified"]
REQUIRED_OPTIMIZATION = ["Do not infer optimization from repeated benchmark arms or exact-scoped read-only work", "Use this skill directly only when the user requests optimization", "catalog-derived adaptive producer", "positively admitted", "intentionally_skipped_simple_task", "All other optimization work is Ending-required", "emits `ending-required`", "different verifier", "before/after", "producer receipt", "--direct-task", "--bootstrap-task", "neither arm enters Task Analyze context"]
REQUIRED_MANAGEMENT = ["Do not load this skill for ordinary exact-scoped read-only work or Direct/Global benchmark worker arms", "directly only for an explicit routing-record", "admitted a delegated route", "native Obsidian category page", "Every task stores a `0-100` score", "Eligible small low-risk edits try Spark first", "Spark quality failure suppresses", "two PASS results trial one rung down", "obsidian_model_memory.py", "Ending Real starts with the producer receipt", "intentionally_skipped_simple_task", "all other management work emits `ending-required`", "terminal memory/classification/record closeout", "terminal ledger event records an Ending result automatically", "legacy read-only", "Model stages (N):", "end_task_trigger_rate=100%", "Never push/sync/publish unless the user explicitly requested publishing"]
REQUIRED_ENTRY = ["# Task Lifecycle", "Merge this section into `~/.codex/AGENTS.md` and `~/AGENTS.md`", "Score every submission 0-100", "Resolve entry first", "Sol/high may downgrade", "Luna-max/lower may upgrade", "exact step-capability/band history wins", "no match stays <=entry", "reuse recovered/lowest-correct", "2+ material result stages auto-activate one graph", "each result/Ending records score,band,pair,purpose,deps,stop", "executes Spark-low first", "Dependency-ready independent nodes run parallel", "shared writes/order/output deps stay linear", "obsidian_adaptive_model_runner.py", "LOCKED_ROUTE_NODE does result/Quick Check only", "parent launches one End after every `ending-required` receipt", "ENDING_TASK_WORKER does assigned checks only:no nested lifecycle", "dynamic_task_graph", "task_route_dispatcher.py run-plan", "task_complexity_score.py", "no reread/full-read/precheck", "output artifact MUST runner", "2 Real PASS trial down 1 rung", "quality FAIL up 1", "`ending-required` or `ending_required=true`=>one independent projectless Ending", "`intentionally_skipped_simple_task`=>no End Task", "missing acknowledgement=>BLOCKED", "gpt-5.3-codex-spark|xhigh", "score scopes checks only", "registry-floor Luna-low", "terminal memory/classification/record closeout", "codex_app__create_thread projectless", "resolve root with codex_app__list_projects", "ending_verification_plan.py create-launches --project-id", "ack threadId+hostId+projectId+actual pair", "audit-launches end_task_trigger_rate=100%", "all checks PASS", "End Task stays globally visible", "never auto-archive/delete", "attempts,first/retry pass", "suitability", "Obsidian link/status", "immutable origin session", "codex_app__send_message_to_thread", "repair_prompt", "fresh projectless End Task", "--repair-of-lifecycle-id", "up to 3 repairs", "missing source session or failed prompt submission is BLOCKED", "BLOCKED only unavailable/external/limit", "Terminal events write local+same-ID Obsidian records", "only receipts move routing", "AUTO_BENCHMARK_ENTRY", "Direct `gpt-5.6-sol|ultra`", "Auto `gpt-5.6-luna|max`", "MUST child/graph then return child JSON", "gate owns separate Ending", "excludes only controller", "compare task vs +Ending", "all exact PASS", "Final Ending PASS/BLOCKED"]
...
NEGATIVE_AGENT_PREFIXES = {"code_agent": "$code-skill: artifact-free exact read-only lookup/audit stays skill-free", "verify_agent": "$verify-skill: low-risk single-result small work records intentionally_skipped_simple_task", "optimization_agent": "$optimization-skill: do not load from benchmark repetition alone or exact-scoped read-only work", "management_agent": "$management-skill: do not load for ordinary exact-scoped read-only work or benchmark workers"}
...
def validate(skill_dir):
    ...
    failures.extend(missing_terms("workflow agent", texts["agent"], ["eligible small low-risk edits try Spark", "matching Obsidian context", "saved quality pair", "One receipt-backed Real PASS retains it", "two PASS trial down", "quality FAIL upgrades producer", "operational failure is neutral", "LOCKED_ROUTE_NODE publishes result/Quick Check only", "After passing receipt parent captures immutable origin", "launches ending_verification_plan.py", "exact project binding", "projectless End Task", "All PASS, stay visible", "never auto-archive/delete", "difficulty/pairs", "attempts", "first/retry", "suitability", "Obsidian link", "no receipt=>non-learning observation", "codex_app__send_message_to_thread", "source session", "fresh projectless Spark-first verification", "max 3", "Never expose private reasoning", "self-verify", "same-task subagent"]))
    ...
    failures.extend(missing_terms("code-skill agent", texts["code_agent"], ["artifact-free exact read-only lookup/audit stays skill-free", "all code writing loads references/code-writing-philosophy.md", "Unity game C# reads nearest AGENTS.md", "Controller/Manager/ScriptableObject core", "Requested Cache/report output requires the runner", "Score every submission 0-100", "safe small code tries Spark-low", "quality failure suppresses the matching band", "immutable origin session", "smallest completion/record check", "ending_verification_plan.py", "one projectless End Task", "exact project binding stays context", "gpt-5.3-codex-spark|xhigh", "gpt-5.6-luna|low", "all must PASS and remain visible", "Never auto-archive/delete", "difficulty/pairs", "attempts, first/retry pass, suitability, and Obsidian link", "codex_app__send_message_to_thread repairs in the source session", "fresh projectless verification", "Never self-verify or use a same-task subagent"]))
    failures.extend(missing_terms("verify-skill agent", texts["verify_agent"], ["low-risk single-result small work records intentionally_skipped_simple_task", "standard+, medium/high-risk, or multi-stage work emits ending-required", "one independent projectless Ending", "ending_verification_plan.py", "One plan may retain distinct checks", "one Ending runs them", "missing ack is BLOCKED", "gpt-5.3-codex-spark|xhigh", "gpt-5.6-luna|low", "never switches verifier", "immutable origin/session", "project ID/root", "real checks all PASS", "terminal task visible", "codex_app__send_message_to_thread", "origin repairs", "fresh Spark-first Ending", "up to three", "Never self-repair", "auto-archive", "expose reasoning", "same-task subagent"]))
    failures.extend(missing_terms("optimization-skill agent", texts["optimization_agent"], ["do not load from benchmark repetition alone", "requested optimization", "authorized reusable improvement", "admitted node", "Preserve behavior", "present the result before verification", "intentionally_skipped_simple_task", "all other work emits ending-required", "one visible projectless Ending", "producer receipt", "before/after evidence", "gpt-5.3-codex-spark|xhigh", "Luna-low", "A different worker must verify", "Ending time is separate from first-result time", "Never self-certify savings"]))
    failures.extend(missing_terms("management-skill agent", texts["management_agent"], ["do not load for ordinary exact-scoped read-only work or benchmark workers", "explicit request/admitted node", "Native Obsidian links plus local receipts", "Producers score 0-100", "small low-risk edits try Spark", "quality evidence moves producer routes", "intentionally_skipped_simple_task", "all other management work emits ending-required", "gpt-5.3-codex-spark|xhigh", "Luna-low", "terminal memory/classification/record closeout", "Retained capabilities remain mandatory", "full regression PASS gates local deploy and GitHub push", "source/deployed/remote separate", "never publish private state"]))
    ...
    if len(entry_body.encode("utf-8")) > 6500:
        failures.append(f"global entry bootstrap exceeds compact limit: {len(entry_body.encode('utf-8'))} > 6500 bytes")
