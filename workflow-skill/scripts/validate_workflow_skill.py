...
REQUIRED_WORKFLOW = [
    ...
    "conditional Ending lifecycle after presentation",
...
def validate(skill_dir):
    ...
    failures.extend(missing_terms("code-skill agent", texts["code_agent"], ["artifact-free exact read-only lookup/audit stays skill-free", "requested Cache/report output requires the runner", "Score every submission 0-100", "small low-risk edit tries Spark-low", "quality failure suppresses the matching band", "Complex/important results needing tests, records, or durable memory use ending_verification_plan.py", "projectless End Task", "exact project binding remains execution context", "all must PASS and remain visible", "Never auto-archive/delete", "difficulty/pairs", "attempts", "first/retry pass", "suitability", "Obsidian link", "same-project repair task", "fresh projectless verification", "never self-verify", "same-task subagent"]))
    failures.extend(missing_terms("verify-skill agent", texts["verify_agent"], ["eligible complex/important results launch an independent post-result Ending", "simple exact read-only work skips Ending", "ending_verification_plan.py", "projectless End Task", "preserve exact project binding as execution context", "Score each verifier 0-100", "all checks PASS", "Every terminal task stays visible", "never auto-archive/delete", "attempt", "first/retry", "suitability", "Obsidian record link/status", "Without receipt, record a non-learning assignment observation", "never summarize old evidence", "FAIL records exact command/output/error", "same-project repair", "fresh projectless verifier", "up to three", "BLOCKED is not verified", "Never expose chain-of-thought", "same-task subagent"]))
