...
REQUIRED_WORKFLOW = [
    ...
    "Ending Real writes the effective producer result",
...
REQUIRED_VERIFY = ["Verification has two scopes", "post-result Ending Real Verify", "execute a real proportional test", "ending_verification_plan.py", "own `0-100` check score", "All required checks must PASS", "{\"type\":\"project\"", "create-launches --project-id", "ack-launch", "end_task_trigger_rate=100%", "Model stages (N):", "remain visible", "Never call `set_thread_archived`", "same saved project", "structured model assessment", "fresh Ending task", "BLOCKED does not count as verified"]
...
def validate(skill_dir):
    ...
    failures.extend(missing_terms("workflow agent", texts["agent"], ["eligible small low-risk edits try Spark", "matching Obsidian context", "saved highest-family quality pair", "One Real PASS keeps it", "two PASS trial one rung down", "quality FAIL upgrades one", "operational failure is quality-neutral", "Publish output after Quick Check", "Every result launches ending_verification_plan.py", "exact saved project", "project/local task", "all must PASS and remain visible", "Never auto-archive/delete", "difficulty/pairs", "attempts", "first/retry pass", "suitability", "Obsidian link", "same-project repair task", "fresh verification", "up to three attempts", "Never expose private reasoning", "self-verify", "same-task subagent"]))
    ...
    failures.extend(missing_terms("code-skill agent", texts["code_agent"], ["exact read-only lookup/audit stays skill-free", "Score every submission 0-100", "small low-risk edit tries Spark-low", "quality failure suppresses the matching band", "Every result requires ending_verification_plan.py", "project/local task", "exact saved project", "all must PASS and remain visible", "Never auto-archive/delete", "difficulty/pairs", "attempts", "first/retry pass", "suitability", "Obsidian link", "same-project repair task", "fresh verification", "never self-verify", "same-task subagent"]))
    failures.extend(missing_terms("verify-skill agent", texts["verify_agent"], ["every user task launches an independent post-result Ending lifecycle", "including exact read-only work", "ending_verification_plan.py", "exact saved project", "project/local End Task", "own 0-100 score", "all checks must PASS", "Every terminal task stays visible", "never auto-archive/delete", "attempt count", "first/retry pass", "suitability", "Obsidian record link/status", "Never summarize prior evidence", "FAIL records exact command/output/error", "same-project repair task", "fresh verifier", "up to three attempts", "BLOCKED never counts as verified", "Never expose chain-of-thought", "same-task subagent"]))
