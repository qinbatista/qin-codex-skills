...
REQUIRED_CODE = ["score role or Obsidian-context quality pair", "bounded native Obsidian route", "project/task/module/file/symbol/code", "One Real PASS retains a quality pair", "two PASS results downgrade one rung", "quality FAIL upgrades one rung", "small band tries Spark first", "zero-result, zero-token Spark operational failure", "Old local `model_experience.json` remains legacy read-only", "requested output artifact is producer work", "Cache note or report is an artifact", "mandatory post-result Ending lifecycle still runs", "ending_verification_plan.py", "projectless `End Task-{task}-{check}`", "All checks must PASS", "remain visible", "never auto-archive or delete", "project-bound `Fix Task-{task}-{check}`", "fresh projectless Ending check", "unreceipted known assignment is observation-only", "same-task Ending subagent", "different `verify-skill` worker performs independent verification"]
REQUIRED_VERIFY = ["Verification has two scopes", "post-result Ending Real Verify", "execute a real proportional test", "ending_verification_plan.py", "own `0-100` check score", "All required checks must PASS", "{\"type\":\"projectless\"", "create-launches", "ack-launch", "end_task_trigger_rate=100%", "Model stages (N):", "remain visible", "Never call `set_thread_archived`", "exact saved project", "structured model assessment", "learning_eligible=false", "fresh Ending task", "BLOCKED does not count as verified"]
...
REQUIRED_ENTRY = ["# Task Lifecycle", "Merge this section into `~/.codex/AGENTS.md`", "Score every submission 0-100", "Resolve entry first", "Sol/high may downgrade", "Luna-max/lower may upgrade", "exact step-capability/band history wins", "no match stays <=entry", "reuse recovered/lowest-correct", "2+ material result stages auto-activate one graph", "each result/Ending records score,band,pair,purpose,deps,stop", "executes Spark-low first", "Dependency-ready independent nodes run parallel", "shared writes/order/output deps stay linear", "obsidian_adaptive_model_runner.py", "LOCKED_ROUTE_NODE does result/Quick Check only", "entry parent launches Ending after passing receipt", "ENDING_TASK_WORKER does assigned check only:no nested lifecycle", "dynamic_task_graph", "task_route_dispatcher.py run-plan", "task_complexity_score.py", "no reread/full-read/precheck", "output artifact MUST runner", "2 Real PASS trial down 1 rung", "quality FAIL up 1", "Every result,including read-only,triggers a visible projectless End Task", "codex_app__create_thread projectless", "resolve root with codex_app__list_projects", "ending_verification_plan.py create-launches --project-id", "ack threadId+hostId+projectId", "audit-launches end_task_trigger_rate=100%", "handoff is pending", "all checks PASS", "End Tasks stay globally visible", "never auto-archive/delete", "attempts,first/retry pass", "suitability", "Obsidian link/status", "same-project Fix", "fresh projectless End Task", "up to 3 repairs", "BLOCKED only unavailable/external/limit", "Terminal events write local+same-ID Obsidian records", "only receipts move routing", "AUTO_BENCHMARK_ENTRY", "Direct `gpt-5.6-sol|ultra`", "Auto `gpt-5.6-luna|max`", "MUST child/graph then return child JSON", "gate owns separate Ending", "excludes only controller", "compare task vs +Ending", "all exact PASS", "Final PASS/BLOCKED Ending-only"]
REQUIRED_SELECTION = ["# Catalog-Generated Model Selection", "assets/model-capability-ladder.json", "scripts/model_registry.py", "`~/.codex/models_cache.json`", "bootstrap it once", "Only an explicit user model-update request", "highest numeric GPT family", "Older numeric families remain catalog-only", "bounded native Obsidian route", "merged local history", "terminal Ending event automatically records the matched producer verdict", "optional priority producer", "native project → Model Switch → category → shared-category projection", "Exact read-only", "task_complexity_score.py", "obsidian_adaptive_model_runner.py", "multi-node production task runs one `dynamic_task_graph`", "based on its own score, not the parent task score"]
...
def validate(skill_dir):
    ...
    failures.extend(missing_terms("code-skill agent", texts["code_agent"], ["artifact-free exact read-only lookup/audit stays skill-free", "requested Cache/report output requires the runner", "Score every submission 0-100", "small low-risk edit tries Spark-low", "quality failure suppresses the matching band", "Every result requires ending_verification_plan.py", "projectless End Task", "exact project binding remains execution context", "all must PASS and remain visible", "Never auto-archive/delete", "difficulty/pairs", "attempts", "first/retry pass", "suitability", "Obsidian link", "same-project repair task", "fresh projectless verification", "never self-verify", "same-task subagent"]))
    failures.extend(missing_terms("verify-skill agent", texts["verify_agent"], ["every user task launches an independent post-result Ending", "including exact read-only work", "ending_verification_plan.py", "projectless End Task", "preserve exact project binding as execution context", "Score each verifier 0-100", "all checks PASS", "Every terminal task stays visible", "never auto-archive/delete", "attempt", "first/retry", "suitability", "Obsidian record link/status", "Without receipt, record a non-learning assignment observation", "never summarize old evidence", "FAIL records exact command/output/error", "same-project repair", "fresh projectless verifier", "up to three", "BLOCKED is not verified", "Never expose chain-of-thought", "same-task subagent"]))
