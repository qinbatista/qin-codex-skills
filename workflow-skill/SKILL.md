...
Use result-producing Workflow for one Obsidian-context adaptive quality producer with a one-rung-stronger operational fallback, or after full `task-analyze-skill` returns a dynamically scored locked multi-node route. Ineligible ordinary work remains inline; a low-risk single-result small task is explicitly `intentionally_skipped_simple_task`, while standard/complex/advanced, medium/high-risk, and multi-stage work use the independent Ending lifecycle after presentation. Continue in the same task; no hook is used; never print the private machine plan.
...
4. One admitted producer runs through `obsidian_adaptive_model_runner.py --emit-result`; before the producer call, show its flushed `model-switch-notice` with the user-visible pair and task-part announcement. Ordinary text/code starts at the matching history boundary or, without history, the weaker of its contextual pair and entry anchor. One Real PASS retains the pair, two Real PASS results trial one rung down, and quality FAIL upgrades one rung. The final dynamic foreground and Ending manifests each carry `model_switch_summary`: every planned result node and the single Ending node are present, that Ending is pending before release then reconciled once, and the aggregate names Spark use, quality/fallback/failure counts, parallel result waves, and unsplit continuity groups. Code completion includes one bounded Quick Check before presentation, never a separate verifier.
...
4. for every `ending-required` producer summary, capture the immutable origin session (`thread_id`, `host_id`, exact project ID, and project root), release the Ending Real handoff, resolve the exact saved project with `codex_app__list_projects`, run `ending_verification_plan.py create-launches --project-id <resolved-id>`, invoke its single generated projectless `codex_app__create_thread` call, acknowledge the returned `threadId`, `hostId`, project ID, actual pair, and any approved availability reason, and require `audit-launches` to report `end_task_trigger_rate=100%`. Name it exactly `End Task-{concise related task name}`, link it, and return without waiting. A missing acknowledgement is BLOCKED, never a successful final. Worker prompts keep the originating project root as execution context and use project-root-relative paths. A handoff without host acknowledgement is pending, never triggered. Keep the global End Task visible after terminal status; never auto-archive or delete it. When `create_thread` is callable, invoke the generated projectless call; never inspect app-server schemas/commands or emulate task creation. If thread tools are unavailable, return the completed result and exact BLOCKED handoff immediately; the outer host creates the End Task. Exact project-resolution failure follows the same boundary. Never poll or self-run Ending. A low-risk single-result small task reports `intentionally_skipped_simple_task` and does not enter this step.
...
For every Ending-required task, Ending begins only after the main result and its passing producer receipt is available. A `LOCKED_ROUTE_NODE` never starts or waits for Ending; the entry parent binds that receipt, builds `ending_verification_plan.py`, and creates exactly one persistent projectless `End Task-{task}` containing the explicit unit/integration/API/build/render/state checks required by that task. Each check keeps its own `0-100` score for proportional scope and classification, but the one Ending primary is fixed `gpt-5.3-codex-spark|xhigh`. Only explicit model, effort, scheduler, or required-modality unavailability permits the registry-floor `gpt-5.6-luna|low` fallback; correctness, quality, protocol, execution, timeout, or acceptance failure never switches the verifier. All required checks must PASS. The entry parent creates the task only when thread tools are callable; otherwise it emits a BLOCKED handoff for the outer host. Start the local lifecycle with score, plan, and `--producer-receipt` when present. A low-risk single-result small task is explicitly `intentionally_skipped_simple_task` instead of creating a completion-only Ending.
...
Do not run broad verification before the user first sees the result; show the main result after Quick Check. Ending bypasses result-producing performance admission. The Ending worker starts with `ENDING_TASK_WORKER` and runs its bounded assigned checks plus semantic acceptance, preferring a digest/state check of a current immutable PASS release report over repeating the exhaustive gate. PASS requires the expected observable result. FAIL or acceptance mismatch records exact command/output/error and automatically submits the generated repair prompt through `codex_app__send_message_to_thread` to the immutable origin session; that session repairs the authorized result, presents a new result, and starts a fresh Spark-first Ending task. Continue for up to three repairs. BLOCKED never counts as verified.
...
Delegated code nodes retain `code-skill`. Eligible small low-risk code, write, command, probe, transform, and execute segments use the catalog Spark priority producer from their node score, regardless of parent score; other production executes its score role or saved contextual quality pair. Every task's single projectless Ending uses fixed `gpt-5.3-codex-spark|xhigh`; score scopes its checks only. Explicit Spark model/effort/scheduler or required-modality unavailability permits only the registry-floor Luna-low fallback, never a quality-driven verifier escalation. Local execution does not fabricate producer metadata.
