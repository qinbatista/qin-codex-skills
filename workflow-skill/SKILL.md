...
Use result-producing Workflow for one Obsidian-context adaptive quality producer with a one-rung-stronger operational fallback or a dynamically scored locked multi-node route. Ineligible ordinary work remains inline. Every durable code change and every standard/complex/advanced, medium/high-risk, or multi-stage result uses the independent Ending lifecycle after presentation; only eligible low-risk single-result small non-code work records `intentionally_skipped_simple_task`. Continue in the same task; no hook is used; never print the private machine plan.
...
For every durable code change and every other Ending-required task, Ending begins only after the whole producer flow has settled, the final aggregate result has released, and its passing producer receipt is available; code uses `CODE READY`. A finished child or subprocess never opens it. The entry parent creates one persistent global projectless `End Task-{task}` and proves that placement through `codex_app__list_threads` readback with null/absent project context. It never creates Ending in the source project, the current task, or a same-task subtask. One Ending primary is fixed `gpt-5.3-codex-spark|xhigh` as its sole lifecycle owner; only explicit controller availability permits registry-floor `gpt-5.6-luna|low` fallback. Each check keeps a score, surface, execution mode, and required Skills. Deterministic command/syntax/unit/API/file-state checks run directly on Spark. Semantic runtime/integration/code-quality/prompt/UI/visual checks may use the saved Terra/Sol `ENDING_CHECK_WORKER` pair. Workers are bounded internal subagents, not nested Ending tasks, and cannot edit, repair, route, or close lifecycle. All required checks must PASS. Only eligible small non-code may skip Ending.
...
Delegated code nodes retain `code-skill`. Eligible small low-risk implementation may still use Spark-low as producer, but every durable code result gets one detached Ending. Its controller is fixed Spark-xhigh; score and verification surface select only the bounded check scope and optional Terra/Sol check-worker pair. Spark controller availability alone permits Luna-low fallback. Local execution does not fabricate receipts.
