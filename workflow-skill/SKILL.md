...
The explicit workflow turns a request into a target map, chooses the relevant executor skill routes, drives the major goal to completion, returns the user-facing result when the `Main Goal Done Gate` is met, and moves post-goal validation, verification, tests, documents, sync/status proof, and wiki/log closeout into background `Ending Workflow` subagent(s) when subagent tools are callable for the current task. Final response is allowed after the required background ending workers are dispatched and their names/purposes are reported, or after the explicit blocked status is reported when subagent spawning is not callable or not authorized. The user does not wait for every ending worker to finish unless they explicitly ask to wait.
...
Use the fast path when the user asks for a simple, obvious, low-risk action and the expected result is directly knowable without broad reading or proof. Execute it directly, run only the smallest action-done confirmation in the main task, return the result to the user-facing path, then delegate the same task-local `Ending Workflow` task to subagent(s) when subagent tools are callable for the current task.
...
Ending workers do not start their own workflow tree. The parent prompt must make the worker execute directly: no `$workflow-skill`, no global route restart, and no nested subagents. If an ending worker needs work outside its bounded assignment, it reports blocked instead of delegating again.
...
- A subagent/delegation tool is `callable` only when it is exposed in the current environment and allowed for this current task by higher-priority system, developer, and tool instructions. If a tool exists but higher-priority instructions forbid spawning unless the user explicitly requests subagents, delegation, or parallel agent work, treat it as not callable for this task and report `Ending Workflow blocked: no subagent tool` with the policy reason.
- When the main goal is done, start bounded `Ending Workflow` subagent(s) when the subagent tool is callable for the current task, report each agent name/purpose, and return the result to the user path without waiting for every ending worker to finish. This applies no matter whether the task is fast-path simple, lightweight, or explicit workflow.
...
- The main agent must call the available subagent/delegation tool for the ending task whenever that tool is callable for the current task. A worker may report that no durable file updates are needed only after completing the `Related Closeout Inventory`; a no-op from the changed file alone is invalid.
...
- `Ending Worker Direct-Execution Contract`: every Ending Workflow subagent prompt must explicitly say it is an Ending Workflow worker only, must not call `$workflow-skill`, must not restart the global workflow route, and must not create, spawn, resume, or wait on subagents. The worker must do the assigned task directly with local tools, code, commands, and bounded file edits. If the worker thinks another subagent or another workflow route is needed, it must report blocked with checked sources and remaining items instead of delegating again.
- Ending worker prompts must include the concrete purpose, allowed files or directories, allowed commands or checks, forbidden actions such as pushing when not requested, expected evidence, and the final report shape. Do not give an ending worker a vague instruction such as "use workflow-skill" or "handle closeout"; the prompt itself must carry the bounded task.
- If no subagent tool is callable for the current task and closeout is required by higher-priority project/global rules, state `Ending Workflow blocked: no subagent tool` with the tool-policy or availability reason, then keep only the minimum required closeout in the main agent and avoid expanding it into broad main-lane work.
- If no subagent tool is callable for the current task and closeout would otherwise be optional, state `Ending Workflow blocked: no subagent tool` with the reason instead of deferring or silently skipping it.
...
This is a hard pre-final gate and hard pre-final dispatch gate. The final response is allowed after the main agent has created the required background subagent(s), reported their names/purposes, and stated that they are running in the background, or stated that the subagent tool is not callable for the current task. The user does not wait for all ending workers to complete unless they explicitly asked to wait.

1. Detect whether a subagent/delegation tool is callable for the current task: it must be exposed in the environment and allowed by higher-priority system, developer, and tool instructions. When it is callable, identify the independent closeout purposes, then call all independent purpose-specific `Ending Workflow` workers in parallel before the final response. A plan, queue note, intention, or written promise is not delegation.
2. For every worker prompt, include the `Ending Worker Direct-Execution Contract`: do not call `$workflow-skill`, do not restart global routing, do not create/spawn/resume/wait on subagents, and execute the assigned purpose directly with local tools. If another worker would be needed, report blocked instead of delegating.
3. Record every worker id, name, or tool-return handle immediately as `Ending Workflow delegated: <worker-id-or-name>` with its purpose.
4. Do not wait for every ending worker to complete before final response. Let ending workers run in the background and rely on their notifications or a later follow-up to report completed files, no-op inventory summaries, blocked closeout, or failures.
5. If a worker later reports a real failure, treat it as a background follow-up. Repair in a new step only if the user asks or the failure proves a required precondition was misclassified.
6. If no subagent/delegation tool is callable for the current task, including when a higher-priority tool policy forbids spawning without explicit user authorization, state `Ending Workflow blocked: no subagent tool` with the reason before doing any required minimum closeout in the main agent.
7. Every final response after task work must report visible background ending dispatch states for each worker: `Ending Workflow delegated: <worker-id-or-name> running (<purpose>)`, `Ending Workflow delegated: <worker-id-or-name> completed`, `Ending Workflow delegated: <worker-id-or-name> no-op`, `Ending Workflow delegated: <worker-id-or-name> blocked`, or `Ending Workflow blocked: no subagent tool`. A completed/no-op/blocked state is only required when the worker already finished before final response. A no-op final state must include a concise checked-source inventory summary. A blocked state must include remaining items and report blocked closeout with remaining items; a blocked state must include remaining items.
