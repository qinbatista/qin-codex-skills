...
Inline work uses one direct task action or direct execution surface, then shows the completed main result immediately. A producer summary with `ending-required` or `ending_required=true` starts one independent Ending afterward; `intentionally_skipped_simple_task` does not.
