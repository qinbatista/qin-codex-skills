...
class SyncGlobalSkillsReadmeTest(unittest.TestCase):
    ...
    def test_english_readme_uses_durable_template_and_current_contract(self):
        ...
        self.assertIn("Obsidian record link/status", readme)
        self.assertIn("never auto-archive or delete themselves", readme)
    ...
    def test_chinese_readme_is_compact_diagram_first_and_has_memory_contract(self):
        ...
        self.assertIn("FAIL 会在同一项目建立 Fix Task", readme)
        self.assertIn("PASS/FAIL/BLOCKED task 都保持可见", readme)
        self.assertIn("End/Fix task 永不自动归档或删除", readme)
        self.assertIn("PASS/FAIL/BLOCKED task 都保持可见", readme)
