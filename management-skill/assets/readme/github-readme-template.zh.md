...
3. **每个 producer 返回有效 PASS receipt 后，由入口父任务在当前准确的已保存项目中为每个真实检查另开一个评分并选模的 `End Task-<任务名>-<检查>`。** 锁定 producer 不递归启动 Ending；父任务绑定 receipt、按规范项目根解析项目，并要求真实 project/local task 回执。
