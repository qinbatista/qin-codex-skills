...
Task Analyze owns selection and storage. Every eligible text/code producer runs the catalog-derived adaptive route, and starts a post-presentation Ending only when the result is complex or important and needs tests, records, or durable memory. The terminal ledger event records the result automatically; tool-only routes have no adaptive producer sample.
