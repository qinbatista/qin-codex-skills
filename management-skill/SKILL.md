...
- Categories/model roles are shared cold-start hints only. Normal tasks read and execute the saved quality pair. Only an explicit user model-update request may rescan local `~/.codex/models_cache.json`; select its highest numeric GPT family and order variants weakest to strongest, but keep the last valid ladder if the cache is unavailable and never fetch models over the network. Search only within matching project/task/module/file/symbol/code context: one Real PASS retains the pair, two PASS results trial one rung down, correctness/quality FAIL moves one rung up, and a like-for-like cost tie ranks tokens then time then weaker pair. Reuse the frozen pair with `trial=false`. The catalog fast producer is schedule-only for disjoint source branches. Reuse existing Obsidian project hierarchy nodes before creating missing task/module/file/symbol nodes.
...
The durable README sources are `assets/readme/github-readme-template.md` and `assets/readme/github-readme-template.zh.md`. Keep both reviewed, compact, direct, and scan-first. Prefer one-line visible items, use tables only when shared columns are necessary, and remove prose already carried by a visual or source-backed evidence. They must use the exact product name `Auto Best Model`, identify it as Codex-only, explain that the saved highest-family ladder changes only on an explicit local model-update request, and describe direct contextual-quality execution, two-PASS descent, one-FAIL ascent, schedule-only fast branches, receipt-bound Ending learning, and the two-world task versus task+check benchmark without naming a hypothetical future family. Keep project change memory distinct from adaptive model learning, and show that ModelExperience reuses existing project hierarchy nodes. Emphasize **finish job first, return result, background verify**. Show the full producer/verification flow and responsive static benchmark visual, list exactly eight public Skills, and include source-backed token/time plus Ending token/time evidence. Never embed raw prompts, paths, session IDs, receipts, private model records, or internal-only details. `scripts/sync_global_skills.py` reads the templates when generating root `README.md` and `README.zh.md`.
