...
- Store active private model experience only as Obsidian Markdown under `Projects/<project-key>/ModelExperience/`; never mirror it. Old local `task-analyze-skill/local/adaptive-routing/model_experience.json` is legacy read-only history.
...
- Categories/model roles are shared cold-start hints only. Every active route carries the exact full GPT-5.6 Luna-low→Sol-ultra quality/fallback ladder. Eligible text/code producers try Spark first at low effort for easy work and high effort for complex work. Search only within matching project/task/module/file/symbol/code context while calibration is open; move one GPT-5.6 effort/model rung at a time, then reuse the frozen quality pair with `trial=false`. Ending Real records the matched producer receipt; Spark operational failures stay neutral, while a Spark correctness/quality failure starts a new GPT-5.6 repair lifecycle.
...
Ending Real writes a sanitized project-scoped record through `../project-memory-skill/scripts/obsidian_model_memory.py record` only after a matched producer receipt establishes pass/fail. Obsidian is the sole active private authority; there is no local model-learning fallback. If the vault is unavailable, write nothing and use shared cold-start/inline behavior. Central `TaskModelExperience/` is legacy read-only history.
