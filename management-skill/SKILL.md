...
Use `sync_global_skills.py deploy --source-dir <repository-root> --skills-dir <global-skills-directory>` for an explicit local installation from the maintained repository source. The user's install/update request authorizes the initial local write and any repair-driven reinstallation in that task; never ask the user to run a gate, repeat a command, or approve each retry. After bounded source-integrity and public-safety checks, snapshot the current managed Skills and global `AGENTS.md` targets, preserve unrelated local Skills and `task-analyze-skill/local/`, then install the eight managed Skills and canonical Task Lifecycle contract provisionally before regression testing.

While that provisional installation is active, Codex runs the installed Skill platform checker plus the complete deployed retained-capability gate. A deployment from the maintained local repository also runs the source gate and real-sample attestations. A `pull` from an already-published remote clone runs installed/deployed/platform/parity checks and repository/public-safety validation; it must not require untracked `Cache/` evidence that publication intentionally excludes. Execute the real command path on the current host and use static or injected branch tests for the other supported Windows, macOS, and Linux paths; state which operating systems were physically executed. Any failure is repair evidence, not a request for user work: restore the last installation automatically, repair the maintained source, run one focused Quick Check, reinstall, and repeat the post-install checks. Report `Installation complete` only after every applicable check passes. If an external or unavailable condition prevents repair, restore the previous installation and report the exact blocker without giving the user a gate procedure. Local deployment never publishes to GitHub.
...
When a management action touches Skill runtime code (including managed Skill helper scripts), treat it as Skill runtime surface. Local installation executes the checker from the provisional installed copy before completion; GitHub publication executes it before external mutation. Codex owns checker failures and repairs them instead of assigning commands to the user.
