...
Task Analyze owns selection and storage. Every eligible text/code producer runs the catalog-derived adaptive route, and every user submission receives a bounded personal-memory relevance scan. A post-presentation Ending starts for every new user task; no candidate means no preference-memory write, never no Ending. The terminal ledger event records the result automatically; tool-only routes have no adaptive producer sample.
...
For management work, present the completed requested result immediately; do not insert a foreground Mini or Fast Verify. Use the compact Result Model Disclosure from `task-analyze-skill/references/route-contract.md` verbatim. Do not expand it into the former repeated model, evidence, previous-model, switch-summary, or reason lines. Management is only the publication/deployment stage of a composite task: never replace implementation, focused-test, publication, deployment, Obsidian-record, and Ending identities with one management pair. When the route has two or more model-executed nodes, append the generated `Model stages (N):` block from the final `model_switch_summary`. First-result time ends at that presentation. Every new management task launches a visible projectless Ending Task in the global task list for Real Verify and terminal memory/classification/record closeout; deeper local/remote comparison, hash/no-diff proof, reports, logs, docs, or memory use real checks, while simple reads and one-value results use the smallest completion/record check. The originating project root remains its execution context. It must have a real `create_thread` acknowledgement and `audit-launches` must report `end_task_trigger_rate=100%`; a handoff file alone is not deployment verification. It remains visible after PASS/FAIL/BLOCKED and prints the structured model-attempt/suitability/next-route/Obsidian record summary; no End/Fix task auto-archives or deletes itself.
