...
Task Analyze owns selection and storage. Every eligible text/code producer runs the catalog-derived adaptive route. Ending-required work receives a bounded personal-memory relevance scan inside its Ending; a low-risk single-result small task records `intentionally_skipped_simple_task` and skips both. Standard/complex/advanced work, any medium/high-risk task, and multi-stage work emit `ending-required`; their parent creates and acknowledges the projectless Ending. The terminal ledger event records an Ending result automatically; tool-only routes have no adaptive producer sample.
...
- Record sanitized session effort separately: project/task/module, hashed session key, same-task turns, user-effort count, repeated-failure state, solving surface, information burden, model difficulty/family, estimated step count/class, estimated low-through-ultra effort, prior model pair(s), preferred solving pair, and selected route. Matching corrective feedback chooses model family from difficulty/information and effort from estimated steps before moving to a stronger model at that effort; a topic jump starts a separate state. Never mirror raw prompts, results, or session IDs.
...
The durable README sources are `assets/readme/github-readme-template.md` and `assets/readme/github-readme-template.zh.md`. Keep both reviewed, compact, direct, and scan-first. Prefer one-line visible items, use tables only when shared columns are necessary, and remove prose already carried by a visual or source-backed evidence. They must use the exact product name `Auto Best Model`, identify it as Codex-only, explain that the saved highest-family ladder changes only on an explicit local model-update request, and describe 0–100 task scoring, exact step-capability history, bidirectional entry-aware producer routing, Spark-low priority for eligible small low-risk edits, separate compound-step assignments, two-PASS descent, one-FAIL ascent, receipt-bound producer learning, the explicit `intentionally_skipped_simple_task` exception, and one fixed Spark-xhigh projectless Ending for Ending-required work. Score scopes Ending checks only; explicit Spark availability/capability failure permits the registry-floor Luna-low fallback, never quality escalation. Describe the immutable-origin-session repair prompt plus fresh Ending retry contract. Publish the newest frozen two-world benchmark exactly as measured. Label stable selected-execution time/tokens as the primary savings, keep matching/controller/calibration/retry/fallback/repair/probe/Ending cost as separate diagnostics, show actual first-result wait without relabelling it, and report the Luna/Sol entry-invariance result. Retain older frozen assets as historical evidence without carrying their savings into the current claim or naming a hypothetical future family. Keep project change memory distinct from adaptive model learning, and show the native project → Model Switch → stable category → shared-category graph while task/module/file/symbol stay record fields. Emphasize **finish job first, return result, then launch one separate fast Ending for every Ending-required task**. Show the full producer/verification flow and responsive static benchmark visual, list exactly eight public Skills, and include source-backed token/time plus Ending correctness evidence. Never embed raw prompts, paths, session IDs, receipts, private model records, or internal-only details. `scripts/sync_global_skills.py` reads the templates when generating root `README.md` and `README.zh.md`.
...
For management work, present the completed requested result immediately; do not insert a foreground Mini or Fast Verify. Use the compact Result Model Disclosure from `task-analyze-skill/references/route-contract.md` verbatim. Do not expand it into the former repeated model, evidence, previous-model, switch-summary, or reason lines. Management is only the publication/deployment stage of a composite task: never replace implementation, focused-test, publication, deployment, Obsidian-record, and Ending identities with one management pair. When the route has two or more model-executed nodes, append the generated `Model stages (N):` block from the final `model_switch_summary`. First-result time ends at that presentation. A low-risk single-result small management task records `intentionally_skipped_simple_task`; all other management work emits `ending-required` and launches a visible projectless Ending Task in the global task list for Real Verify and terminal memory/classification/record closeout. Deeper local/remote comparison, hash/no-diff proof, reports, logs, docs, or memory use real checks. The originating project root remains its execution context. It must have a real `create_thread` acknowledgement and `audit-launches` must report `end_task_trigger_rate=100%`; a missing acknowledgement is BLOCKED, not deployment verification. It remains visible after PASS/FAIL/BLOCKED and prints the structured model-attempt/suitability/next-route/Obsidian record summary; no End/Fix task auto-archives or deletes itself.
