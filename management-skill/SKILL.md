...
Keep source portable and free of private history, machine paths, and secrets. Temporary support belongs in task-owned `Cache/temp-*` and is removed after release readback unless review or debugging still needs it. Reusable tests stay with their skill; retained `Cache/remote-*` evidence requires a project use and reason and is preserved with sync destination pending until selected.
