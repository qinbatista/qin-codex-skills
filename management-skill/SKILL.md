...
- Categories/model roles are shared cold-start hints only. Normal tasks read the saved quality ladder. Only an explicit user model-update request may rescan local `~/.codex/models_cache.json`; select its highest numeric GPT family and order variants weakest to strongest, but keep the last valid ladder if the cache is unavailable and never fetch models over the network. Use the saved optional priority producer first at low effort for easy work and high effort for complex work. Search only within matching project/task/module/file/symbol/code context while calibration is open; move one quality effort/model rung at a time, then reuse the frozen pair with `trial=false`. Reuse existing Obsidian project hierarchy nodes before creating missing task/module/file/symbol nodes. Priority-producer operational failures stay neutral; correctness/quality failure is recorded before a new contextual quality-pair repair lifecycle.
...
The durable README sources are `assets/readme/github-readme-template.md` and `assets/readme/github-readme-template.zh.md`. Keep both reviewed, compact, direct, and scan-first. Prefer one-line visible items, use tables only when shared columns are necessary, and remove prose already carried by a visual or source-backed evidence. They must use the exact product name `Auto Best Model`, identify it as Codex-only, explain that the saved highest-family ladder changes only on an explicit local model-update request, and describe the saved optional priority producer, contextual quality fallback, receipt-bound Ending learning, and quality-repair lifecycle without naming a hypothetical future family. Keep project change memory (local JSONL authority with optional Obsidian projection) distinct from adaptive model learning (private project-scoped Obsidian only), and show that ModelExperience reuses existing project hierarchy nodes. Show the producer/verification flow and a responsive static model-flow visual, show project/module/file memory, list exactly eight public Skills, and include the latest verified benchmark visual backed by sanitized public evidence; expose PASS or FAIL exactly as the frozen strategy gate reports it. Never embed raw benchmark dumps, prompts, paths, session IDs, receipts, private model records, or internal-only details. `scripts/sync_global_skills.py` reads the templates when generating root `README.md` and `README.zh.md`.
